function StatsFromDist_AggVars=StatsFromDistribution_AggVars_PType(StationaryDist, Policy,n_d,n_a,n_z,N_j,Names_i,d_grid, a_grid, z_grid,Phi_aprime,FnsToEvaluateFn, Parameters,FnsToEvaluateParamNames, PTypeDistNames, options, AgeDependentGridParamNames)
    ...
    for kk=1:numFnsToEvaluate
        if isa(FnsToEvaluateFn{kk},'struct')
            if isfield(FnsToEvaluateFn{kk}, Names_i{ii})
                FnsToEvaluateFn_temp{jj}=FnsToEvaluateFn{kk}.(Names_i{ii});
            ...
            FnsToEvaluateFn_temp{jj}=FnsToEvaluateFn{kk};
            FnsToEvaluateParamNames_temp(jj).Names=FnsToEvaluateParamNames(kk).Names;
            WhichFnsForCurrentPType(kk)=jj; jj=jj+1;
        end
    end
