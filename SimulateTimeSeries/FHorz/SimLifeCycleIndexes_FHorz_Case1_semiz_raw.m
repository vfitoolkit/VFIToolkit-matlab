function SimLifeCycleKron=SimLifeCycleIndexes_FHorz_Case1_semiz_raw(PolicyIndexesKron,N_d1,N_j,cumsumpi_z_J,cumsumpi_semiz_J,seedpoint,simperiods)
...
if N_d1==0
    ...
    for jj=0:periods
        ...
        temp=PolicyIndexesKron(:,currstate(1),currstate(2),currstate(3),jj+initialage); % (d2,aprime)
        % d2=temp(1);
        currstate(1)=temp(2);
    ...
    for jj=0:periods
        ...
        temp=PolicyIndexesKron(:,currstate(1),currstate(2),currstate(3),jj+initialage); % (d1,d2,aprime)
        currstate(1)=temp(3);

        [~,currstate(2)]=max(cumsumpi_semiz_J(currstate(2),:,temp(2),jj+initialage)>rand(1,1));
        [~,currstate(3)]=max(cumsumpi_z_J(currstate(3),:,jj+initialage)>rand(1,1));
    end
    SimLifeCycleKron(4,initialage:(initialage+periods))=initialage:1:(initialage+periods);
end
