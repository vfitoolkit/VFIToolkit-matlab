...
if simoptions.experienceasset==1
    ...
    if prod(simoptions.n_semiz)==0
        SimPanelIndexes=SimPanelIndexes_FHorz(gather(InitialDist),gather(Policy(l_d+1:end,:,:,:)),n_d,n_a,n_z,N_j,gather(pi_z_J), simoptions); % Policy_aprime
    else
...
if N_d==0
    ...
    daprimePolicy_gridvals=zeros(N_a*N_semizze,l_d+l_aprime,N_j); % Note: N_e=1 if no e variables
end

for jj=1:N_j    
    if N_d==0
        [~,aprimePolicy_gridvals_j]=CreateGridvals_Policy(Policy(:,:,jj),n_d,n_a,n_a,n_semizze,d_grid,a_grid,simoptions,1, 1); % handles gridinterplayer=1 in here
