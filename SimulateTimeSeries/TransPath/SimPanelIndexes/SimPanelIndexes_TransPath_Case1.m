function SimPanel=SimPanelIndexes_TransPath_Case1(PolicyPathKron, T, AgentDist_initial, n_d, n_a, n_z, pi_z, simoptions)
...
l_daprime=l_d+l_a; % for now, just the basics

SimPanel=zeros(l_daprime+l_a+l_z,simoptions.simperiods,simoptions.numbersims); % (a,z)
if simoptions.parallel==0
    ...
    parfor ii=1:simoptions.numbersims % This is only change from the simoptions.parallel==0
        seedpoint=seedpoints(ii,:);
        % Since a finite-horizon value fn problem and a transition path are much the same thing we can just piggy back on the codes for FHorz.
        SimLifeCycleKron=SimLifeCycleIndexes_FHorz_Case1_raw(PolicyPathKron,N_d,T,cumsumpi_z_T, [seedpoint,1], simperiodstemp);        

        SimPanel_ii=zeros(l_daprime+l_a+l_z,simperiodstemp);
        SimPanel_ii(l_daprime+1:l_daprime+l_a,:)=ind2sub_vec_homemadet(n_a, SimLifeCycleKron(1,:)); % a
        SimPanel_ii(l_daprime+l_a+1:l_daprime+l_a+l_z,:)=ind2sub_vec_homemadet(n_z, SimLifeCycleKron(2,:)); % z
        % SimLifeCycleKron(3,:) is t, but we don't care
