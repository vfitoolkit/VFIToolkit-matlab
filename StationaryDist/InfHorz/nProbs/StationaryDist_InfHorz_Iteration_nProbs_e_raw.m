function StationaryDist=StationaryDist_InfHorz_Iteration_nProbs_e_raw(StationaryDist,Policy_aprime,PolicyProbs,N_probs,N_a,N_z,N_e,pi_z,pi_e,simoptions)
...
% Policy_aprime and PolicyProbs are currently [N_a,N_z*N_e,N_probs]
...
Policy_aprimez=gather(reshape(Policy_aprimez,[N_a*N_z*N_e,N_probs])); % sparse() requires inputs to be 2-D
PolicyProbs=gather(reshape(PolicyProbs,[N_a*N_z*N_e,N_probs])); % sparse() requires inputs to be 2-D
...
Gammatranspose=sparse(Policy_aprimez,II2,PolicyProbs,N_a*N_z,N_a*N_z*N_e); % Note: sparse() will accumulate at repeated indices
