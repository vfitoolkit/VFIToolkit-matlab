...
Policy_dsemiexo=gather(reshape(Policy_dsemiexo,[N_a*N_semiz,N_j])); % (a,semiz,j)
Policy_aprime=reshape(Policy_aprime,[N_a*N_semiz,N_probs,N_j]);
PolicyProbs=reshape(PolicyProbs,[N_a*N_semiz,N_probs,N_j]);

% precompute
semizindexcorrespondingtod2_c=repelem((1:1:N_semiz)',N_a,1);
...
% Precompute
II2=repelem(gpuArray(1:1:N_a*N_semiz),N_semiz*N_probs,1); % Index for this period (a,semiz), note the N_probs-copies
% Note: repelem(gpuArray(1:1:N_a*N_semiz),N_semiz*N_probs,1) is just a simpler way to write repelem(gpuArray(1:1:N_a*N_semiz)',1,N_semiz*N_probs)'

for jj=1:(N_j-1)
    firststep=repmat(Policy_aprime(:,:,jj),1,N_semiz)+N_a*repelem(0:1:N_semiz-1,1,N_probs); % (a',semiz')-by-(N_probs,semiz)
    ...
    % First, get Gamma
    Gammatranspose=sparse(firststep',II2,(repmat(PolicyProbs(:,:,jj),1,N_semiz).*repelem(semiztransitions,1,N_probs))',N_a*N_semiz,N_a*N_semiz); % Note: sparse() will accumulate at repeated indices [only relevant at grid end points]
