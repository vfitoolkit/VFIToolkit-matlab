...
%         StationaryDistKron.j001=jequaloneDistKron;
    ...
    if tempcounter>=2 % I simply assume you won't converge on the first try when using dynasty
        ...
        N_a=daz_gridstructure.N_a.j001;
        N_z=daz_gridstructure.N_z.j001;
        StationaryDistdist=reshape(StationaryDistKron.j001-StationaryDistKronold,[N_a*N_z,1]); StationaryDistdist(isnan(StationaryDistdist))=0;
        currdist=max(abs(StationaryDistdist)); %IS THIS reshape() & max() FASTER THAN max(max()) WOULD BE?
    end
