function StationaryDistKron=StationaryDist_FHorz_Case2_AgeDepGrids_DynastyStoreP_Iter_raw(jequaloneDistKron,AgeWeightParamNames,PolicyKron,daz_gridstructure,N_j,Phi_aprime,Case2_Type,Parameters,PhiaprimeParamNames,simoptions)
...
% Create and store Ptranspose for each age in Ptran
Ptran=struct();
if simoptions.parallel<2
...
elseif simoptions.parallel==2 % GPU
    ...
    for jj=1:N_j
        ...
        elseif Case2_Type==11 % phi(d,a,z')
            ...
            Ptemp=zeros(N_a_j,N_aprime_j*N_z_j*N_zprime_j,'gpuArray');
            Ptemp(reshape(permute(Phi_of_Policy,[2,1,3]),[1,N_a_j*N_z_j*N_zprime_j])+N_aprime_j*(gpuArray(0:1:N_a_j*N_z_j*N_zprime_j-1)))=1;
            Ptranspose=kron(pi_z_j',ones(N_aprime_j,N_a_j,'gpuArray')).*reshape(Ptemp,[N_aprime_j*N_zprime_j,N_a_j*N_z_j]);
...
while currdist>simoptions.tolerance
    ...
    for jj=1:N_j
        ...
        % Make a three digit number out of jjplus1
        jplus1str=daz_gridstructure.jstr{jjplus1};
        StationaryDistKron.(jplus1str)=Ptran.(jstr)*StationaryDistKron.(jstr);
    end
