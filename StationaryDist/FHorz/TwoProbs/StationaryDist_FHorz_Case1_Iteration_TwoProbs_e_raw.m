function StationaryDist=StationaryDist_FHorz_Case1_Iteration_TwoProbs_e_raw(jequaloneDistKron,AgeWeightParamNames,Policy_aprime,PolicyProbs,N_a,N_z,N_e,N_j,pi_z_J,pi_e_J,Parameters)
...
Policy_aprimez=gather(reshape(Policy_aprimez,[N_a*N_z*N_e,2,N_j])); % (a,z,e,2,j)
PolicyProbs=gather(reshape(PolicyProbs,[N_a*N_z*N_e,2,N_j])); % (a,z,e,2,j)
...
StationaryDist=zeros(N_a*N_z*N_e,N_j);
StationaryDist(:,1)=jequaloneDistKron;
StationaryDist_jj=sparse(gather(jequaloneDistKron)); % sparse() creates a matrix of zeros

% Precompute
II2=[1:1:N_a*N_z*N_e; 1:1:N_a*N_z*N_e]'; % Index for this period (a,z,e), note the 2 copies

for jj=1:(N_j-1)

    % First, get Gamma
    Gammatranspose=sparse(Policy_aprimez(:,:,jj),II2,PolicyProbs(:,:,jj),N_a*N_z,N_a*N_z*N_e); % Note: sparse() will accumulate at repeated indices [only relevant at grid end points]
