function StationaryDistKron=StationaryDist_FHorz_Case1_Simulation_TwoProbs_raw(jequaloneDistKron,AgeWeightParamNames,Policy_aprime,PolicyProbs,N_a,N_z,N_j,pi_z_J, Parameters, simoptions)
...
if simoptions.parallel==1
    ...
    parfor ncore_c=1:simoptions.ncores
        ...
        for ii=1:nsimspercore
            ...
            for jj=1:(N_j-1)
                whichoftwo=2-(PolicyProbs(currstate(1),currstate(2),1,jj)>rand(1)); % if the random number is less than the probability, then first, otherwise second
                currstate(1)=Policy_aprime(currstate(1),currstate(2),whichoftwo,jj);
