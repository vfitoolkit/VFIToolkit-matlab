function StationaryDistKron=StationaryDist_FHorz_Case1_Simulation_SemiExo_TwoProbs_raw(jequaloneDistKron,AgeWeightParamNames,Policy_dsemiexo,Policy_aprime,PolicyProbs,N_a,N_semiz,N_z,N_j,pi_z_J,pi_semiz_J, Parameters, simoptions)
...
if simoptions.parallel==1
    ...
    for ncore_c=1:simoptions.ncores
        ...
        for ii=1:nsimspercore
            ...
            for jj=1:(N_j-1)
                whichoftwo=2-(PolicyProbs(currstate(1),currstate(2),currstate(3),1,jj)>rand(1)); % if the random number is less than the probability, then first, otherwise second
                d_semiexo=Policy_dsemiexo(currstate(1),currstate(2),currstate(3));
                currstate(1)=Policy_aprime(currstate(1),currstate(2),currstate(3),whichoftwo,jj);
                currstate(2)=find(cumsum_pi_semiz_J(currstate(2),:,d_semiexo,jj)>rand(1,1),1,'first');
