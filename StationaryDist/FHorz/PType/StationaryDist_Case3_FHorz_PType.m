function StationaryDist=StationaryDist_Case3_FHorz_PType(jequaloneDist,AgeWeightsParamNames,PTypeDistParamNames,Policy,n_d,n_a,n_z,n_u,N_j,Names_i,d_grid,a_grid,u_grid,pi_z,pi_u,aprimeFn,Parameters,simoptions)
...
for ii=1:N_i
    ...
    if isfinite(N_j_temp)
        StationaryDist_ii=StationaryDist_FHorz_Case3(jequaloneDist_temp,AgeWeightParamNames_temp,Policy_temp,n_d_temp,n_a_temp,n_z_temp,n_u_temp,N_j_temp,d_grid_temp,a_grid_temp,u_grid_temp,pi_z_temp,pi_u_temp,aprimeFn_temp,Parameters_temp,simoptions_temp);
    else % PType actually allows for infinite horizon as well
        error('Infinite horizon not supported for Case3, email me if you need/want it')
    end
