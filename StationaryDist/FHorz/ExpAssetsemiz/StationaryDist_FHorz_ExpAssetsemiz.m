function StationaryDist=StationaryDist_FHorz_ExpAssetsemiz(jequaloneDist,AgeWeightParamNames,Policy,n_d,n_a,n_semiz,n_z,N_j,pi_semiz_J,semiz_gridvals_J,pi_z_J,Parameters,simoptions)
% experienceassetsemiz: the experience asset is driven by the semi-exogenous state.
% semiz is always present; z is an optional ordinary exogenous state.
...
n_d2=n_d(end-simoptions.l_dexperienceassetsemiz-simoptions.l_dsemiz+1:end-simoptions.l_dsemiz); % decision variables that controls experience asset
if length(n_d)>2
    n_d1=n_d(1:end-2);
    l_d1=length(n_d1);
else
...
% aprimeFnParamNames in same fashion; leading inputs are (d2, a2, semiz)
% l_d2=length(n_d2);
l_a2=length(n_a2);
...
% no ordinary z it must be treated as a singleton (prod([])/prod(0) would break it)
if N_z==0
    N_zbuild=1;
else
    N_zbuild=N_z;
...
whichisdforexpassetsemiz=length(n_d)-simoptions.l_dexperienceassetsemiz-simoptions.l_dsemiz+1:length(n_d)-simoptions.l_dsemiz;  % which is the decision variable that influences the experience asset (it is the 'second last' decision variable)
for jj=1:N_j
    aprimeFnParamsVec=CreateVectorFromParams(Parameters, aprimeFnParamNames,jj);
    [aprimeIndexes, aprimeProbs]=CreateaprimePolicyExperienceAssetsemiz(Policy(:,:,:,jj),simoptions.aprimeFn, whichisdforexpassetsemiz, n_d, n_a1,n_a2, n_semiz, N_semiz,N_zbuild,N_e, d_grid, a2_grid, semiz_gridvals_J(:,:,jj), aprimeFnParamsVec);
...
%% Do the Tan improvement (using the generic SemiExo n-probability iterators)
