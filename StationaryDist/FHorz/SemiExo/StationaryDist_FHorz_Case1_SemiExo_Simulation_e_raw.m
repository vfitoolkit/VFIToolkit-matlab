...
if simoptions.parallel==1
    ...
    StationaryDistKron=zeros(N_a,N_z,N_semiz,N_e,N_j,simoptions.ncores);
    ...
    for ncore_c=1:simoptions.ncores
        StationaryDistKron_ncore_c=zeros(N_a,N_z,N_semiz,N_e,N_j);
        for ii=1:nsimspercore
            ...
            currstate=ind2sub_homemade([N_a,N_z,N_semiz,N_e],currstate);
            ...
            for jj=1:(N_j-1)
                currstate(1)=PolicyIndexesKron(2,currstate(1),currstate(2)+N_z*(currstate(3)-1),currstate(4),jj);
                currstate(2)=find(cumsum_pi_z_J(currstate(2),:,jj)>rand(1,1),1,'first');
                dsub=ind2sub_homemade([n_d1,n_d2],PolicyIndexesKron(1,currstate(1),currstate(2)+N_z*(currstate(3)-1),currstate(4),jj));
                currstate(3)=find(cumsum_pi_semiz_J(currstate(3),:,dsub(end),jj)>rand(1,1),1,'first');
...
elseif simoptions.parallel==0 % Note: You probably never want to actually use this.
    StationaryDistKron=zeros(N_a,N_z,N_semiz,N_e,N_j);
    cumsum_pi_z_J=cumsum(pi_z_J,2);
    cumsum_pi_semiz_J=cumsum(pi_semiz_J,2);
    cumsum_pi_e_J=cumsum(pi_e_J,1);
    jequaloneDistKroncumsum=cumsum(jequaloneDistKron);
