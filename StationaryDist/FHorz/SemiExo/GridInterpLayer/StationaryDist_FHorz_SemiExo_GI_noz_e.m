function StationaryDist=StationaryDist_FHorz_SemiExo_GI_noz_e(jequaloneDist,AgeWeightParamNames,Policy,n_d,n_a,n_semiz,n_e,N_j,pi_semiz_J,pi_e_J,Parameters,simoptions)
...
N_semize=N_semiz*N_e;

%%
jequaloneDist=gather(reshape(jequaloneDist,[N_a*N_semiz*N_e,1]));

Policy=reshape(Policy,[size(Policy,1),N_a,N_semize,1,N_j]);
...
Policy_aprime=zeros(N_a,N_semize,2,N_j,'gpuArray'); % the lower grid point
PolicyProbs=zeros(N_a,N_semize,2,N_j,'gpuArray'); % The fourth dimension is lower/upper grid point
if isscalar(n_a)
...
%%
StationaryDist=StationaryDist_FHorz_Iteration_SemiExo_TwoProbs_noz_e_raw(jequaloneDist,AgeWeightParamNames,Policy_dsemiexo,Policy_aprime,PolicyProbs,N_a,N_semiz,N_e,N_j,pi_semiz_J,pi_e_J,Parameters);
