function StationaryDistKron=StationaryDist_FHorz_Iteration_SemiExo_noz_raw(jequaloneDistKron,AgeWeightParamNames,Policy_dsemiexo,Policy_aprime,N_dsemiz,N_a,N_semiz,N_j,pi_semiz_J,Parameters)
...
semizindex=repelem(gpuArray(1:1:N_semiz)',N_a,1)+N_semiz*gpuArray(0:1:N_semiz-1)+(N_semiz*N_semiz)*(Policy_dsemiexo-1)+(N_semiz*N_semiz*N_dsemiz)*shiftdim(gpuArray(0:1:N_j-1),-1); % index for semiz, plus that for semiz' (in the semiz' dim) and dsemiexo and j; their indexes in pi_semiz_J
