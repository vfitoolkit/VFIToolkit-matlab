...
Policy_aprimez=Policy_aprime+N_a*gpuArray(0:1:N_bothz-1); % Note: add semiz&z index following the semiz&z dimension
Policy_aprimez=gather(reshape(Policy_aprimez,[N_a*N_bothz,N_u*2,N_j])); % (a,semiz,z,u,2,j)
...
% Precompute
II2=repelem((1:1:N_a*N_bothz)',1,N_u*2*N_semiz); % Index for this period (a,semiz,z), note the two copies

for jj=1:(N_j-1)
    firststep=repmat(Policy_aprimez(:,:,jj),1,N_semiz)+kron(N_a*N_semiz*(0:1:N_z-1)',ones(N_a*N_semiz,1))+N_a*repelem(0:1:N_semiz-1,1,N_u*2); % (a',semiz',z')-by-(N_u*2,semiz)
