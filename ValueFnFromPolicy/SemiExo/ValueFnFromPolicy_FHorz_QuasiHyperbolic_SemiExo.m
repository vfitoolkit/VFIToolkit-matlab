...
if vfoptions.gridinterplayer==1
    error('ValueFnFromPolicy_FHorz_QuasiHyperbolic_SemiExo: gridinterplayer not yet implemented for semiz QH dual {V,Valt}')
end
...
% Decompose Policy into per-state aprime_idx and d_semiz_idx (joint index into n_dsemiz).
if N_e==0
...
% Lookup EVnext at each state's (aprime, semiz_from, z_from if present, d_semiz).
% Returns array shaped to (N_a, N_semiz [, N_z] [, N_e]).
