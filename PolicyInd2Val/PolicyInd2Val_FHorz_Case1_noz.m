function PolicyValues=PolicyInd2Val_FHorz_Case1_noz(PolicyIndexes,n_d,n_a,N_j,d_grid,a_grid)
...
if Parallel==2
    if l_d==0
        PolicyIndexes=reshape(PolicyIndexes,[l_a,N_a*N_j]);
        PolicyValues=zeros(l_a,N_a*N_j,'gpuArray');
        ...
        PolicyValues=reshape(PolicyValues,[l_a,n_a,N_j]);
    else
        PolicyIndexes=reshape(PolicyIndexes,[l_d+l_a,N_a*N_j]);
        PolicyValues=zeros(l_d+l_a,N_a*N_j,'gpuArray');
        ...
        PolicyValues=reshape(PolicyValues,[l_d+l_a,n_a,N_j]);
    end
end

if Parallel~=2
    if n_d(1)==0
        PolicyValues=zeros(l_a,N_a,N_j);
        for jj=1:N_j
            for a_c=1:N_a
                temp_a=ind2grid_homemade(PolicyIndexes(a_c,jj),n_a,a_grid);
                for ii=1:l_a
                    PolicyValues(ii,a_c,jj)=temp_a(ii);
                end
            end
        end
        PolicyValues=reshape(PolicyValues,[l_a,n_a,N_j]);
    else
        PolicyValues=zeros(l_d+l_a,N_a,N_j);
        for jj=1:N_j
            for a_c=1:N_a
                temp_d=ind2grid_homemade(n_d,PolicyIndexes(1,a_c,jj),d_grid);
                for ii=1:l_d
                    PolicyValues(ii,a_c,jj)=temp_d(ii);
                end
                temp_a=ind2grid_homemade(n_a,PolicyIndexes(2,a_c,jj),a_grid);
                for ii=1:l_a
                    PolicyValues(l_d+ii,a_c,jj)=temp_a(ii);
                end
            end
        end
        PolicyValues=reshape(PolicyValues,[l_d+l_a,n_a,N_j]);
