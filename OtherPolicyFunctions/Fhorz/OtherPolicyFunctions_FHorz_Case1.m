...
if Parallel==2
    OtherPolicies=zeros([length(ValuesFn),sizePolicyIndexes(2:end)],'gpuArray');
    ...
    OtherPolicies=zeros(length(ValuesFn),1);
    d_val=zeros(l_d,1);
            ...
            for j1=1:N_a
                ...
                for j2=1:N_z
                    ...
                    for jj=1:N_j
                        ...
                        % Includes check for cases in which no parameters are actually required
                        if isempty(ValuesFnsParamNames) %|| strcmp(SSvalueParamNames(1),'')) % check for 'SSvalueParamNames={}'
                            Values(j1,j2,jj)=ValuesFn{i}(d_val,aprime_val,a_val,z_val);
                        else
                            SSvalueParamsVec=CreateVectorFromParams(Parameters,ValuesFnsParamNames(i).Names,jj);
                            Values(j1,j2,jj)=ValuesFn{i}(d_val,aprime_val,a_val,z_val,SSvalueParamsVec);
                        end
                    end
                end
            end
