function [V,Policy]=ValueFnIter_Case1_FHorz_DC1(n_d, n_a, n_z, N_j, d_grid, a_grid, z_gridvals_J, pi_z_J, ReturnFn, Parameters, DiscountFactorParamNames, ReturnFnParamNames, vfoptions)
...
if vfoptions.parallel==2
    if length(n_a)==1
        if N_d==0
            if isfield(vfoptions,'n_e')
                if N_z==0
                    error('I am too lazy right now to implement divide-and-conquer for model with e but without z')
    ...
    error('vfoptions.divideandconquer==1 is only available on GPU (you must have vfoptions.parallel=2, you do NOT have vfoptions.parallel=2)')
end
