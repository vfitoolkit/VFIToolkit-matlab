...
if ~isfield(vfoptions,'V_Jplus1')
    % n-Monotonicity
    ReturnMatrix_ii=CreateReturnFnMatrix_Case1_Disc_DC1_nod_Par2e(ReturnFn, n_z, n_e, a_grid, a_grid(level1ii), z_gridvals_J(:,:,N_j), e_gridvals_J(:,:,N_j), ReturnFnParamsVec);
    ...
    [maxaprimeii,~]=max(max(maxindex,[],3),[],4); % Get the max aprime index, conditional on a, across all possible z & e
    [minaprimeii,~]=min(min(maxindex,[],3),[],4); % Get the min aprime index, conditional on a, across all possible z & e
    ...
    % We just want aprime to use in n-Monotonicity. But we want it conditional on a, but allow for full range that different d give
    [maxaprimeii,~]=max(max(maxindex1,[],3),[],4); % Get the max aprime index, conditional on a, across all possible z & e
    [minaprimeii,~]=min(min(maxindex1,[],3),[],4); % Get the min aprime index, conditional on a, across all possible z & e
    maxaprimeii=maxaprimeii(:); % turn into column vector
    minaprimeii=minaprimeii(:); % turn into column vector
    % COMMENT TO SELF: I tried to instead "Get max aprime index, conditional on (a,z), across all possible d". But then I have to add a for-loop over z (when doing ReturnMatrix_ii) and this was slower.
...
for reverse_j=1:N_j-1
    ...
    % n-Monotonicity
    ReturnMatrix_ii=CreateReturnFnMatrix_Case1_Disc_DC1_nod_Par2e(ReturnFn, n_z, n_e, a_grid, a_grid(level1ii), z_gridvals_J(:,:,jj), e_gridvals_J(:,:,jj), ReturnFnParamsVec);
