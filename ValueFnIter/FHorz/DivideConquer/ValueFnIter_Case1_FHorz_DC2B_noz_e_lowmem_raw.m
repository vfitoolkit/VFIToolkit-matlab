function [V,Policy2]=ValueFnIter_Case1_FHorz_DC2B_noz_e_lowmem_raw(n_d,n_a,n_e,N_j, d_grid, a_grid, e_gridvals_J, pi_e_J, ReturnFn, Parameters, DiscountFactorParamNames, ReturnFnParamNames, vfoptions)
% lowmem=loop over e
...
if ~isfield(vfoptions,'V_Jplus1')
    for e_c=1:N_e
        ...
        % n-Monotonicity
        ReturnMatrix_ii_e=CreateReturnFnMatrix_Case1_Disc_DC2B_Par2(ReturnFn, n_d, special_n_e, d_gridvals, a1_grid, a2_grid, a1_grid(level1ii), a2_grid, e_vals, ReturnFnParamsVec, 1);
        ...
        for ii=1:(vfoptions.level1n-1)
            ...
            if maxgap(ii)>0
                ...
                ReturnMatrix_ii_e=CreateReturnFnMatrix_Case1_Disc_DC2B_Par2(ReturnFn, n_d, special_n_e, d_gridvals, a1_grid(a1primeindexes), a2_grid, a1_grid(level1ii(ii)+1:level1ii(ii+1)-1), a2_grid, e_vals, ReturnFnParamsVec,2);
                ...
                allind=dind+N_d*a2primeind+N_d*N_a2*a2ind; % loweredge is n_d-by-1-by-n_a2-by-1-by-n_a2-by-n_e
                Policy(curraindex,e_c,N_j)=shiftdim(maxindexfix+N_d*(loweredge(allind)-1),1);
            else
                loweredge=maxindex1(:,1,:,ii,:);
                % Just use aprime(ii) for everything
                ReturnMatrix_ii_e=CreateReturnFnMatrix_Case1_Disc_DC2B_Par2(ReturnFn, n_d, special_n_e, d_gridvals, a1_grid(loweredge), a2_grid, a1_grid(level1ii(ii)+1:level1ii(ii+1)-1), a2_grid, e_vals, ReturnFnParamsVec,2);
                [Vtempii,maxindex]=max(ReturnMatrix_ii_e,[],1);
                V(curraindex,e_c,N_j)=shiftdim(Vtempii,1);
                % maxindex needs to be reworked:
                %  the a2prime is only an 'after maxgap(ii)+1, but needs to be after N_a1'
    ...
    DiscountedentireEV=DiscountFactorParamsVec*reshape(repmat(shiftdim(V_Jplus1,-1),N_d,1,1,1),[N_d,N_a1,N_a2,1,1]); % [d,aprime,1,1]
    ...
    for e_c=1:N_e
        ...
        entireRHS_ii=ReturnMatrix_ii_e+DiscountedentireEV;
        ...
        for ii=1:(vfoptions.level1n-1)
            ...
            if maxgap(ii)>0
                ...
                entireRHS_ii=ReturnMatrix_ii_e+DiscountedentireEV(reshape(daprime,[N_d*(maxgap(ii)+1)*N_a2,level1iidiff(ii)*N_a2]));
                ...
                %  the a1prime is relative to loweredge(allind), need to 'add' the loweredge
                a2ind=repelem((0:1:N_a2-1),1,level1iidiff(ii)); % already includes -1
                allind=dind+N_d*a2primeind+N_d*N_a2*a2ind+N_d*N_a2*N_a2*eind; % loweredge is n_d-by-1-by-n_a2-by-1-by-n_a2-by-n_e
                Policy(curraindex,:,N_j)=shiftdim(maxindexfix+N_d*(loweredge(allind)-1),1);
            else
                loweredge=maxindex1(:,1,:,ii,:);
                % Just use aprime(ii) for everything
                ReturnMatrix_ii_e=CreateReturnFnMatrix_Case1_Disc_DC2B_Par2(ReturnFn, n_d, n_e, d_gridvals, a1_grid(loweredge), a2_grid, a1_grid(level1ii(ii)+1:level1ii(ii+1)-1), a2_grid, e_vals, ReturnFnParamsVec,2);
                ...
                entireRHS_ii=ReturnMatrix_ii_e+DiscountedentireEV(reshape(daprime,[N_d*1*N_a2,level1iidiff(ii)*N_a2]));
                [Vtempii,maxindex]=max(entireRHS_ii,[],1);
                V(curraindex,e_c,N_j)=shiftdim(Vtempii,1);
                % maxindex needs to be reworked:
                %  the a2prime is only an 'after maxgap(ii)+1, but needs to be after N_a1'
