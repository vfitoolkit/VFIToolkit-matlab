...
for ii = 1:(num_anchors - 1)
    ...
    if maxgap(ii) > 0
        low_pass = min(loweredge_a1, N_choice_a1_dc - maxgap(ii));
        [V_seg, Pol_apr_seg, Pol_d1_seg, L2idx_seg, L2flag_seg, Pol_a1_per_a2_seg] = EvalBlockFn(seg_state_chunk, low_pass, maxgap(ii));
    else
