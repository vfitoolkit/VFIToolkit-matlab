...
if ~isfield(vfoptions,'V_Jplus1')
    for z_c=1:N_z
        ...
        % Store
        curraindex=repmat(level1ii',N_a2,1)+N_a1*repelem(a2Bind',vfoptions.level1n,1);
        ...
        for ii=1:(vfoptions.level1n-1)
            curraindex=repmat((level1ii(ii)+1:1:level1ii(ii+1)-1)',N_a2,1)+N_a1*repelem(a2Bind',level1iidiff(ii),1);
            if maxgap(ii)>0
                ...
                a2ind=repelem(a2Bind,1,level1iidiff(ii)); % already includes -1
    ...
    for z_c=1:N_z
        ...
        for ii=1:(vfoptions.level1n-1)
            ...
            if maxgap(ii)>0
                ...
                % aprime possibilities are (maxgap(ii)+1)-n_a2-by-1-by-n_a2-by-n_z
                ReturnMatrix_ii_z=CreateReturnFnMatrix_Case1_Disc_DC2B_nod_Par2(ReturnFn, special_n_z, a1_grid(aprimeindexes), a2_grid, a1_grid(level1ii(ii)+1:level1ii(ii+1)-1), a2_grid, z_vals, ReturnFnParamsVec,2);
                aprime=repelem(aprimeindexes,1,1,level1iidiff(ii),1,1)+N_a1*a2Bind; % the current aprimeii(ii):aprimeii(ii+1)
                ...
                % Just use aprime(ii) for everything
                ReturnMatrix_ii_z=CreateReturnFnMatrix_Case1_Disc_DC2B_nod_Par2(ReturnFn, special_n_z, a1_grid(loweredge), a2_grid, a1_grid(level1ii(ii)+1:level1ii(ii+1)-1), a2_grid, z_vals, ReturnFnParamsVec,2);
                aprime=repelem(loweredge,1,1,level1iidiff(ii),1,1)+N_a1*a2Bind; % the current aprimeii(ii):aprimeii(ii+1)
