...
if ~isfield(vfoptions,'V_Jplus1')
    ...
    for ii=1:(vfoptions.level1n-1)
        ...
        if maxgap(ii)>0
            ...
            entireRHS_ii=ReturnMatrix_ii+DiscountedentireEV(reshape(daprime,[N_d*(maxgap(ii)+1)*N_a2,level1iidiff(ii)*N_a2,N_e]));
            ...
            entireRHS_ii=ReturnMatrix_ii+DiscountedentireEV(reshape(daprime,[N_d*1*N_a2,level1iidiff(ii)*N_a2,N_e]));
            [Vtempii,maxindex]=max(entireRHS_ii,[],1);
            V(curraindex,:,N_j)=shiftdim(Vtempii,1);
            % maxindex needs to be reworked:
            %  the a2prime is only an 'after maxgap(ii)+1, but needs to be after N_a1'
