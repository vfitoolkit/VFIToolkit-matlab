...
if vfoptions.lowmemory==0
    midpoints_jj=zeros(N_a,N_z,N_e,'gpuArray');
elseif vfoptions.lowmemory==1 % loops over e
    midpoints_jj=zeros(N_a,N_z,'gpuArray');
...
if ~isfield(vfoptions,'V_Jplus1')
    ...
    elseif vfoptions.lowmemory==1
        for e_c=1:N_e
            ...
            % Just keep the 'midpoint' vesion of maxindex1 [as GI]
            midpoints_jj(level1ii,:,e_c)=maxindex1;
            ...
            for ii=1:(vfoptions.level1n-1)
                ...
                if maxgap(ii)>0
                    ...
                    [~,maxindex]=max(ReturnMatrix_ii,[],1);
                    midpoints_jj(curraindex,:,e_c)=shiftdim(maxindex+loweredge-1,1);
                else
                    loweredge=maxindex1(1,ii,:);
                    midpoints_jj(curraindex,:,e_c)=repelem(loweredge,length(curraindex),1);
                end
            end
    ...
    elseif vfoptions.lowmemory==2
        for z_c=1:N_z
            ...
            for e_c=1:N_e
                ...
                % Just keep the 'midpoint' vesion of maxindex1 [as GI]
                midpoints_jj(level1ii,z_c,e_c)=maxindex1;
                ...
                for ii=1:(vfoptions.level1n-1)
                    ...
                    if maxgap(ii)>0
                        ...
                        [~,maxindex]=max(ReturnMatrix_ii,[],1);
                        midpoints_jj(curraindex,z_c,e_c)=shiftdim(maxindex+loweredge-1,1);
                    else
                        loweredge=maxindex1(1,ii);
                        midpoints_jj(curraindex,z_c,e_c)=repelem(loweredge,length(curraindex),1);
                    end
                end
    ...
    elseif vfoptions.lowmemory==2
        for z_c=1:N_z
            ...
            for e_c=1:N_e
                ...
                entireRHS_ii=ReturnMatrix_ii+DiscountFactorParamsVec*reshape(EVinterp(aprimez(:)),[n2long,N_a]);
                [Vtempii,maxindexL2]=max(entireRHS_ii,[],1);
                V(:,z_c,e_c,N_j)=shiftdim(Vtempii,1);
