...
if ~isfield(vfoptions,'V_Jplus1')
    % n-Monotonicity
    ReturnMatrix_ii=CreateReturnFnMatrix_Case1_Disc_DC1_noz_Par2(ReturnFn, n_d, d_grid, a_grid, a_grid(level1ii), ReturnFnParamsVec,1);
    ...
    [maxaprimeii,~]=max(maxindex1,[],1); % Get the max aprime index, conditional on a, across all possible optimal d
    [minaprimeii,~]=min(maxindex1,[],1); % Get the min aprime index, conditional on a, across all possible optimal d
    ...
    for ii=1:(vfoptions.level1n-1)
        ReturnMatrix_ii=CreateReturnFnMatrix_Case1_Disc_DC1_noz_Par2(ReturnFn, n_d, d_grid, a_grid(minaprimeii(ii):maxaprimeii(ii+1)), a_grid(level1ii(ii)+1:level1ii(ii+1)-1), ReturnFnParamsVec,2);
        % (d,aprime,a)
    ...
    for ii=1:(vfoptions.level1n-1)
        if maxaprimeii(ii+1)>minaprimeii(ii)
            ReturnMatrix_ii=CreateReturnFnMatrix_Case1_Disc_DC1_noz_Par2(ReturnFn, n_d, d_grid, a_grid(minaprimeii(ii):maxaprimeii(ii+1)), a_grid(level1ii(ii)+1:level1ii(ii+1)-1), ReturnFnParamsVec,2);
            daprime=(repmat(1:1:N_d,1,maxaprimeii(ii+1)-minaprimeii(ii)+1)+N_d*repelem(minaprimeii(ii)-1:1:maxaprimeii(ii+1)-1,1,N_d))'; % all the d, with the current aprimeii(ii):aprimeii(ii+1)
            ...
            ReturnMatrix_ii=CreateReturnFnMatrix_Case1_Disc_DC1_noz_Par2(ReturnFn, n_d, d_grid, a_grid(minaprimeii(ii)), a_grid(level1ii(ii)+1:level1ii(ii+1)-1), ReturnFnParamsVec,2);
            daprime=((1:1:N_d)+N_d*(minaprimeii(ii)-1))'; % all the d, with the current aprimeii(ii):aprimeii(ii+1)
            entireRHS_ii=ReturnMatrix_ii+DiscountFactorParamsVec*entireEV(daprime);
            [Vtempii,maxindex]=max(entireRHS_ii,[],1);
            V(level1ii(ii)+1:level1ii(ii+1)-1,jj)=shiftdim(Vtempii,1);
            Policytemp(level1ii(ii)+1:level1ii(ii+1)-1)=shiftdim(maxindex,1)+N_d*(minaprimeii(ii)-1);
...
for reverse_j=1:N_j-1
    ...
    [maxaprimeii,~]=max(maxindex1,[],1); % Get the max aprime index, conditional on a, across all possible optimal d
    [minaprimeii,~]=min(maxindex1,[],1); % Get the min aprime index, conditional on a, across all possible optimal d
    maxaprimeii=maxaprimeii(:); % turn into column vector (otherwise has just third dimension)
    minaprimeii=minaprimeii(:); % turn into column vector (otherwise has just third dimension)
