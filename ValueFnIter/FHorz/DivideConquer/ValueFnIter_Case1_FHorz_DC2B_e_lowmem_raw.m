function [V,Policy2]=ValueFnIter_Case1_FHorz_DC2B_e_lowmem_raw(n_d,n_a,n_z,n_e,N_j, d_grid, a_grid, z_gridvals_J,e_gridvals_J, pi_z_J, pi_e_J, ReturnFn, Parameters, DiscountFactorParamNames, ReturnFnParamNames, vfoptions)
% lowmem=loop over e
...
% lowmem, so don't need eind anymore
...
if ~isfield(vfoptions,'V_Jplus1')
    for e_c=1:N_e
        ...
        for ii=1:(vfoptions.level1n-1)
            ...
            if maxgap(ii)>0
                ...
                allind=dind+N_d*a2primeind+N_d*N_a2*a2ind+N_d*N_a2*N_a2*zind; % loweredge is n_d-by-1-by-n_a2-by-1-by-n_a2-by-n_z-by-n_e
    ...
    for e_c=1:N_e
        ...
        entireRHS_ii=ReturnMatrix_ii_e+DiscountedentireEV; % autofill e
        ...
        for ii=1:(vfoptions.level1n-1)
            ...
            if maxgap(ii)>0
                ...
                entireRHS_ii=ReturnMatrix_ii_e+DiscountedentireEV(reshape(daprimez,[N_d*(maxgap(ii)+1)*N_a2,level1iidiff(ii)*N_a2,N_z]));
                ...
                entireRHS_ii=ReturnMatrix_ii_e+DiscountedentireEV(reshape(daprimez,[N_d*1*N_a2,level1iidiff(ii)*N_a2,N_z]));
                [Vtempii,maxindex]=max(entireRHS_ii,[],1);
                V(curraindex,:,e_c,N_j)=shiftdim(Vtempii,1);
                % maxindex needs to be reworked:
                %  the a2prime is only an 'after maxgap(ii)+1, but needs to be after N_a1'
