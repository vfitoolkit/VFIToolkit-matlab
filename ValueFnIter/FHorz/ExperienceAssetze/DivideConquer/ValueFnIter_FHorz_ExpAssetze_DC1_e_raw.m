...
if ~isfield(vfoptions,'V_Jplus1')
    ...
    % Note: aprimeIndex is [N_d2,N_a2,N_z,N_e], whereas aprimeProbs is [N_d2,N_a2,N_z,N_e]   (z and e are both the current values)
        ...
        for e_c=1:N_e
            ...
            for ii=1:(vfoptions.level1n-1)
                ...
                if maxgap(ii)>0
                    ...
                    entireRHS_ii=reshape(reshape(ReturnMatrix_ii,[N_d*(maxgap(ii)+1),level1iidiff(ii),N_a2,N_z])+reshape(DiscountedEV_e(d2aprimez),[N_d*(maxgap(ii)+1),1,N_a2,N_z]),[N_d*(maxgap(ii)+1),level1iidiff(ii)*N_a2,N_z]);
                    [Vtempii,maxindex]=max(entireRHS_ii,[],1);
                    V(curraindex,:,e_c,N_j)=shiftdim(Vtempii,1);
                    % maxindex does not need reworking, as with expasset there is no a2prime
                    %  the a1prime is relative to loweredge(allind), need to 'add' the loweredge
                    ...
                    entireRHS_ii=reshape(reshape(ReturnMatrix_ii,[N_d,level1iidiff(ii),N_a2,N_z])+reshape(DiscountedEV_e(d2aprimez),[N_d,1,N_a2,N_z]),[N_d,level1iidiff(ii)*N_a2,N_z]);
                    [Vtempii,maxindex]=max(entireRHS_ii,[],1);
                    V(curraindex,:,e_c,N_j)=shiftdim(Vtempii,1);
                    % maxindex does not need reworking, as with expasset there is no a2prime
                    %  the a1prime is relative to loweredge(allind), need to 'add' the loweredge
            ...
            for e_c=1:N_e
                ...
                for ii=1:(vfoptions.level1n-1)
                    ...
                    if maxgap(ii)>0
                        ...
                        entireRHS_ii_ze=reshape(reshape(ReturnMatrix_ii,[N_d*(maxgap(ii)+1),level1iidiff(ii),N_a2])+reshape(DiscountedEV_ze(d2aprime),[N_d*(maxgap(ii)+1),1,N_a2]),[N_d*(maxgap(ii)+1),level1iidiff(ii)*N_a2]);
        ...
        for e_c=1:N_e
            ...
            for ii=1:(vfoptions.level1n-1)
                ...
                if maxgap(ii)>0
                    ...
                    d2aprimez=d2ind+N_d2*(a1primeindexes-1)+N_d2*N_a1*a2ind+N_d2*N_a*zind; % [N_d,maxgap+1,1,N_a2,N_z]; linear index into DiscountedEV_e [N_d2,N_a1,1,N_a2,N_z]
                    entireRHS_ii_e=reshape(reshape(ReturnMatrix_ii_e,[N_d*(maxgap(ii)+1),level1iidiff(ii),N_a2,N_z])+reshape(DiscountedEV_e(d2aprimez),[N_d*(maxgap(ii)+1),1,N_a2,N_z]),[N_d*(maxgap(ii)+1),level1iidiff(ii)*N_a2,N_z]);
                    ...
                    d2aprimez=d2ind+N_d2*(loweredge-1)+N_d2*N_a1*a2ind+N_d2*N_a*zind; % [N_d,1,1,N_a2,N_z]; linear index into DiscountedEV_e [N_d2,N_a1,1,N_a2,N_z]
                    entireRHS_ii_e=reshape(reshape(ReturnMatrix_ii_e,[N_d,level1iidiff(ii),N_a2,N_z])+reshape(DiscountedEV_e(d2aprimez),[N_d,1,N_a2,N_z]),[N_d,level1iidiff(ii)*N_a2,N_z]);
                    [Vtempii,maxindex]=max(entireRHS_ii_e,[],1);
                    V(curraindex,:,e_c,jj)=shiftdim(Vtempii,1);
                    % maxindex does not need reworking, as with expasset there is no a2prime
                    %  the a1prime is relative to loweredge(allind), need to 'add' the loweredge
            ...
            for e_c=1:N_e
                ...
                for ii=1:(vfoptions.level1n-1)
                    ...
                    if maxgap(ii)>0
                        ...
                        d2aprime=d2ind+N_d2*(a1primeindexes-1)+N_d2*N_a1*a2ind; % [N_d,maxgap+1,1,N_a2]; linear index into DiscountedEV_ze [N_d2,N_a1,1,N_a2]
                        entireRHS_ii_ze=reshape(reshape(ReturnMatrix_ii_ze,[N_d*(maxgap(ii)+1),level1iidiff(ii),N_a2])+reshape(DiscountedEV_ze(d2aprime),[N_d*(maxgap(ii)+1),1,N_a2]),[N_d*(maxgap(ii)+1),level1iidiff(ii)*N_a2]);
                        [Vtempii,maxindex]=max(entireRHS_ii_ze,[],1);
                        V(curraindex,z_c,e_c,jj)=shiftdim(Vtempii,1);
                        % maxindex does not need reworking, as with expasset there is no a2prime
                        %  the a1prime is relative to loweredge(allind), need to 'add' the loweredge
