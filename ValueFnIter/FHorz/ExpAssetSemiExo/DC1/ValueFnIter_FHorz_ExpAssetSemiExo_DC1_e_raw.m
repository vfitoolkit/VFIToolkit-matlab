...
if ~isfield(vfoptions,'V_Jplus1')
    ...
    if vfoptions.lowmemory==0
        for d3_c=1:N_d3
            ...
            DiscountedentireEV=DiscountFactorParamsVec*repelem(reshape(entireEV,[N_d2,N_a1,1,N_a2,N_bothz]),N_d1,1); % (d2,a1prime,1,a2,zprime)
            ...
            for ii=1:(vfoptions.level1n-1)
                ...
                if maxgap(ii)>0
                    ...
                    entireRHS_ii=ReturnMatrix_ii_d3+DiscountedentireEV(reshape(daprime,[N_d1*N_d2*(maxgap(ii)+1),level1iidiff(ii)*N_a2,N_bothz,N_e]));
                    ...
                    a2ind=repelem((0:1:N_a2-1),1,level1iidiff(ii)); % already includes -1
                    allind=dind+N_d1*N_d2*a2ind+N_d1*N_d2*N_a2*bothzind+N_d1*N_d2*N_a2*N_bothz*eind; % loweredge is n_d-by-1-by-1-by-n_a2-by-n_a2-by-n_z-by-n_e
                    ...
                    entireRHS_ii=ReturnMatrix_ii_d3+DiscountedentireEV(reshape(daprime,[N_d1*N_d2*1,level1iidiff(ii)*N_a2,N_bothz,N_e]));
                    [Vtempii,maxindex]=max(entireRHS_ii,[],1);
                    V_ford3_jj(curraindex,:,:,d3_c)=shiftdim(Vtempii,1);
                    % maxindex does not need reworking, as with expasset there is no a2prime
                    %  the a1prime is relative to loweredge(allind), need to 'add' the loweredge
                    dind=(rem(maxindex-1,N_d1*N_d2)+1);
                    a2ind=repelem((0:1:N_a2-1),1,level1iidiff(ii)); % already includes -1
                    allind=dind+N_d1*N_d2*a2ind+N_d1*N_d2*N_a2*bothzind+N_d1*N_d2*N_a2*N_bothz*eind; % loweredge is n_d-by-1-by-1-by-n_a2-by-n_z-by-n_e
                    Policy_ford3_jj(curraindex,:,:,d3_c)=shiftdim(maxindex+N_d1*N_d2*(loweredge(allind)-1),1);
                end
            end
        end
