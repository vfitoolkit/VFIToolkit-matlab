...
if ~isfield(vfoptions,'V_Jplus1')
    if vfoptions.lowmemory==0
        % treat e as z in the helper since both have analogous role here (special_n_z slot for e dim)
    ...
    [V(:,:,N_j),Policy(:,:,N_j)]=internal_per_j_noz_e(EVpre,a2primeIndex,a2primeProbs,ReturnFn,DiscountFactorParamsVec,ReturnFnParamsVec,...
        n_d1,n_d3,n_a1,n_a2,n_e,N_d1,N_d2,N_d3,N_d13,N_d23,N_a1,N_a2,N_a,N_e,N_u,...
        d13_gridvals,a1_gridvals,a2_gridvals,e_gridvals_J(:,:,N_j),pi_u_col,...
        level1ii,level1iidiff,a2Bind,eBind,d3ind,vfoptions);
end
...
%% Per-period inner (noz, e). EVnext already integrated over e.
function [V_jj,Policy_jj]=internal_per_j_noz_e(EVnext,a2primeIndex,a2primeProbs,ReturnFn,DiscountFactorParamsVec,ReturnFnParamsVec,...
    n_d1,n_d3,n_a1,n_a2,n_e,N_d1,N_d2,N_d3,N_d13,N_d23,N_a1,N_a2,N_a,N_e,N_u,...
    d13_gridvals,a1_gridvals,a2_gridvals,e_gridvals,pi_u_col,...
    level1ii,level1iidiff,a2Bind,eBind,d3ind,vfoptions)

V_jj=zeros(N_a,N_e,'gpuArray');
Policy_jj=zeros(N_a,N_e,'gpuArray');
...
if vfoptions.lowmemory==0
    ReturnMatrix_ii=CreateReturnFnMatrix_ExpAsset_Disc(ReturnFn, n_d1,n_d3,n_a1,vfoptions.level1n,n_a2,n_e, d13_gridvals, a1_gridvals, a1_gridvals(level1ii), a2_gridvals, e_gridvals, ReturnFnParamsVec,1,0);
    ...
    Policy_jj(curraindex,:)=encodePolicy_with_d2lookup_noz_e(shiftdim(maxindex2,1),N_d1,N_d2,N_d3,N_d13,d2index_resh,N_a1,N_e);
    ...
    for ii=1:(vfoptions.level1n-1)
        ...
        if maxgap(ii)>0
            loweredge=min(maxindex1(:,1,ii,:,:),N_a1-maxgap(ii));
            a1primeindexes=loweredge+(0:1:maxgap(ii));
            ReturnMatrix_ii=CreateReturnFnMatrix_ExpAsset_Disc(ReturnFn, n_d1,n_d3,maxgap(ii)+1,level1iidiff(ii),n_a2,n_e, d13_gridvals, a1_gridvals(a1primeindexes), a1_gridvals(level1ii(ii)+1:level1ii(ii+1)-1), a2_gridvals, e_gridvals, ReturnFnParamsVec,3,0);
            ...
            allind=dind+N_d13*repelem(a2Bind,1,level1iidiff(ii))+N_d13*N_a2*eBind;
            pol_d13_a1=maxindex+N_d13*(loweredge(allind)-1);
            Policy_jj(curraindex,:)=encodePolicy_with_d2lookup_noz_e(shiftdim(pol_d13_a1,1),N_d1,N_d2,N_d3,N_d13,d2index_resh,N_a1,N_e);
        else
            loweredge=maxindex1(:,1,ii,:,:);
            ReturnMatrix_ii=CreateReturnFnMatrix_ExpAsset_Disc(ReturnFn, n_d1,n_d3,1,level1iidiff(ii),n_a2,n_e, d13_gridvals, a1_gridvals(loweredge), a1_gridvals(level1ii(ii)+1:level1ii(ii+1)-1), a2_gridvals, e_gridvals, ReturnFnParamsVec,3,0);
...
elseif vfoptions.lowmemory>=1
    ...
    for e_c=1:N_e
        ...
        Policy_jj(curraindex,e_c)=encodePolicy_with_d2lookup_noz_e_lm(shiftdim(maxindex2,1),N_d1,N_d2,N_d3,N_d13,d2index_resh,N_a1);
        ...
        for ii=1:(vfoptions.level1n-1)
            ...
            if maxgap(ii)>0
                ...
                allind=dind+N_d13*repelem(a2Bind,1,level1iidiff(ii));
                pol_d13_a1=maxindex+N_d13*(loweredge(allind)-1);
                Policy_jj(curraindex,e_c)=encodePolicy_with_d2lookup_noz_e_lm(shiftdim(pol_d13_a1,1),N_d1,N_d2,N_d3,N_d13,d2index_resh,N_a1);
            else
                loweredge=maxindex1(:,1,ii,:);
...
function pol=encodePolicy_with_d2lookup_noz_e(pol_d13_a1,N_d1,N_d2,N_d3,N_d13,d2index_resh,N_a1,N_e)
% pol_d13_a1: [npts,N_e] — but d2index_resh is [N_d3,N_a1] (no e dep)
d1part=rem(pol_d13_a1-1,N_d1)+1;
d3part=rem(ceil(pol_d13_a1/N_d1)-1,N_d3)+1;
a1primepart=ceil(pol_d13_a1/N_d13);
...
function pol=encodePolicy_with_d2lookup_noz_e_lm(pol_d13_a1,N_d1,N_d2,N_d3,N_d13,d2index_resh,N_a1)
% pol_d13_a1: [npts,1]
d1part=rem(pol_d13_a1-1,N_d1)+1;
d3part=rem(ceil(pol_d13_a1/N_d1)-1,N_d3)+1;
a1primepart=ceil(pol_d13_a1/N_d13);
