...
pi_u_col=pi_u(:);
pi_e_J_shift=shiftdim(pi_e_J,-2); % third dim
...
if ~isfield(vfoptions,'V_Jplus1')
    ...
    elseif vfoptions.lowmemory==1
        for e_c=1:N_e
            ...
            Policy(curraindex,:,e_c,N_j)=encodePolicy_no_d2(shiftdim(maxindex2,1),N_d1,N_d2,N_d3,N_d13);
            ...
            for ii=1:(vfoptions.level1n-1)
                ...
                if maxgap(ii)>0
                    ...
                    pol_d13_a1=maxindex+N_d13*(loweredge(allind)-1);
                    Policy(curraindex,:,e_c,N_j)=encodePolicy_no_d2(shiftdim(pol_d13_a1,1),N_d1,N_d2,N_d3,N_d13);
                else
                    loweredge=maxindex1(:,1,ii,:,:);
    ...
    [V(:,:,:,N_j),Policy(:,:,:,N_j)]=internal_per_j_e(EVpre,a2primeIndex,a2primeProbs,ReturnFn,DiscountFactorParamsVec,ReturnFnParamsVec,...
        n_d1,n_d3,n_a1,n_a2,n_z,n_e,N_d1,N_d2,N_d3,N_d13,N_d23,N_a1,N_a2,N_a,N_z,N_e,N_u,...
        d13_gridvals,a1_gridvals,a2_gridvals,z_gridvals_J(:,:,N_j),e_gridvals_J(:,:,N_j),pi_z_J(:,:,N_j),pi_u_col,...
...
for reverse_j=1:N_j-1
    ...
    [V(:,:,:,jj),Policy(:,:,:,jj)]=internal_per_j_e(EVnext,a2primeIndex,a2primeProbs,ReturnFn,DiscountFactorParamsVec,ReturnFnParamsVec,...
        n_d1,n_d3,n_a1,n_a2,n_z,n_e,N_d1,N_d2,N_d3,N_d13,N_d23,N_a1,N_a2,N_a,N_z,N_e,N_u,...
        d13_gridvals,a1_gridvals,a2_gridvals,z_gridvals_J(:,:,jj),e_gridvals_J(:,:,jj),pi_z_J(:,:,jj),pi_u_col,...
        level1ii,level1iidiff,a2Bind,zBind,eBind,d3ind,vfoptions);
end
...
function [V_jj,Policy_jj]=internal_per_j_e(EVnext,a2primeIndex,a2primeProbs,ReturnFn,DiscountFactorParamsVec,ReturnFnParamsVec,...
    n_d1,n_d3,n_a1,n_a2,n_z,n_e,N_d1,N_d2,N_d3,N_d13,N_d23,N_a1,N_a2,N_a,N_z,N_e,N_u,...
    d13_gridvals,a1_gridvals,a2_gridvals,z_gridvals,e_gridvals,pi_z,pi_u_col,...
    level1ii,level1iidiff,a2Bind,zBind,eBind,d3ind,vfoptions)
...
% EVnext is [N_a,N_z] (e already integrated). Now sum over zprime.
...
if vfoptions.lowmemory==0
    ReturnMatrix_ii=CreateReturnFnMatrix_ExpAsset_Disc_e(ReturnFn, n_d1,n_d3,n_a1,vfoptions.level1n,n_a2,n_z,n_e, d13_gridvals, a1_gridvals, a1_gridvals(level1ii), a2_gridvals, z_gridvals, e_gridvals, ReturnFnParamsVec,1,0);
    ...
    Policy_jj(curraindex,:,:)=encodePolicy_with_d2lookup_e(shiftdim(maxindex2,1),N_d1,N_d2,N_d3,N_d13,d2index_resh,N_a1,N_z,N_e);
    ...
    for ii=1:(vfoptions.level1n-1)
        ...
        if maxgap(ii)>0
            loweredge=min(maxindex1(:,1,ii,:,:,:),N_a1-maxgap(ii));
            a1primeindexes=loweredge+(0:1:maxgap(ii));
            ReturnMatrix_ii=CreateReturnFnMatrix_ExpAsset_Disc_e(ReturnFn, n_d1,n_d3,maxgap(ii)+1,level1iidiff(ii),n_a2,n_z,n_e, d13_gridvals, a1_gridvals(a1primeindexes), a1_gridvals(level1ii(ii)+1:level1ii(ii+1)-1), a2_gridvals, z_gridvals, e_gridvals, ReturnFnParamsVec,3,0);
            ...
            allind=dind+N_d13*repelem(a2Bind,1,level1iidiff(ii))+N_d13*N_a2*zBind+N_d13*N_a2*N_z*eBind;
            pol_d13_a1=maxindex+N_d13*(loweredge(allind)-1);
            Policy_jj(curraindex,:,:)=encodePolicy_with_d2lookup_e(shiftdim(pol_d13_a1,1),N_d1,N_d2,N_d3,N_d13,d2index_resh,N_a1,N_z,N_e);
        else
            loweredge=maxindex1(:,1,ii,:,:,:);
            ReturnMatrix_ii=CreateReturnFnMatrix_ExpAsset_Disc_e(ReturnFn, n_d1,n_d3,1,level1iidiff(ii),n_a2,n_z,n_e, d13_gridvals, a1_gridvals(loweredge), a1_gridvals(level1ii(ii)+1:level1ii(ii+1)-1), a2_gridvals, z_gridvals, e_gridvals, ReturnFnParamsVec,3,0);
...
elseif vfoptions.lowmemory>=1
    ...
    for e_c=1:N_e
        ...
        ReturnMatrix_ii_e=CreateReturnFnMatrix_ExpAsset_Disc_e(ReturnFn, n_d1,n_d3,n_a1,vfoptions.level1n,n_a2,n_z,special_n_e, d13_gridvals, a1_gridvals, a1_gridvals(level1ii), a2_gridvals, z_gridvals, e_val, ReturnFnParamsVec,1,0);
        ...
        Policy_jj(curraindex,:,e_c)=encodePolicy_with_d2lookup(shiftdim(maxindex2,1),N_d1,N_d2,N_d3,N_d13,d2index_resh,N_a1,N_z);
        ...
        for ii=1:(vfoptions.level1n-1)
            ...
            if maxgap(ii)>0
                loweredge=min(maxindex1(:,1,ii,:,:),N_a1-maxgap(ii));
                a1primeindexes=loweredge+(0:1:maxgap(ii));
                ReturnMatrix_ii=CreateReturnFnMatrix_ExpAsset_Disc_e(ReturnFn, n_d1,n_d3,maxgap(ii)+1,level1iidiff(ii),n_a2,n_z,special_n_e, d13_gridvals, a1_gridvals(a1primeindexes), a1_gridvals(level1ii(ii)+1:level1ii(ii+1)-1), a2_gridvals, z_gridvals, e_val, ReturnFnParamsVec,3,0);
                d3aprimez=d3ind+N_d3*(a1primeindexes-1)+N_d3*N_a1*zBind;
                ...
                allind=dind+N_d13*repelem(a2Bind,1,level1iidiff(ii))+N_d13*N_a2*zBind;
                pol_d13_a1=maxindex+N_d13*(loweredge(allind)-1);
                Policy_jj(curraindex,:,e_c)=encodePolicy_with_d2lookup(shiftdim(pol_d13_a1,1),N_d1,N_d2,N_d3,N_d13,d2index_resh,N_a1,N_z);
            else
                loweredge=maxindex1(:,1,ii,:,:);
                ReturnMatrix_ii=CreateReturnFnMatrix_ExpAsset_Disc_e(ReturnFn, n_d1,n_d3,1,level1iidiff(ii),n_a2,n_z,special_n_e, d13_gridvals, a1_gridvals(loweredge), a1_gridvals(level1ii(ii)+1:level1ii(ii+1)-1), a2_gridvals, z_gridvals, e_val, ReturnFnParamsVec,3,0);
                d3aprimez=d3ind+N_d3*(loweredge-1)+N_d3*N_a1*zBind;
...
function pol=encodePolicy_with_d2lookup(pol_d13_a1,N_d1,N_d2,N_d3,N_d13,d2index_resh,N_a1,N_z)
% pol_d13_a1: [npts,N_z]
d1part=rem(pol_d13_a1-1,N_d1)+1;
d3part=rem(ceil(pol_d13_a1/N_d1)-1,N_d3)+1;
a1primepart=ceil(pol_d13_a1/N_d13);
...
function pol=encodePolicy_with_d2lookup_e(pol_d13_a1,N_d1,N_d2,N_d3,N_d13,d2index_resh,N_a1,N_z,N_e)
% pol_d13_a1: [npts,N_z,N_e]
d1part=rem(pol_d13_a1-1,N_d1)+1;
d3part=rem(ceil(pol_d13_a1/N_d1)-1,N_d3)+1;
a1primepart=ceil(pol_d13_a1/N_d13);
