...
if ~isfield(vfoptions,'V_Jplus1')
    ...
    [V(:,N_j),Policy(:,N_j)]=internal_per_j_nod1_noz(EVpre,a2primeIndex,a2primeProbs,ReturnFn,DiscountFactorParamsVec,ReturnFnParamsVec,...
        n_d3,n_a1,n_a2,N_d2,N_d3,N_d23,N_a1,N_a2,N_a,N_u,...
        d3_gridvals,a1_gridvals,a2_gridvals,pi_u_col,...
...
for reverse_j=1:N_j-1
    ...
    EVnext=V(:,jj+1);
    [V(:,jj),Policy(:,jj)]=internal_per_j_nod1_noz(EVnext,a2primeIndex,a2primeProbs,ReturnFn,DiscountFactorParamsVec,ReturnFnParamsVec,...
        n_d3,n_a1,n_a2,N_d2,N_d3,N_d23,N_a1,N_a2,N_a,N_u,...
        d3_gridvals,a1_gridvals,a2_gridvals,pi_u_col,...
        level1ii,level1iidiff,a2Bind,d3ind,vfoptions);
end
...
function [V_jj,Policy_jj]=internal_per_j_nod1_noz(EVnext,a2primeIndex,a2primeProbs,ReturnFn,DiscountFactorParamsVec,ReturnFnParamsVec,...
    n_d3,n_a1,n_a2,N_d2,N_d3,N_d23,N_a1,N_a2,N_a,N_u,...
    d3_gridvals,a1_gridvals,a2_gridvals,pi_u_col,...
    level1ii,level1iidiff,a2Bind,d3ind,vfoptions)
...
entireRHS_ii=RM+DEV;
entireRHS_ii=reshape(entireRHS_ii,[N_d3,vfoptions.level1n,N_a1,N_a2]);

[~,maxindex1]=max(entireRHS_ii,[],2);
[Vtempii,maxindex2]=max(reshape(entireRHS_ii,[N_d3*N_a1,vfoptions.level1n*N_a2]),[],1);
curraindex=repmat(level1ii',N_a2,1)+N_a1*repelem((0:1:N_a2-1)',vfoptions.level1n,1);
V_jj(curraindex)=shiftdim(Vtempii,1);
Policy_jj(curraindex)=encodePolicy_with_d2lookup_nod1_z(shiftdim(maxindex2,1),N_d2,N_d3,d2index_resh,N_a1);
...
for ii=1:(vfoptions.level1n-1)
    ...
    if maxgap(ii)>0
        ...
        allind=dind+N_d3*repelem(a2Bind,1,level1iidiff(ii));
        pol_d3_a1=maxindex+N_d3*(loweredge(allind)-1);
        Policy_jj(curraindex)=encodePolicy_with_d2lookup_nod1_z(shiftdim(pol_d3_a1,1),N_d2,N_d3,d2index_resh,N_a1);
    else
        loweredge=maxindex1(:,1,ii,:);
