function [V,Policy]=ValueFnIter_FHorz_RiskyAsset_Refine_nod1_raw(n_d2,n_d3,n_a1,n_a2,n_z,n_u,N_j, d2_grid, d3_grid, a1_grid, a2_grid, z_gridvals_J, u_grid, pi_z_J, pi_u, ReturnFn, aprimeFn, Parameters, DiscountFactorParamNames, ReturnFnParamNames, aprimeFnParamNames, vfoptions)
...
Policy3=zeros(3,N_a,N_z,N_j,'gpuArray'); % d2, d3 and a1prime
...
if ~isfield(vfoptions,'V_Jplus1')
    if vfoptions.lowmemory==0
        ...
        Policy3(2,:,:,N_j)=shiftdim(dindex,-1);
    ...
    elseif vfoptions.lowmemory==1
        for z_c=1:N_z
            ...
            % Now put together entireRHS, which just depends on d3
            entireRHS_z=ReturnMatrix_z+shiftdim(DiscountFactorParamsVec*EV_onlyd3,1);
            
            %Calc the max and it's index
            [Vtemp,maxindex]=max(entireRHS_z,[],1);
            V(:,z_c,N_j)=Vtemp;
            Policy3(2,:,z_c,N_j)=shiftdim(rem(maxindex-1,N_d3)+1,1); % d3
            Policy3(3,:,z_c,N_j)=shiftdim(ceil(maxindex/N_d3),-1);
            Policy3(1,:,z_c,N_j)=shiftdim(d2index(maxindex),1); % d2
        end
    end
end
...
for reverse_j=1:N_j-1
    ...
    elseif vfoptions.lowmemory==1
        for z_c=1:N_z
            ...
            Policy3(1,:,z_c,jj)=shiftdim(d2index(maxindex),1);
        end
    end
end
