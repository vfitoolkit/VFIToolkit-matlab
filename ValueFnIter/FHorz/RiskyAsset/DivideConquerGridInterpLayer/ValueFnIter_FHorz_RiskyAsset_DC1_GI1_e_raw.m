...
% e: iid start-of-period shock (integrated out of EVnext before per_j)
...
if ~isfield(vfoptions,'V_Jplus1')
    if vfoptions.lowmemory==0
        midpoint=zeros(N_d13,1,N_a1,N_a2,N_z,N_e,'gpuArray');
    ...
    [V(:,:,:,N_j),Policy3(:,:,:,:,N_j),PolicyL2flag(:,:,:,:,N_j),d2Policy(:,:,:,:,N_j)]=internal_per_j_e(EVpre,a2primeIndex,a2primeProbs,ReturnFn,DiscountFactorParamsVec,ReturnFnParamsVec,...
        n_d1,n_d3,n_a1,n_a2,n_z,n_e,N_d1,N_d2,N_d3,N_d13,N_d23,N_a1,N_a1prime,N_a2,N_a,N_z,N_e,N_u,...
...
for reverse_j=1:N_j-1
    ...
    [V(:,:,:,jj),Policy3(:,:,:,:,jj),PolicyL2flag(:,:,:,:,jj),d2Policy(:,:,:,:,jj)]=internal_per_j_e(EVnext,a2primeIndex,a2primeProbs,ReturnFn,DiscountFactorParamsVec,ReturnFnParamsVec,...
        n_d1,n_d3,n_a1,n_a2,n_z,n_e,N_d1,N_d2,N_d3,N_d13,N_d23,N_a1,N_a1prime,N_a2,N_a,N_z,N_e,N_u,...
...
%% Per-period inner (e). EVnext already integrated over e.
function [V_jj,Policy3_jj,PolicyL2flag_jj,d2Policy_jj]=internal_per_j_e(EVnext,a2primeIndex,a2primeProbs,ReturnFn,DiscountFactorParamsVec,ReturnFnParamsVec,...
    n_d1,n_d3,n_a1,n_a2,n_z,n_e,N_d1,N_d2,N_d3,N_d13,N_d23,N_a1,N_a1prime,N_a2,N_a,N_z,N_e,N_u,...
...
if vfoptions.lowmemory==0
    ...
    midpoint=max(min(midpoint,n_a1(1)-1),2);
    a1primeindexesfine=(midpoint+(midpoint-1)*n2short)+(-n2short-1:1:1+n2short);
    ReturnMatrix_ii=CreateReturnFnMatrix_ExpAsset_Disc_e(ReturnFn, n_d1,n_d3,n2long,n_a1,n_a2,n_z,n_e, d13_gridvals, a1prime_grid(a1primeindexesfine), a1_gridvals, a2_gridvals, z_gridvals, e_gridvals, ReturnFnParamsVec,2,0);
    da1primez=d3ind+N_d3*(a1primeindexesfine-1)+N_d3*N_a1prime*zindB; % e broadcasts via DiscountedEVinterp broadcasting
    ...
    % d2 lookup
    d3part=rem(ceil(shiftdim(d_ind,1)/N_d1)-1,N_d3)+1; % [N_a,N_z,N_e]
    a1mid=squeeze(midpoint(allind)); % [N_a,N_z,N_e]
    ...
    for e_c=1:N_e
        ...
        for ii=1:(vfoptions.level1n-1)
            ...
            if maxgap(ii)>0
                loweredge=min(maxindex1(:,1,ii,:,:),N_a1-maxgap(ii));
                a1primeindexes=loweredge+(0:1:maxgap(ii));
                ReturnMatrix_ii_e=CreateReturnFnMatrix_ExpAsset_Disc_e(ReturnFn, n_d1,n_d3,maxgap(ii)+1,level1iidiff(ii),n_a2,n_z,special_n_e, d13_gridvals, a1_gridvals(a1primeindexes), a1_gridvals(level1ii(ii)+1:level1ii(ii+1)-1), a2_gridvals, z_gridvals, e_val, ReturnFnParamsVec,3,0);
