function [V,Policy]=ValueFnIter_FHorz_RiskyAsset_EpsteinZin_Refine_nod1_raw(n_d2,n_d3,n_a1,n_a2,n_z,n_u,N_j, d2_grid, d3_grid, a1_grid,a2_grid, z_gridvals_J, u_grid, pi_z_J, pi_u, ReturnFn, aprimeFn, Parameters, DiscountFactorParamNames, ReturnFnParamNames, aprimeFnParamNames, vfoptions, sj, warmglow, ezc1,ezc2,ezc3,ezc4,ezc5,ezc6,ezc7,ezc8,ezc9)
...
Policy3=zeros(3,N_a,N_z,N_j,'gpuArray'); % d2, d3, a1prime
...
if ~isfield(vfoptions,'V_Jplus1')
    if vfoptions.lowmemory==0
        ...
        ReturnMatrix=CreateReturnFnMatrix_Case2_Disc_Par2(ReturnFn, [n_d3,n_a1], [n_a1,n_a2], n_z, [d3_grid; a1_grid], [a1_grid; a2_grid], z_gridvals_J(:,:,N_j), ReturnFnParamsVec);
        ...
        if warmglow==1
            ...
            V(:,:,N_j)=Vtemp;
            Policy3(2,:,:,N_j)=shiftdim(rem(maxindex-1,N_d3)+1,1); % d3
            Policy3(3,:,:,N_j)=shiftdim(ceil(maxindex/N_d3),1); % a1prime
            Policy3(1,:,:,N_j)=shiftdim(d2index(maxindex),1); % note: no a nor z in WGmatrix
        elseif warmglow==0
            %Calc the max and it's index
            [Vtemp,maxindex]=max(ReturnMatrix,[],1);

            V(:,:,N_j)=Vtemp;
            Policy3(1,:,:,N_j)=1; % d2, meaningless
            Policy3(2,:,:,N_j)=rem(maxindex-1,N_d3)+1; % d3
            Policy3(3,:,:,N_j)=ceil(maxindex/N_d3); % a1prime
        end
    ...
    elseif vfoptions.lowmemory==1
        ...
        for z_c=1:N_z
            ...
            ReturnMatrix_z=CreateReturnFnMatrix_Case2_Disc_Par2(ReturnFn, [n_d3,n_a1], [n_a1,n_a2], special_n_z, [d3_grid; a1_grid], [a1_grid; a2_grid], z_val, ReturnFnParamsVec);
            ...
            if warmglow==1
                ...
                V(:,z_c,N_j)=Vtemp;
                Policy3(2,:,z_c,N_j)=shiftdim(rem(maxindex-1,N_d3)+1,1);
                Policy3(3,:,z_c,N_j)=shiftdim(ceil(maxindex/N_d3),1);
                Policy3(1,:,z_c,N_j)=shiftdim(d2index(maxindex),1); % note: no a nor z in WGmatrix
            elseif warmglow==0
                %Calc the max and it's index
                [Vtemp,maxindex]=max(ReturnMatrix_z,[],1);

                V(:,z_c,N_j)=Vtemp;
                Policy3(1,:,z_c,N_j)=1; % d2, meaningless
                Policy3(2,:,z_c,N_j)=rem(maxindex-1,N_d3)+1;
                Policy3(3,:,z_c,N_j)=ceil(maxindex/N_d3);
            end
        end
    end
else
    ...
    elseif vfoptions.lowmemory==1
        for z_c=1:N_z
            ...
            Policy3(2,:,z_c,N_j)=shiftdim(rem(maxindex-1,N_d3)+1,1);
            Policy3(3,:,z_c,N_j)=shiftdim(ceil(maxindex/N_d3),1);
            Policy3(1,:,:,N_j)=shiftdim(d2index(maxindex),1); % note: no a nor z in temp4
        end
...
for reverse_j=1:N_j-1
    ...
    if vfoptions.lowmemory==0
        ...
        ReturnMatrix=CreateReturnFnMatrix_Case2_Disc_Par2(ReturnFn, [n_d3,n_a1], [n_a1,n_a2], n_z, [d3_grid; a1_grid], [a1_grid; a2_grid], z_gridvals_J(:,:,jj), ReturnFnParamsVec);
        % (d,aprime,a,z)
    ...
    elseif vfoptions.lowmemory==1
        for z_c=1:N_z
            ...
            [Vtemp,maxindex]=max(entireRHS_z,[],1);
            V(:,z_c,jj)=Vtemp;
            Policy3(2,:,z_c,jj)=shiftdim(rem(maxindex-1,N_d3)+1,1);
            Policy3(3,:,z_c,jj)=shiftdim(ceil(maxindex/N_d3),1);
            Policy3(1,:,:,jj)=shiftdim(d2index(maxindex),1); % note: no a nor z in temp4
        end
