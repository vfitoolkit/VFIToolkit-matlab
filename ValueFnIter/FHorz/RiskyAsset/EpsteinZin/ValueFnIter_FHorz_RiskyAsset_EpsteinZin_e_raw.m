...
if ~isfield(vfoptions,'V_Jplus1')
    ...
    elseif vfoptions.lowmemory==1
        ...
        for e_c=1:N_e
            ...
            if warmglow==1
                ...
                Policy4(2,:,:,e_c,N_j)=shiftdim(d2index(maxindex),1); % note: no a nor z in WGmatrix
            elseif warmglow==0
                %Calc the max and it's index
                ...
                V(:,:,e_c,N_j)=Vtemp;
                dindex=rem(maxindex-1,N_d13)+1;
                Policy4(1,:,:,e_c,N_j)=rem(dindex-1,N_d1)+1;
                Policy4(2,:,:,e_c,N_j)=1; % d2, meaningless
                Policy4(3,:,:,e_c,N_j)=ceil(dindex/N_d1);
                Policy4(4,:,:,e_c,N_j)=ceil(maxindex/N_d13);
            end
        end
        ...
        for e_c=1:N_e
            ...
            for z_c=1:N_z
                ...
                if warmglow==1
                    ...
                    Policy4(2,:,z_c,e_c,N_j)=shiftdim(d2index(maxindex),1); % note: no a nor z in WGmatrix
                elseif warmglow==0
                    %Calc the max and it's index
                    ...
                    V(:,z_c,e_c,N_j)=Vtemp;
                    dindex=rem(maxindex-1,N_d13)+1;
                    Policy4(1,:,z_c,e_c,N_j)=rem(dindex-1,N_d1)+1;
                    Policy4(2,:,z_c,e_c,N_j)=1; % d2, meaningless
                    Policy4(3,:,z_c,e_c,N_j)=ceil(dindex/N_d1);
                    Policy4(4,:,z_c,e_c,N_j)=ceil(maxindex/N_d13);
                end
            end
        end
    ...
    if vfoptions.lowmemory==0
        ...
        Policy4(3,:,:,:,N_j)=shiftdim(rem(maxindex-1,N_d3)+1,1);
        Policy4(4,:,:,:,N_j)=shiftdim(ceil(maxindex/N_d3),1);
        Policy4(1,:,:,:,N_j)=shiftdim(d1index(maxindex+N_d3*N_a1*aind+N_d3*N_a1*N_a*zind+N_d3*N_a1*N_a*N_z*eind),1);
    ...
    elseif vfoptions.lowmemory==1
        ...
        for e_c=1:N_e
            ...
            Policy4(3,:,:,e_c,N_j)=shiftdim(rem(maxindex-1,N_d3)+1,1);
            Policy4(4,:,:,e_c,N_j)=shiftdim(ceil(maxindex/N_d3),1);
            Policy4(1,:,:,e_c,N_j)=shiftdim(d1index(maxindex+N_d3*N_a1*aind+N_d3*N_a1*N_a*zind),1);
            Policy4(2,:,:,e_c,N_j)=shiftdim(d2index(maxindex+N_d3*N_a1*zind),1); % note: no a in temp4
        end
    ...
    elseif vfoptions.lowmemory==2
        for z_c=1:N_z
            ...
            for e_c=1:N_e
                ...
                Policy4(3,:,z_c,e_c,N_j)=shiftdim(rem(maxindex-1,N_d3)+1,1);
                Policy4(4,:,z_c,e_c,N_j)=shiftdim(ceil(maxindex/N_d3),1);
                Policy4(1,:,z_c,e_c,N_j)=shiftdim(d1index(maxindex+N_d3*N_a1*aind),1);
                Policy4(2,:,z_c,e_c,N_j)=shiftdim(d2index(maxindex),1); % note: no a in temp4
            end
        end
    end
end
...
for reverse_j=1:N_j-1
    ...
    if vfoptions.lowmemory==0
        ...
        Policy4(1,:,:,:,jj)=shiftdim(d1index(maxindex+N_d3*N_a1*aind+N_d3*N_a1*N_a*zind+N_d3*N_a1*N_a*N_z*eind),1);
    ...
    elseif vfoptions.lowmemory==2
        ...
        for z_c=1:N_z
            ...
            for e_c=1:N_e
                ...
                Policy4(1,:,z_c,e_c,jj)=shiftdim(d1index(maxindex+N_d3*N_a1*aind),1);
                Policy4(2,:,z_c,e_c,jj)=shiftdim(d2index(maxindex),1); % note: no a in temp4
            end
        end
    end
