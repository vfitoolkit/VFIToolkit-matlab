...
if ~isfield(vfoptions,'V_Jplus1')
    ...
    for ii=1:(vfoptions.level1n-1)
        ...
        if maxgap(ii)>0
            ...
            entireRHS_ii=reshape(reshape(ReturnMatrix_ii,[N_d2*(maxgap(ii)+1),level1iidiff(ii),N_a2])+reshape(DiscountedEV(d2aprime),[N_d2*(maxgap(ii)+1),1,N_a2]),[N_d2*(maxgap(ii)+1),level1iidiff(ii)*N_a2]);
            ...
            entireRHS_ii=reshape(reshape(ReturnMatrix_ii,[N_d2,level1iidiff(ii),N_a2])+reshape(DiscountedEV(d2aprime),[N_d2,1,N_a2]),[N_d2,level1iidiff(ii)*N_a2]);
            [Vtempii,maxindex]=max(entireRHS_ii,[],1);
            V(curraindex,N_j)=shiftdim(Vtempii,1);
            % maxindex does not need reworking, as with expasset there is no a2prime
            %  the a1prime is relative to loweredge(allind), need to 'add' the loweredge
