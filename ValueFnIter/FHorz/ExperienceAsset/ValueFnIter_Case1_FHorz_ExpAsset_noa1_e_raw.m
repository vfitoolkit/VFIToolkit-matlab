function [V,Policy]=ValueFnIter_Case1_FHorz_ExpAsset_noa1_e_raw(n_d1,n_d2,n_a2,n_z,n_e,N_j, d1_grid, d2_grid, a2_grid,z_gridvals_J,e_gridvals_J,pi_z_J,pi_e_J, ReturnFn, aprimeFn, Parameters, DiscountFactorParamNames, ReturnFnParamNames, aprimeFnParamNames, vfoptions)
...
if ~isfield(vfoptions,'V_Jplus1')
    ...
    V_Jplus1=sum(shiftdim(pi_e_J(:,N_j),2).*reshape(vfoptions.V_Jplus1,[N_a,N_z,N_e]),3);
    % Using V_Jplus1
    ...
    entireRHS=ReturnMatrix+DiscountFactorParamsVec*repelem(entireEV,N_d1,1); % should autofill e dimension
...
for reverse_j=1:N_j-1
    ...
    EV1=Vnext(a2primeIndex,:).*shiftdim(pi_z_J(:,:,jj)',-1);    % (d2,a2,zprime), the lower aprime
    EV1(isnan(EV1))=0; % where V=-Inf but pi_z is zero
    EV1=sum(EV1,2);
    EV2=Vnext(a2primeIndex+1,:).*shiftdim(pi_z_J(:,:,jj)',-1);    % (d2,a2,zprime), the upper aprime
    EV2(isnan(EV2))=0; % where V=-Inf but pi_z is zero
    EV2=sum(EV2,2);
