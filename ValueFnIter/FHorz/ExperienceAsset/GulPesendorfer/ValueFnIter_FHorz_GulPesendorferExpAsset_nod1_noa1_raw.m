function [V,Policy]=ValueFnIter_FHorz_GulPesendorferExpAsset_nod1_noa1_raw(n_d2,n_a2,n_z,N_j, d2_gridvals, a2_grid,z_gridvals_J,pi_z_J, ReturnFn, TemptationFn, aprimeFn, Parameters, DiscountFactorParamNames, ReturnFnParamNames, TemptationFnParamNames, aprimeFnParamNames, vfoptions)
...
if ~isfield(vfoptions,'V_Jplus1')
    if vfoptions.lowmemory==0
        ...
        TemptationMatrix=CreateReturnFnMatrix_Case2_Disc(TemptationFn,n_d2, n_a2, n_z, d2_gridvals, a2_gridvals, z_gridvals_J(:,:,N_j), TemptationFnParamsVec); % with only the experience asset, can just use Case2 command
    ...
    elseif vfoptions.lowmemory==1
        for z_c=1:N_z
            ...
            TemptationMatrix_z=CreateReturnFnMatrix_Case2_Disc(TemptationFn,n_d2, n_a2, special_n_z, d2_gridvals, a2_gridvals, z_val, TemptationFnParamsVec); % with only the experience asset, can just use Case2 command
    ...
    elseif vfoptions.lowmemory==1
        for z_c=1:N_z
            ...
            entireRHS=ReturnMatrix_z+TemptationMatrix_z+DiscountFactorParamsVec*EV_z;

            % Calc the max and it's index
            [Vtemp,maxindex]=max(entireRHS,[],1);

            V(:,z_c,N_j)=shiftdim(Vtemp-MostTempting_z,1);
...
for reverse_j=1:N_j-1
    ...
    if vfoptions.lowmemory==0
        ...
        TemptationMatrix=CreateReturnFnMatrix_Case2_Disc(TemptationFn,n_d2, n_a2, n_z, d2_gridvals, a2_gridvals, z_gridvals_J(:,:,jj), TemptationFnParamsVec); % with only the experience asset, can just use Case2 command
        MostTempting=max(TemptationMatrix,[],1);
        MostTempting(MostTempting==-Inf)=0; % a state where EVERY choice is infeasible leaves this -Inf: V there must be -Inf (as in the standard solver), not -Inf-(-Inf)=NaN
    ...
    elseif vfoptions.lowmemory==1
        for z_c=1:N_z
            ...
            V(:,z_c,jj)=shiftdim(Vtemp-MostTempting_z,1);
