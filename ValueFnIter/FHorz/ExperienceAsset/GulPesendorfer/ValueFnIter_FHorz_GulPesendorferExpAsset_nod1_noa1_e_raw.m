function [V,Policy]=ValueFnIter_FHorz_GulPesendorferExpAsset_nod1_noa1_e_raw(n_d2,n_a2,n_z,n_e,N_j, d2_gridvals, a2_grid,z_gridvals_J,e_gridvals_J,pi_z_J,pi_e_J, ReturnFn, TemptationFn, aprimeFn, Parameters, DiscountFactorParamNames, ReturnFnParamNames, TemptationFnParamNames, aprimeFnParamNames, vfoptions)
...
if ~isfield(vfoptions,'V_Jplus1')
    if vfoptions.lowmemory==0
        ...
        TemptationMatrix=CreateReturnFnMatrix_Case2_Disc_e(TemptationFn,n_d2, n_a2, n_z,n_e, d2_gridvals, a2_gridvals, z_gridvals_J(:,:,N_j), e_gridvals_J(:,:,N_j), TemptationFnParamsVec); % with only the experience asset, can just use Case2 command
    ...
    elseif vfoptions.lowmemory==1
        for e_c=1:N_e
            ...
            TemptationMatrix_e=CreateReturnFnMatrix_Case2_Disc_e(TemptationFn,n_d2, n_a2, n_z,special_n_e, d2_gridvals, a2_gridvals, z_gridvals_J(:,:,N_j), e_gridvals_J(e_c,:,N_j), TemptationFnParamsVec); % with only the experience asset, can just use Case2 command
    ...
    elseif vfoptions.lowmemory==2
        for z_c=1:N_z
            for e_c=1:N_e
                ...
                TemptationMatrix_ze=CreateReturnFnMatrix_Case2_Disc_e(TemptationFn,n_d2, n_a2, special_n_z,special_n_e, d2_gridvals, a2_gridvals, z_gridvals_J(z_c,:,N_j), e_gridvals_J(e_c,:,N_j), TemptationFnParamsVec); % with only the experience asset, can just use Case2 command
    ...
    if vfoptions.lowmemory==0
        ...
        entireRHS=ReturnMatrix+TemptationMatrix+DiscountFactorParamsVec*EV; % should autofill the e dimension
    ...
    elseif vfoptions.lowmemory==1
        for e_c=1:N_e
            ...
            TemptationMatrix_e=CreateReturnFnMatrix_Case2_Disc_e(TemptationFn,n_d2, n_a2, n_z, special_n_e, d2_gridvals, a2_gridvals, z_gridvals_J(:,:,N_j), e_val, TemptationFnParamsVec); % with only the experience asset, can just use Case2 command
            MostTempting_e=max(TemptationMatrix_e,[],1);
            MostTempting_e(MostTempting_e==-Inf)=0; % a state where EVERY choice is infeasible leaves this -Inf: V there must be -Inf (as in the standard solver), not -Inf-(-Inf)=NaN

            entireRHS=ReturnMatrix_e+TemptationMatrix_e+DiscountFactorParamsVec*EV;
    ...
    elseif vfoptions.lowmemory==2
        for z_c=1:N_z
            ...
            for e_c=1:N_e
                ...
                TemptationMatrix_ze=CreateReturnFnMatrix_Case2_Disc_e(TemptationFn,n_d2, n_a2, special_n_z, special_n_e, d2_gridvals, a2_gridvals, z_val, e_val, TemptationFnParamsVec); % with only the experience asset, can just use Case2 command
                MostTempting_ze=max(TemptationMatrix_ze,[],1);
                MostTempting_ze(MostTempting_ze==-Inf)=0; % a state where EVERY choice is infeasible leaves this -Inf: V there must be -Inf (as in the standard solver), not -Inf-(-Inf)=NaN

                entireRHS=ReturnMatrix_ze+TemptationMatrix_ze+DiscountFactorParamsVec*EV_z;
...
for reverse_j=1:N_j-1
    ...
    if vfoptions.lowmemory==0
        ...
        TemptationMatrix=CreateReturnFnMatrix_Case2_Disc_e(TemptationFn,n_d2, n_a2, n_z, n_e, d2_gridvals, a2_gridvals, z_gridvals_J(:,:,jj), e_gridvals_J(:,:,jj), TemptationFnParamsVec); % with only the experience asset, can just use Case2 command
        MostTempting=max(TemptationMatrix,[],1);
        MostTempting(MostTempting==-Inf)=0; % a state where EVERY choice is infeasible leaves this -Inf: V there must be -Inf (as in the standard solver), not -Inf-(-Inf)=NaN
    ...
    elseif vfoptions.lowmemory==1
        for e_c=1:N_e
            ...
            TemptationMatrix_e=CreateReturnFnMatrix_Case2_Disc_e(TemptationFn,n_d2, n_a2, n_z, special_n_e, d2_gridvals, a2_gridvals, z_gridvals_J(:,:,jj), e_val, TemptationFnParamsVec); % with only the experience asset, can just use Case2 command
            MostTempting_e=max(TemptationMatrix_e,[],1);
            MostTempting_e(MostTempting_e==-Inf)=0; % a state where EVERY choice is infeasible leaves this -Inf: V there must be -Inf (as in the standard solver), not -Inf-(-Inf)=NaN
