function [V,Policy]=ValueFnIter_FHorz_GulPesendorferExpAsset_DC1_GI1_e_raw(n_d1,n_d2,n_a1,n_a2,n_z,n_e,N_j, d_gridvals, d2_gridvals, a1_gridvals, a2_grid, z_gridvals_J, e_gridvals_J, pi_z_J, pi_e_J, ReturnFn, TemptationFn, aprimeFn, Parameters, DiscountFactorParamNames, ReturnFnParamNames, TemptationFnParamNames, aprimeFnParamNames, vfoptions)
...
if vfoptions.lowmemory==0
    ...
    midpointT=zeros(N_d,1,N_a1,N_a2,N_z,N_e,'gpuArray'); % v's own coarse argmax (for the most-tempting term)
elseif vfoptions.lowmemory==1
    midpoint=zeros(N_d,1,N_a1,N_a2,N_z,'gpuArray');
    midpointT=zeros(N_d,1,N_a1,N_a2,N_z,'gpuArray'); % v's own coarse argmax (for the most-tempting term)
elseif vfoptions.lowmemory==2
    midpoint=zeros(N_d,1,N_a1,N_a2,'gpuArray');
    midpointT=zeros(N_d,1,N_a1,N_a2,'gpuArray'); % v's own coarse argmax (for the most-tempting term)
end
...
if ~isfield(vfoptions,'V_Jplus1')
 if vfoptions.lowmemory==0
        ...
        for ii=1:(vfoptions.level1n-1)
            ...
            TemptationMatrix_full=CreateReturnFnMatrix_ExpAsset_Disc_e(TemptationFn, n_d1,n_d2,n_a1,length(curraindex),n_a2,n_z,n_e, d_gridvals, a1_gridvals, a1_gridvals(curraindex), a2_gridvals, z_gridvals_J(:,:,N_j), e_gridvals_J(:,:,N_j), TemptationFnParamsVec,1,0);
            ...
            if maxgap(ii)>0
                ...
                TemptationMatrix_ii=CreateReturnFnMatrix_ExpAsset_Disc_e(TemptationFn, n_d1,n_d2,maxgap(ii)+1,level1iidiff(ii),n_a2,n_z,n_e, d_gridvals, a1_gridvals(a1primeindexes), a1_gridvals(level1ii(ii)+1:level1ii(ii+1)-1), a2_gridvals, z_gridvals_J(:,:,N_j), e_gridvals_J(:,:,N_j), TemptationFnParamsVec,3,0); % Level 3 as DC1+GI; Level=3, Refine=0
        ...
        TemptationMatrix_ii=CreateReturnFnMatrix_ExpAsset_Disc_e(TemptationFn, n_d1, n_d2, n2long, n_a1,n_a2,n_z,n_e, d_gridvals, a1prime_grid(a1primeindexes), a1_gridvals, a2_gridvals, z_gridvals_J(:,:,N_j), e_gridvals_J(:,:,N_j), TemptationFnParamsVec,2,0); % [N_d,N_a1prime,N_a1,N_a2,N_z,N_e]; Level=2, Refine=0
        ...
        TemptationMatrix_Tii=CreateReturnFnMatrix_ExpAsset_Disc_e(TemptationFn, n_d1, n_d2, n2long, n_a1,n_a2,n_z,n_e, d_gridvals, a1prime_grid(a1primeindexesT), a1_gridvals, a2_gridvals, z_gridvals_J(:,:,N_j), e_gridvals_J(:,:,N_j), TemptationFnParamsVec,2,0);
    ...
    elseif vfoptions.lowmemory==1
        for e_c=1:N_e
            ...
            for ii=1:(vfoptions.level1n-1)
                curraindex=(level1ii(ii)+1:1:level1ii(ii+1)-1)'; % just a1
                % v's coarse argmax over the FULL a1prime grid for these a1 (never just the window)
                TemptationMatrix_full=CreateReturnFnMatrix_ExpAsset_Disc_e(TemptationFn, n_d1,n_d2,n_a1,length(curraindex),n_a2,n_z,special_n_e, d_gridvals, a1_gridvals, a1_gridvals(curraindex), a2_gridvals, z_gridvals_J(:,:,N_j), e_val, TemptationFnParamsVec,1,0);
                [~,maxindexT]=max(TemptationMatrix_full,[],2);
                midpointT(:,1,curraindex,:,:)=maxindexT;
                if maxgap(ii)>0
                    loweredge=min(maxindex1(:,1,ii,:,:),N_a1-maxgap(ii)); % maxindex1(ii,:), but avoid going off top of grid when we add maxgap(ii) points
                    ...
                    TemptationMatrix_ii_e=CreateReturnFnMatrix_ExpAsset_Disc_e(TemptationFn, n_d1,n_d2,maxgap(ii)+1,level1iidiff(ii),n_a2,n_z,special_n_e, d_gridvals, a1_gridvals(a1primeindexes), a1_gridvals(level1ii(ii)+1:level1ii(ii+1)-1), a2_gridvals, z_gridvals_J(:,:,N_j), e_val, TemptationFnParamsVec,3,0); % Level 3 as DC1+GI; Level=3, Refine=0
            ...
            TemptationMatrix_ii=CreateReturnFnMatrix_ExpAsset_Disc_e(TemptationFn, n_d1, n_d2, n2long, n_a1,n_a2,n_z,special_n_e, d_gridvals, a1prime_grid(a1primeindexes), a1_gridvals, a2_gridvals, z_gridvals_J(:,:,N_j), e_val, TemptationFnParamsVec,2,0); % [N_d,N_a1prime,N_a1,N_a2,N_z]; Level=2, Refine=0
            Ftemp_ii=ReturnMatrix_ii+TemptationMatrix_ii;
            [Vtempii,maxindexL2]=max(Ftemp_ii,[],1);
            % Most-tempting term: fine refinement of v around v's own coarse argmax
            midpointT=max(min(midpointT,n_a1(1)-1),2);
            a1primeindexesT=(midpointT+(midpointT-1)*n2short)+(-n2short-1:1:1+n2short);
            TemptationMatrix_Tii=CreateReturnFnMatrix_ExpAsset_Disc_e(TemptationFn, n_d1, n_d2, n2long, n_a1,n_a2,n_z,special_n_e, d_gridvals, a1prime_grid(a1primeindexesT), a1_gridvals, a2_gridvals, z_gridvals_J(:,:,N_j), e_val, TemptationFnParamsVec,2,0);
            MostTempting=max(TemptationMatrix_Tii,[],1);
            MostTempting(MostTempting==-Inf)=0; % a state where EVERY choice is infeasible leaves this -Inf: V there must be -Inf (as in the standard solver), not -Inf-(-Inf)=NaN
        ...
        for z_c=1:N_z
            ...
            for e_c=1:N_e
                ...
                for ii=1:(vfoptions.level1n-1)
                    curraindex=(level1ii(ii)+1:1:level1ii(ii+1)-1)'; % just a1
                    % v's coarse argmax over the FULL a1prime grid for these a1 (never just the window)
                    TemptationMatrix_full=CreateReturnFnMatrix_ExpAsset_Disc_e(TemptationFn, n_d1,n_d2,n_a1,length(curraindex),n_a2,special_n_z,special_n_e, d_gridvals, a1_gridvals, a1_gridvals(curraindex), a2_gridvals, z_val, e_val, TemptationFnParamsVec,1,0);
                    [~,maxindexT]=max(TemptationMatrix_full,[],2);
                    midpointT(:,1,curraindex,:)=maxindexT;
                    if maxgap(ii)>0
                        loweredge=min(maxindex1(:,1,ii,:),N_a1-maxgap(ii)); % maxindex1(ii,:), but avoid going off top of grid when we add maxgap(ii) points
                        ...
                        TemptationMatrix_ii_ze=CreateReturnFnMatrix_ExpAsset_Disc_e(TemptationFn, n_d1,n_d2,maxgap(ii)+1,level1iidiff(ii),n_a2,special_n_z,special_n_e, d_gridvals, a1_gridvals(a1primeindexes), a1_gridvals(level1ii(ii)+1:level1ii(ii+1)-1), a2_gridvals, z_val, e_val, TemptationFnParamsVec,3,0); % Level 3 as DC1+GI; Level=3, Refine=0
                ...
                TemptationMatrix_ii=CreateReturnFnMatrix_ExpAsset_Disc_e(TemptationFn, n_d1, n_d2, n2long, n_a1,n_a2,special_n_z,special_n_e, d_gridvals, a1prime_grid(a1primeindexes), a1_gridvals, a2_gridvals, z_val, e_val, TemptationFnParamsVec,2,0); % [N_d,N_a1prime,N_a1,N_a2]; Level=2, Refine=0
                Ftemp_ii=ReturnMatrix_ii+TemptationMatrix_ii;
                [Vtempii,maxindexL2]=max(Ftemp_ii,[],1);
                % Most-tempting term: fine refinement of v around v's own coarse argmax
                midpointT=max(min(midpointT,n_a1(1)-1),2);
                a1primeindexesT=(midpointT+(midpointT-1)*n2short)+(-n2short-1:1:1+n2short);
                TemptationMatrix_Tii=CreateReturnFnMatrix_ExpAsset_Disc_e(TemptationFn, n_d1, n_d2, n2long, n_a1,n_a2,special_n_z,special_n_e, d_gridvals, a1prime_grid(a1primeindexesT), a1_gridvals, a2_gridvals, z_val, e_val, TemptationFnParamsVec,2,0);
                MostTempting=max(TemptationMatrix_Tii,[],1);
                MostTempting(MostTempting==-Inf)=0; % a state where EVERY choice is infeasible leaves this -Inf: V there must be -Inf (as in the standard solver), not -Inf-(-Inf)=NaN
    ...
    if vfoptions.lowmemory==0
        ...
        for ii=1:(vfoptions.level1n-1)
            ...
            if maxgap(ii)>0
                ...
                entireRHS_ii=ReturnMatrix_ii+TemptationMatrix_ii+DiscountedEV(d2aprimez);
        ...
        TemptationMatrix_ii=CreateReturnFnMatrix_ExpAsset_Disc_e(TemptationFn, n_d1, n_d2, n2long, n_a1,n_a2,n_z,n_e, d_gridvals, a1prime_grid(a1primeindexesfine), a1_gridvals, a2_gridvals, z_gridvals_J(:,:,N_j), e_gridvals_J(:,:,N_j), TemptationFnParamsVec,2,0); % [N_d,N_a1prime,N_a1,N_a2,N_z,N_e]; Level=2, Refine=0
        Ftemp_ii=ReturnMatrix_ii+TemptationMatrix_ii;
        ...
        entireRHS_ii=Ftemp_ii+reshape(DiscountedEVinterp(da1primea2z),[N_d*n2long,N_a1*N_a2,N_z,N_e]);
        ...
        for e_c=1:N_e
            ...
            for ii=1:(vfoptions.level1n-1)
                ...
                if maxgap(ii)>0
                    ...
                    entireRHS_ii_e=ReturnMatrix_ii_e+TemptationMatrix_ii_e+DiscountedEV(d2aprimez);
            ...
            TemptationMatrix_ii=CreateReturnFnMatrix_ExpAsset_Disc_e(TemptationFn, n_d1,n_d2,n2long,n_a1,n_a2,n_z,special_n_e, d_gridvals, a1prime_grid(a1primeindexesfine), a1_gridvals, a2_gridvals, z_gridvals_J(:,:,N_j), e_val, TemptationFnParamsVec,2,0); % [N_d,N_a1prime,N_a1,N_a2,N_z]; Level=2, Refine=0
            Ftemp_ii=ReturnMatrix_ii+TemptationMatrix_ii;
            ...
            entireRHS_ii=Ftemp_ii+reshape(DiscountedEVinterp(da1primea2z),[N_d*n2long,N_a1*N_a2,N_z]);
            [Vtempii,maxindexL2]=max(entireRHS_ii,[],1);
            % Most-tempting term: fine refinement of v around v's own coarse argmax
            midpointT=max(min(midpointT,n_a1(1)-1),2);
            a1primeindexesT=(midpointT+(midpointT-1)*n2short)+(-n2short-1:1:1+n2short);
            ...
            for e_c=1:N_e
                ...
                for ii=1:(vfoptions.level1n-1)
                    ...
                    if maxgap(ii)>0
                        ...
                        entireRHS_ii_ze=ReturnMatrix_ii_z+TemptationMatrix_ii_z+DiscountedEV_z(d2aprime);
                ...
                TemptationMatrix_ii=CreateReturnFnMatrix_ExpAsset_Disc_e(TemptationFn, n_d1, n_d2, n2long, n_a1,n_a2,special_n_z,special_n_e, d_gridvals, a1prime_grid(a1primeindexesfine), a1_gridvals, a2_gridvals, z_val, e_val, TemptationFnParamsVec,2,0); % [N_d,N_a1prime,N_a1,N_a2]; Level=2, Refine=0
                Ftemp_ii=ReturnMatrix_ii+TemptationMatrix_ii;
                ...
                entireRHS_ii=Ftemp_ii+reshape(DiscountedEVinterp_z(da1primea2),[N_d*n2long,N_a1*N_a2]);
                [Vtempii,maxindexL2]=max(entireRHS_ii,[],1);
                % Most-tempting term: fine refinement of v around v's own coarse argmax
                midpointT=max(min(midpointT,n_a1(1)-1),2);
                a1primeindexesT=(midpointT+(midpointT-1)*n2short)+(-n2short-1:1:1+n2short);
...
for reverse_j=1:N_j-1
    ...
    if vfoptions.lowmemory==0
        ...
        for ii=1:(vfoptions.level1n-1)
            curraindex=(level1ii(ii)+1:1:level1ii(ii+1)-1)'; % just a1
            % v's coarse argmax over the FULL a1prime grid for these a1 (never just the window)
            TemptationMatrix_full=CreateReturnFnMatrix_ExpAsset_Disc_e(TemptationFn, n_d1,n_d2,n_a1,length(curraindex),n_a2,n_z,n_e, d_gridvals, a1_gridvals, a1_gridvals(curraindex), a2_gridvals, z_gridvals_J(:,:,jj), e_gridvals_J(:,:,jj), TemptationFnParamsVec,1,0);
            [~,maxindexT]=max(TemptationMatrix_full,[],2);
            midpointT(:,1,curraindex,:,:,:)=maxindexT;
            if maxgap(ii)>0
                loweredge=min(maxindex1(:,1,ii,:,:,:),N_a1-maxgap(ii)); % maxindex1(ii,:), but avoid going off top of grid when we add maxgap(ii) points
                ...
                TemptationMatrix_ii=CreateReturnFnMatrix_ExpAsset_Disc_e(TemptationFn, n_d1,n_d2,maxgap(ii)+1,level1iidiff(ii),n_a2,n_z,n_e, d_gridvals, a1_gridvals(a1primeindexes), a1_gridvals(level1ii(ii)+1:level1ii(ii+1)-1), a2_gridvals, z_gridvals_J(:,:,jj), e_gridvals_J(:,:,jj), TemptationFnParamsVec,3,0); % Level 3 as DC1+GI; Level=3, Refine=0
                d2aprimez=d2ind+N_d2*(a1primeindexes-1)+N_d2*N_a1*a2ind+N_d2*N_a1*N_a2*zind; % [N_d,maxgap+1,1,N_a2,N_z,N_e]; linear index into DiscountedEV [N_d2,N_a1,1,N_a2,N_z]
                entireRHS_ii=ReturnMatrix_ii+TemptationMatrix_ii+DiscountedEV(d2aprimez);
                [~,maxindex]=max(entireRHS_ii,[],2);
                midpoint(:,1,curraindex,:,:,:)=maxindex+(loweredge-1);
        ...
        TemptationMatrix_ii=CreateReturnFnMatrix_ExpAsset_Disc_e(TemptationFn, n_d1, n_d2, n2long, n_a1,n_a2,n_z,n_e, d_gridvals, a1prime_grid(a1primeindexesfine), a1_gridvals, a2_gridvals, z_gridvals_J(:,:,jj), e_gridvals_J(:,:,jj), TemptationFnParamsVec,2,0); % [N_d,N_a1prime,N_a1,N_a2,N_z,N_e]; Level=2, Refine=0
        Ftemp_ii=ReturnMatrix_ii+TemptationMatrix_ii;
        da1primea2z=d2ind+N_d2*(a1primeindexesfine-1)+N_d2*N_a1prime*a2ind+N_d2*N_a1prime*N_a2*zind; % [N_d,n2long,N_a1,N_a2,N_z,N_e]; linear index into DiscountedEVinterp [N_d2,N_a1prime,1,N_a2,N_z]
        entireRHS_ii=Ftemp_ii+reshape(DiscountedEVinterp(da1primea2z),[N_d*n2long,N_a1*N_a2,N_z,N_e]);
        [Vtempii,maxindexL2]=max(entireRHS_ii,[],1);
        ...
        midpointT=max(min(midpointT,n_a1(1)-1),2);
        a1primeindexesT=(midpointT+(midpointT-1)*n2short)+(-n2short-1:1:1+n2short);
        TemptationMatrix_Tii=CreateReturnFnMatrix_ExpAsset_Disc_e(TemptationFn, n_d1, n_d2, n2long, n_a1,n_a2,n_z,n_e, d_gridvals, a1prime_grid(a1primeindexesT), a1_gridvals, a2_gridvals, z_gridvals_J(:,:,jj), e_gridvals_J(:,:,jj), TemptationFnParamsVec,2,0);
        MostTempting=max(TemptationMatrix_Tii,[],1);
        MostTempting(MostTempting==-Inf)=0; % a state where EVERY choice is infeasible leaves this -Inf: V there must be -Inf (as in the standard solver), not -Inf-(-Inf)=NaN
    ...
    elseif vfoptions.lowmemory==1
        ...
        for e_c=1:N_e
            ...
            for ii=1:(vfoptions.level1n-1)
                curraindex=(level1ii(ii)+1:1:level1ii(ii+1)-1)'; % just a1
                % v's coarse argmax over the FULL a1prime grid for these a1 (never just the window)
                TemptationMatrix_full=CreateReturnFnMatrix_ExpAsset_Disc_e(TemptationFn, n_d1,n_d2,n_a1,length(curraindex),n_a2,n_z,special_n_e, d_gridvals, a1_gridvals, a1_gridvals(curraindex), a2_gridvals, z_gridvals_J(:,:,jj), e_val, TemptationFnParamsVec,1,0);
                [~,maxindexT]=max(TemptationMatrix_full,[],2);
                midpointT(:,1,curraindex,:,:)=maxindexT;
                if maxgap(ii)>0
                    loweredge=min(maxindex1(:,1,ii,:,:),N_a1-maxgap(ii)); % maxindex1(ii,:), but avoid going off top of grid when we add maxgap(ii) points
                    ...
                    TemptationMatrix_ii_e=CreateReturnFnMatrix_ExpAsset_Disc_e(TemptationFn, n_d1,n_d2,maxgap(ii)+1,level1iidiff(ii),n_a2,n_z,special_n_e, d_gridvals, a1_gridvals(a1primeindexes), a1_gridvals(level1ii(ii)+1:level1ii(ii+1)-1), a2_gridvals, z_gridvals_J(:,:,jj), e_val, TemptationFnParamsVec,3,0); % Level 3 as DC1+GI; Level=3, Refine=0
                    d2aprimez=d2ind+N_d2*(a1primeindexes-1)+N_d2*N_a1*a2ind+N_d2*N_a1*N_a2*zind; % [N_d,maxgap+1,1,N_a2,N_z]; linear index into DiscountedEV [N_d2,N_a1,1,N_a2,N_z]
                    entireRHS_ii_e=ReturnMatrix_ii_e+TemptationMatrix_ii_e+DiscountedEV(d2aprimez);
                    [~,maxindex]=max(entireRHS_ii_e,[],2);
                    midpoint(:,1,curraindex,:,:)=maxindex+(loweredge-1);
            ...
            TemptationMatrix_ii=CreateReturnFnMatrix_ExpAsset_Disc_e(TemptationFn, n_d1,n_d2,n2long,n_a1,n_a2,n_z,special_n_e, d_gridvals, a1prime_grid(a1primeindexesfine), a1_gridvals, a2_gridvals, z_gridvals_J(:,:,jj), e_val, TemptationFnParamsVec,2,0); % [N_d,N_a1prime,N_a1,N_a2,N_z]; Level=2, Refine=0
            Ftemp_ii=ReturnMatrix_ii+TemptationMatrix_ii;
            da1primea2z=d2ind+N_d2*(a1primeindexesfine-1)+N_d2*N_a1prime*a2ind+N_d2*N_a1prime*N_a2*zind; % [N_d,n2long,N_a1,N_a2,N_z]; linear index into DiscountedEVinterp [N_d2,N_a1prime,1,N_a2,N_z]
            entireRHS_ii=Ftemp_ii+reshape(DiscountedEVinterp(da1primea2z),[N_d*n2long,N_a1*N_a2,N_z]);
            [Vtempii,maxindexL2]=max(entireRHS_ii,[],1);
            ...
            midpointT=max(min(midpointT,n_a1(1)-1),2);
            a1primeindexesT=(midpointT+(midpointT-1)*n2short)+(-n2short-1:1:1+n2short);
            TemptationMatrix_Tii=CreateReturnFnMatrix_ExpAsset_Disc_e(TemptationFn, n_d1,n_d2,n2long,n_a1,n_a2,n_z,special_n_e, d_gridvals, a1prime_grid(a1primeindexesT), a1_gridvals, a2_gridvals, z_gridvals_J(:,:,jj), e_val, TemptationFnParamsVec,2,0);
            MostTempting=max(TemptationMatrix_Tii,[],1);
            MostTempting(MostTempting==-Inf)=0; % a state where EVERY choice is infeasible leaves this -Inf: V there must be -Inf (as in the standard solver), not -Inf-(-Inf)=NaN
