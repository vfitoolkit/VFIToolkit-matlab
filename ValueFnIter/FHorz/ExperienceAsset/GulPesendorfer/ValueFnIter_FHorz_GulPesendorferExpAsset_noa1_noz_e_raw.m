function [V,Policy]=ValueFnIter_FHorz_GulPesendorferExpAsset_noa1_noz_e_raw(n_d1,n_d2,n_a2,n_e,N_j, d_gridvals, d2_gridvals, a2_grid,e_gridvals_J,pi_e_J, ReturnFn, TemptationFn, aprimeFn, Parameters, DiscountFactorParamNames, ReturnFnParamNames, TemptationFnParamNames, aprimeFnParamNames, vfoptions)
...
if ~isfield(vfoptions,'V_Jplus1')
    if vfoptions.lowmemory==0
        ...
        TemptationMatrix=CreateReturnFnMatrix_Case2_Disc(TemptationFn,[n_d1,n_d2], n_a2, n_e, d_gridvals, a2_gridvals, e_gridvals_J(:,:,N_j), TemptationFnParamsVec); % with only the experience asset, can just use Case2 command
    ...
    elseif vfoptions.lowmemory==1
        for e_c=1:N_e
            ...
            TemptationMatrix_e=CreateReturnFnMatrix_Case2_Disc(TemptationFn,[n_d1,n_d2], n_a2, special_n_e, d_gridvals, a2_gridvals, e_val, TemptationFnParamsVec); % with only the experience asset, can just use Case2 command
    ...
    if vfoptions.lowmemory==0
        ...
        entireRHS=ReturnMatrix+TemptationMatrix+DiscountFactorParamsVec*repelem(EV,N_d1,1,1); % Should autofill e dimension
...
for reverse_j=1:N_j-1
    ...
    if vfoptions.lowmemory==0
        ...
        TemptationMatrix=CreateReturnFnMatrix_Case2_Disc(TemptationFn,[n_d1,n_d2], n_a2, n_e, d_gridvals, a2_gridvals, e_gridvals_J(:,:,jj), TemptationFnParamsVec); % with only the experience asset, can just use Case2 command
        MostTempting=max(TemptationMatrix,[],1);
        MostTempting(MostTempting==-Inf)=0; % a state where EVERY choice is infeasible leaves this -Inf: V there must be -Inf (as in the standard solver), not -Inf-(-Inf)=NaN
