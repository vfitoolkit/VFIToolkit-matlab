...
if ~isfield(vfoptions,'V_Jplus1')
    if vfoptions.lowmemory==0
        ...
        TemptationMatrix=CreateReturnFnMatrix_ExpAsset_Disc_e(TemptationFn, 0,n_d2,n_a1,n_a1,n_a2,n_z,n_e, d2_gridvals, a1_gridvals, a1_gridvals, a2_gridvals, z_gridvals_J(:,:,N_j), e_gridvals_J(:,:,N_j), TemptationFnParamsVec,1,0); % Level=1, Refine=0
        ...
        TemptationMatrix_Tii=CreateReturnFnMatrix_ExpAsset_Disc_e(TemptationFn, 0, n_d2, n2long, n_a1,n_a2,n_z,n_e, d2_gridvals, a1prime_grid(a1primeindexesT), a1_gridvals, a2_gridvals, z_gridvals_J(:,:,N_j), e_gridvals_J(:,:,N_j), TemptationFnParamsVec,2,0); % [N_d,N_a1prime,N_a1,N_a2,N_z,N_e]; Level=2, Refine=0
        ...
        TemptationMatrix_ii=CreateReturnFnMatrix_ExpAsset_Disc_e(TemptationFn, 0, n_d2, n2long, n_a1,n_a2,n_z,n_e, d2_gridvals, a1prime_grid(aprimeindexes), a1_gridvals, a2_gridvals, z_gridvals_J(:,:,N_j), e_gridvals_J(:,:,N_j), TemptationFnParamsVec,2,0); % [N_d,N_a1prime,N_a1,N_a2,N_z,N_e]; Level=2, Refine=0
    ...
    elseif vfoptions.lowmemory==1
        ...
        for e_c=1:N_e
            ...
            TemptationMatrix_e=CreateReturnFnMatrix_ExpAsset_Disc_e(TemptationFn, 0,n_d2,n_a1,n_a1,n_a2,n_z,special_n_e, d2_gridvals, a1_gridvals, a1_gridvals, a2_gridvals, z_gridvals_J(:,:,N_j), e_val, TemptationFnParamsVec,1,0); % Level=1, Refine=0
            % Calc the max and it's index
            ...
            midpointT=max(min(maxindexT,n_a1(1)-1),2);
            a1primeindexesT=(midpointT+(midpointT-1)*n2short)+(-n2short-1:1:1+n2short);
            TemptationMatrix_Tii=CreateReturnFnMatrix_ExpAsset_Disc_e(TemptationFn, 0, n_d2, n2long, n_a1,n_a2,n_z,special_n_e, d2_gridvals, a1prime_grid(a1primeindexesT), a1_gridvals, a2_gridvals, z_gridvals_J(:,:,N_j), e_val, TemptationFnParamsVec,2,0); % [N_d,N_a1prime,N_a1,N_a2,N_z]; Level=2, Refine=0
            MostTempting=max(TemptationMatrix_Tii,[],1);
            MostTempting(MostTempting==-Inf)=0; % a state where EVERY choice is infeasible leaves this -Inf: V there must be -Inf (as in the standard solver), not -Inf-(-Inf)=NaN
            ...
            TemptationMatrix_ii=CreateReturnFnMatrix_ExpAsset_Disc_e(TemptationFn, 0, n_d2, n2long, n_a1,n_a2,n_z,special_n_e, d2_gridvals, a1prime_grid(aprimeindexes), a1_gridvals, a2_gridvals, z_gridvals_J(:,:,N_j), e_val, TemptationFnParamsVec,2,0); % [N_d,N_a1prime,N_a1,N_a2,N_z]; Level=2, Refine=0
            Ftemp_ii=ReturnMatrix_ii+TemptationMatrix_ii;
            [Vtempii,maxindexL2]=max(Ftemp_ii,[],1);
    ...
    elseif vfoptions.lowmemory==2
        ...
        for z_c=1:N_z
            ...
            for e_c=1:N_e
                ...
                TemptationMatrix_ze=CreateReturnFnMatrix_ExpAsset_Disc_e(TemptationFn, 0,n_d2,n_a1,n_a1,n_a2,special_n_z,special_n_e, d2_gridvals, a1_gridvals, a1_gridvals, a2_gridvals, z_val, e_val, TemptationFnParamsVec,1,0); % Level=1, Refine=0
                % Calc the max and it's index
                ...
                midpointT=max(min(maxindexT,n_a1(1)-1),2);
                a1primeindexesT=(midpointT+(midpointT-1)*n2short)+(-n2short-1:1:1+n2short);
                TemptationMatrix_Tii=CreateReturnFnMatrix_ExpAsset_Disc_e(TemptationFn, 0, n_d2, n2long, n_a1,n_a2,special_n_z,special_n_e, d2_gridvals, a1prime_grid(a1primeindexesT), a1_gridvals, a2_gridvals, z_val, e_val, TemptationFnParamsVec,2,0); % [N_d,N_a1prime,N_a1,N_a2,N_z]; Level=2, Refine=0
                MostTempting=max(TemptationMatrix_Tii,[],1);
                MostTempting(MostTempting==-Inf)=0; % a state where EVERY choice is infeasible leaves this -Inf: V there must be -Inf (as in the standard solver), not -Inf-(-Inf)=NaN
                ...
                TemptationMatrix_ii=CreateReturnFnMatrix_ExpAsset_Disc_e(TemptationFn, 0, n_d2, n2long, n_a1,n_a2,special_n_z,special_n_e, d2_gridvals, a1prime_grid(aprimeindexes), a1_gridvals, a2_gridvals, z_val, e_val, TemptationFnParamsVec,2,0); % [N_d,N_a1prime,N_a1,N_a2,N_z]; Level=2, Refine=0
                Ftemp_ii=ReturnMatrix_ii+TemptationMatrix_ii;
                [Vtempii,maxindexL2]=max(Ftemp_ii,[],1);
    ...
    if vfoptions.lowmemory==0
        ...
        midpointT=max(min(maxindexT,n_a1(1)-1),2);
        a1primeindexesT=(midpointT+(midpointT-1)*n2short)+(-n2short-1:1:1+n2short);
        TemptationMatrix_Tii=CreateReturnFnMatrix_ExpAsset_Disc_e(TemptationFn, 0, n_d2, n2long, n_a1,n_a2,n_z,n_e, d2_gridvals, a1prime_grid(a1primeindexesT), a1_gridvals, a2_gridvals, z_gridvals_J(:,:,N_j), e_gridvals_J(:,:,N_j), TemptationFnParamsVec,2,0); % [N_d,N_a1prime,N_a1,N_a2,N_e]; Level=2, Refine=0
        MostTempting=max(TemptationMatrix_Tii,[],1);
        MostTempting(MostTempting==-Inf)=0; % a state where EVERY choice is infeasible leaves this -Inf: V there must be -Inf (as in the standard solver), not -Inf-(-Inf)=NaN
        ...
        TemptationMatrix_ii=CreateReturnFnMatrix_ExpAsset_Disc_e(TemptationFn, 0, n_d2, n2long, n_a1,n_a2,n_z,n_e, d2_gridvals, a1prime_grid(a1primeindexesfine), a1_gridvals, a2_gridvals, z_gridvals_J(:,:,N_j), e_gridvals_J(:,:,N_j), TemptationFnParamsVec,2,0); % [N_d,N_a1prime,N_a1,N_a2,N_e]; Level=2, Refine=0
        Ftemp_ii=ReturnMatrix_ii+TemptationMatrix_ii;
        d2a1primea2z=(1:1:N_d2)'+N_d2*(a1primeindexesfine-1)+N_d2*N_a1prime*a2ind+N_d2*N_a1prime*N_a2*zind;
        entireRHS_ii=Ftemp_ii+reshape(DiscountedEVinterp(d2a1primea2z(:)),[N_d2*n2long,N_a1*N_a2,N_z,N_e]);
    ...
    elseif vfoptions.lowmemory==1
        for e_c=1:N_e
            ...
            midpointT=max(min(maxindexT,n_a1(1)-1),2);
            a1primeindexesT=(midpointT+(midpointT-1)*n2short)+(-n2short-1:1:1+n2short);
            TemptationMatrix_Tii=CreateReturnFnMatrix_ExpAsset_Disc_e(TemptationFn, 0, n_d2, n2long, n_a1,n_a2,n_z,special_n_e, d2_gridvals, a1prime_grid(a1primeindexesT), a1_gridvals, a2_gridvals, z_gridvals_J(:,:,N_j), e_val, TemptationFnParamsVec,2,0); % [N_d,N_a1prime,N_a1,N_a2,N_e]; Level=2, Refine=0
            MostTempting=max(TemptationMatrix_Tii,[],1);
            MostTempting(MostTempting==-Inf)=0; % a state where EVERY choice is infeasible leaves this -Inf: V there must be -Inf (as in the standard solver), not -Inf-(-Inf)=NaN
            ...
            TemptationMatrix_ii=CreateReturnFnMatrix_ExpAsset_Disc_e(TemptationFn, 0, n_d2, n2long, n_a1,n_a2,n_z,special_n_e, d2_gridvals, a1prime_grid(a1primeindexesfine), a1_gridvals, a2_gridvals, z_gridvals_J(:,:,N_j), e_val, TemptationFnParamsVec,2,0); % [N_d,N_a1prime,N_a1,N_a2,N_e]; Level=2, Refine=0
            Ftemp_ii=ReturnMatrix_ii+TemptationMatrix_ii;
            d2a1primea2z=(1:1:N_d2)'+N_d2*(a1primeindexesfine-1)+N_d2*N_a1prime*a2ind+N_d2*N_a1prime*N_a2*zind;
            entireRHS_ii=Ftemp_ii+reshape(DiscountedEVinterp(d2a1primea2z(:)),[N_d2*n2long,N_a1*N_a2,N_z]);
            [Vtempii,maxindexL2]=max(entireRHS_ii,[],1);
            V(:,:,e_c,N_j)=shiftdim(Vtempii-MostTempting,1);
            d_ind=rem(maxindexL2-1,N_d2)+1;
    ...
    elseif vfoptions.lowmemory==2
        for z_c=1:N_z
            ...
            for e_c=1:N_e
                ...
                TemptationMatrix=CreateReturnFnMatrix_ExpAsset_Disc_e(TemptationFn, 0,n_d2,n_a1,n_a1,n_a2,special_n_z,special_n_e, d2_gridvals, a1_gridvals, a1_gridvals, a2_gridvals, z_val, e_val, TemptationFnParamsVec,1,0); % Level=1, Refine=0
                entireRHS=ReturnMatrix+TemptationMatrix+DiscountedEV_z; % autofill 3rd dim to N_a1
                ...
                midpointT=max(min(maxindexT,n_a1(1)-1),2);
                a1primeindexesT=(midpointT+(midpointT-1)*n2short)+(-n2short-1:1:1+n2short);
                TemptationMatrix_Tii=CreateReturnFnMatrix_ExpAsset_Disc_e(TemptationFn, 0, n_d2, n2long, n_a1,n_a2,special_n_z,special_n_e, d2_gridvals, a1prime_grid(a1primeindexesT), a1_gridvals, a2_gridvals, z_val, e_val, TemptationFnParamsVec,2,0); % [N_d,N_a1prime,N_a1,N_a2,N_e]; Level=2, Refine=0
                MostTempting=max(TemptationMatrix_Tii,[],1);
                MostTempting(MostTempting==-Inf)=0; % a state where EVERY choice is infeasible leaves this -Inf: V there must be -Inf (as in the standard solver), not -Inf-(-Inf)=NaN
                ...
                TemptationMatrix_ii=CreateReturnFnMatrix_ExpAsset_Disc_e(TemptationFn, 0, n_d2, n2long, n_a1,n_a2,special_n_z,special_n_e, d2_gridvals, a1prime_grid(a1primeindexesfine), a1_gridvals, a2_gridvals, z_val, e_val, TemptationFnParamsVec,2,0); % [N_d,N_a1prime,N_a1,N_a2,N_e]; Level=2, Refine=0
                Ftemp_ii=ReturnMatrix_ii+TemptationMatrix_ii;
                d2a1primea2=(1:1:N_d2)'+N_d2*(a1primeindexesfine-1)+N_d2*N_a1prime*a2ind;
                entireRHS_ii=Ftemp_ii+reshape(DiscountedEVinterp_z(d2a1primea2(:)),[N_d2*n2long,N_a1*N_a2]);
                [Vtempii,maxindexL2]=max(entireRHS_ii,[],1);
                V(:,z_c,e_c,N_j)=shiftdim(Vtempii-MostTempting,1);
                d_ind=rem(maxindexL2-1,N_d2)+1;
...
for reverse_j=1:N_j-1
    ...
    if vfoptions.lowmemory==0
        ...
        TemptationMatrix=CreateReturnFnMatrix_ExpAsset_Disc_e(TemptationFn, 0,n_d2,n_a1,n_a1,n_a2,n_z,n_e, d2_gridvals, a1_gridvals, a1_gridvals, a2_gridvals, z_gridvals_J(:,:,jj), e_gridvals_J(:,:,jj), TemptationFnParamsVec,1,0); % Level=1, Refine=0
        entireRHS=ReturnMatrix+TemptationMatrix+DiscountedEV; % autofill 3rd dim to N_a1
        ...
        midpointT=max(min(maxindexT,n_a1(1)-1),2);
        a1primeindexesT=(midpointT+(midpointT-1)*n2short)+(-n2short-1:1:1+n2short);
        TemptationMatrix_Tii=CreateReturnFnMatrix_ExpAsset_Disc_e(TemptationFn, 0, n_d2, n2long, n_a1,n_a2,n_z,n_e, d2_gridvals, a1prime_grid(a1primeindexesT), a1_gridvals, a2_gridvals, z_gridvals_J(:,:,jj), e_gridvals_J(:,:,jj), TemptationFnParamsVec,2,0); % [N_d,N_a1prime,N_a1,N_a2,N_e]; Level=2, Refine=0
        MostTempting=max(TemptationMatrix_Tii,[],1);
        MostTempting(MostTempting==-Inf)=0; % a state where EVERY choice is infeasible leaves this -Inf: V there must be -Inf (as in the standard solver), not -Inf-(-Inf)=NaN
        ...
        TemptationMatrix_ii=CreateReturnFnMatrix_ExpAsset_Disc_e(TemptationFn, 0, n_d2, n2long, n_a1,n_a2,n_z,n_e, d2_gridvals, a1prime_grid(a1primeindexesfine), a1_gridvals, a2_gridvals, z_gridvals_J(:,:,jj), e_gridvals_J(:,:,jj), TemptationFnParamsVec,2,0); % [N_d,N_a1prime,N_a1,N_a2,N_e]; Level=2, Refine=0
        Ftemp_ii=ReturnMatrix_ii+TemptationMatrix_ii;
        d2a1primea2z=(1:1:N_d2)'+N_d2*(a1primeindexesfine-1)+N_d2*N_a1prime*a2ind+N_d2*N_a1prime*N_a2*zind;
        entireRHS_ii=Ftemp_ii+reshape(DiscountedEVinterp(d2a1primea2z(:)),[N_d2*n2long,N_a1*N_a2,N_z,N_e]);
        [Vtempii,maxindexL2]=max(entireRHS_ii,[],1);
    ...
    elseif vfoptions.lowmemory==1
        for e_c=1:N_e
            ...
            TemptationMatrix_e=CreateReturnFnMatrix_ExpAsset_Disc_e(TemptationFn, 0,n_d2,n_a1,n_a1,n_a2,n_z,special_n_e, d2_gridvals, a1_gridvals, a1_gridvals, a2_gridvals, z_gridvals_J(:,:,jj), e_val, TemptationFnParamsVec,1,0); % Level=1, Refine=0
            entireRHS_e=ReturnMatrix_e+TemptationMatrix_e+DiscountedEV; % autofill 3rd dim to N_a1
            ...
            midpointT=max(min(maxindexT,n_a1(1)-1),2);
            a1primeindexesT=(midpointT+(midpointT-1)*n2short)+(-n2short-1:1:1+n2short);
            TemptationMatrix_Tii=CreateReturnFnMatrix_ExpAsset_Disc_e(TemptationFn, 0, n_d2, n2long, n_a1,n_a2,n_z,special_n_e, d2_gridvals, a1prime_grid(a1primeindexesT), a1_gridvals, a2_gridvals, z_gridvals_J(:,:,jj), e_val, TemptationFnParamsVec,2,0); % [N_d,N_a1prime,N_a1,N_a2,N_e]; Level=2, Refine=0
            MostTempting=max(TemptationMatrix_Tii,[],1);
            MostTempting(MostTempting==-Inf)=0; % a state where EVERY choice is infeasible leaves this -Inf: V there must be -Inf (as in the standard solver), not -Inf-(-Inf)=NaN
            ...
            TemptationMatrix_ii=CreateReturnFnMatrix_ExpAsset_Disc_e(TemptationFn, 0, n_d2, n2long, n_a1,n_a2,n_z,special_n_e, d2_gridvals, a1prime_grid(a1primeindexesfine), a1_gridvals, a2_gridvals, z_gridvals_J(:,:,jj), e_val, TemptationFnParamsVec,2,0); % [N_d,N_a1prime,N_a1,N_a2,N_e]; Level=2, Refine=0
            Ftemp_ii=ReturnMatrix_ii+TemptationMatrix_ii;
            d2a1primea2z=(1:1:N_d2)'+N_d2*(a1primeindexesfine-1)+N_d2*N_a1prime*a2ind+N_d2*N_a1prime*N_a2*zind;
            entireRHS_ii=Ftemp_ii+reshape(DiscountedEVinterp(d2a1primea2z(:)),[N_d2*n2long,N_a1*N_a2,N_z]);
            [Vtempii,maxindexL2]=max(entireRHS_ii,[],1);
