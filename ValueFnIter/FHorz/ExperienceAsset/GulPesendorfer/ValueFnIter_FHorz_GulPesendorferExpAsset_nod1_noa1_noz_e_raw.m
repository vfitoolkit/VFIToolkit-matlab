function [V,Policy]=ValueFnIter_FHorz_GulPesendorferExpAsset_nod1_noa1_noz_e_raw(n_d2,n_a2,n_e,N_j, d2_gridvals, a2_grid, e_gridvals_J, pi_e_J, ReturnFn, TemptationFn, aprimeFn, Parameters, DiscountFactorParamNames, ReturnFnParamNames, TemptationFnParamNames, aprimeFnParamNames, vfoptions)
...
if ~isfield(vfoptions,'V_Jplus1')
    if vfoptions.lowmemory==0
        ...
        TemptationMatrix=CreateReturnFnMatrix_Case2_Disc(TemptationFn,n_d2, n_a2, n_e, d2_gridvals, a2_gridvals, e_gridvals_J(:,:,N_j), TemptationFnParamsVec); % with only the experience asset, can just use Case2 command
    ...
    elseif vfoptions.lowmemory==1
        for e_c=1:N_e
            ...
            TemptationMatrix_e=CreateReturnFnMatrix_Case2_Disc(TemptationFn,n_d2, n_a2, special_n_e, d2_gridvals, a2_gridvals, e_gridvals_J(e_c,:,N_j), TemptationFnParamsVec); % with only the experience asset, can just use Case2 command
    ...
    if vfoptions.lowmemory==0
        ...
        entireRHS=ReturnMatrix+TemptationMatrix+DiscountFactorParamsVec*EV; % should autofill e dimension
...
for reverse_j=1:N_j-1
    ...
    if vfoptions.lowmemory==0
        ...
        TemptationMatrix=CreateReturnFnMatrix_Case2_Disc(TemptationFn,n_d2, n_a2, n_e, d2_gridvals, a2_gridvals, e_gridvals_J(:,:,jj), TemptationFnParamsVec); % with only the experience asset, can just use Case2 command
        MostTempting=max(TemptationMatrix,[],1);
        MostTempting(MostTempting==-Inf)=0; % a state where EVERY choice is infeasible leaves this -Inf: V there must be -Inf (as in the standard solver), not -Inf-(-Inf)=NaN
    ...
    elseif vfoptions.lowmemory==1
        for e_c=1:N_e
            ...
            TemptationMatrix_e=CreateReturnFnMatrix_Case2_Disc(TemptationFn,n_d2, n_a2, special_n_e, d2_gridvals, a2_gridvals, e_gridvals_J(e_c,:,jj), TemptationFnParamsVec); % with only the experience asset, can just use Case2 command
            MostTempting_e=max(TemptationMatrix_e,[],1);
            MostTempting_e(MostTempting_e==-Inf)=0; % a state where EVERY choice is infeasible leaves this -Inf: V there must be -Inf (as in the standard solver), not -Inf-(-Inf)=NaN
