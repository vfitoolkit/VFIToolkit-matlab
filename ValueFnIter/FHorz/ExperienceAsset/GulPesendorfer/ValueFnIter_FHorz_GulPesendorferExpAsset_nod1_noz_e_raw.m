function [V,Policy]=ValueFnIter_FHorz_GulPesendorferExpAsset_nod1_noz_e_raw(n_d2,n_a1,n_a2,n_e, N_j, d2_gridvals, a1_gridvals, a2_grid, e_gridvals_J, pi_e_J, ReturnFn, TemptationFn, aprimeFn, Parameters, DiscountFactorParamNames, ReturnFnParamNames, TemptationFnParamNames, aprimeFnParamNames, vfoptions)
...
if ~isfield(vfoptions,'V_Jplus1')
    if vfoptions.lowmemory==0
        ...
        TemptationMatrix=CreateReturnFnMatrix_ExpAsset_Disc(TemptationFn, 0, n_d2,n_a1,n_a1,n_a2,n_e, d2_gridvals, a1_gridvals, a1_gridvals, a2_gridvals, e_gridvals_J(:,:,N_j), TemptationFnParamsVec,0,0); % Level=0, Refine=0
    ...
    elseif vfoptions.lowmemory==1
        for e_c=1:N_e
            ...
            TemptationMatrix=CreateReturnFnMatrix_ExpAsset_Disc(TemptationFn, 0, n_d2,n_a1,n_a1,n_a2,special_n_e, d2_gridvals, a1_gridvals, a1_gridvals, a2_gridvals, e_val, TemptationFnParamsVec,0,0); % Level=0, Refine=0
            MostTempting=max(TemptationMatrix,[],1);
            MostTempting(MostTempting==-Inf)=0; % a state where EVERY choice is infeasible leaves this -Inf: V there must be -Inf (as in the standard solver), not -Inf-(-Inf)=NaN
            entireRHS=ReturnMatrix+TemptationMatrix;
            %Calc the max and it's index
    ...
    if vfoptions.lowmemory==0
        ...
        entireRHS=ReturnMatrix+TemptationMatrix+DiscountFactorParamsVec*repelem(EV,1,N_a1,1); % should autofill e dimension
    ...
    elseif vfoptions.lowmemory==1
        for e_c=1:N_e
            ...
            TemptationMatrix_e=CreateReturnFnMatrix_ExpAsset_Disc(TemptationFn, 0, n_d2,n_a1,n_a1,n_a2,special_n_e, d2_gridvals, a1_gridvals, a1_gridvals, a2_gridvals, e_val, TemptationFnParamsVec,0,0); % Level=0, Refine=0
            ...
            entireRHS=ReturnMatrix_e+TemptationMatrix_e+DiscountFactorParamsVec*repelem(EV,1,N_a1,1); % should autofill e dimension
...
for reverse_j=1:N_j-1
    ...
    if vfoptions.lowmemory==0
        ...
        TemptationMatrix=CreateReturnFnMatrix_ExpAsset_Disc(TemptationFn, 0, n_d2,n_a1,n_a1,n_a2,n_e, d2_gridvals, a1_gridvals, a1_gridvals, a2_gridvals, e_gridvals_J(:,:,jj), TemptationFnParamsVec,0,0); % Level=0, Refine=0
        MostTempting=max(TemptationMatrix,[],1);
        MostTempting(MostTempting==-Inf)=0; % a state where EVERY choice is infeasible leaves this -Inf: V there must be -Inf (as in the standard solver), not -Inf-(-Inf)=NaN
