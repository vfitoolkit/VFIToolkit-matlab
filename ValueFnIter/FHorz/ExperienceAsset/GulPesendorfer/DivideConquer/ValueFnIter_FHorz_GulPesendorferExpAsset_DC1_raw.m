function [V,Policy]=ValueFnIter_FHorz_GulPesendorferExpAsset_DC1_raw(n_d1,n_d2,n_a1,n_a2,n_z,N_j, d_gridvals, d2_gridvals, a1_gridvals, a2_grid, z_gridvals_J, pi_z_J, ReturnFn, TemptationFn, aprimeFn, Parameters, DiscountFactorParamNames, ReturnFnParamNames, TemptationFnParamNames, aprimeFnParamNames, vfoptions)
...
if ~isfield(vfoptions,'V_Jplus1')
    if vfoptions.lowmemory==0
        ...
        TemptationMatrix_ii=CreateReturnFnMatrix_ExpAsset_Disc(TemptationFn, n_d1,n_d2,n_a1,vfoptions.level1n,n_a2,n_z, d_gridvals, a1_gridvals, a1_gridvals(level1ii), a2_gridvals, z_gridvals_J(:,:,N_j), TemptationFnParamsVec,1,0); % Level=1, Refine=0
        ...
        for ii=1:(vfoptions.level1n-1)
            ...
            TemptationMatrix_full=CreateReturnFnMatrix_ExpAsset_Disc(TemptationFn, n_d1,n_d2,n_a1,level1iidiff(ii),n_a2,n_z, d_gridvals, a1_gridvals, a1_gridvals(level1ii(ii)+1:level1ii(ii+1)-1), a2_gridvals, z_gridvals_J(:,:,N_j), TemptationFnParamsVec,1,0); % Level=1, Refine=0
            ...
            if maxgap(ii)>0
                ...
                TemptationMatrix_ii=CreateReturnFnMatrix_ExpAsset_Disc(TemptationFn, n_d1,n_d2,maxgap(ii)+1,level1iidiff(ii),n_a2,n_z, d_gridvals, a1_gridvals(a1primeindexes), a1_gridvals(level1ii(ii)+1:level1ii(ii+1)-1), a2_gridvals, z_gridvals_J(:,:,N_j), TemptationFnParamsVec,2,0); % Level=2, Refine=0
                entireRHS_ii=ReturnMatrix_ii+TemptationMatrix_ii;
                ...
                TemptationMatrix_ii=CreateReturnFnMatrix_ExpAsset_Disc(TemptationFn, n_d1,n_d2,1,level1iidiff(ii),n_a2,n_z, d_gridvals, a1_gridvals(loweredge), a1_gridvals(level1ii(ii)+1:level1ii(ii+1)-1), a2_gridvals, z_gridvals_J(:,:,N_j), TemptationFnParamsVec,2,0); % Level=2, Refine=0
                entireRHS_ii=ReturnMatrix_ii+TemptationMatrix_ii;
                [Vtempii,maxindex]=max(entireRHS_ii,[],1);
                V(curraindex,:,N_j)=shiftdim(Vtempii-MostTempting_ii,1);
                % maxindex does not need reworking, as with expasset there is no a2prime
    ...
    elseif vfoptions.lowmemory==1
         ...
         for z_c=1:N_z
            ...
            TemptationMatrix_ii_z=CreateReturnFnMatrix_ExpAsset_Disc(TemptationFn, n_d1,n_d2,n_a1,vfoptions.level1n,n_a2,special_n_z, d_gridvals, a1_gridvals, a1_gridvals(level1ii), a2_gridvals, z_val, TemptationFnParamsVec,1,0); % Level=1, Refine=0
            ...
            for ii=1:(vfoptions.level1n-1)
                curraindex=repmat((level1ii(ii)+1:1:level1ii(ii+1)-1)',N_a2,1)+N_a1*repelem((0:1:N_a2-1)',level1iidiff(ii),1);
                % Most-tempting term over the FULL (d,aprime) grid for these a (never just the window)
                TemptationMatrix_full=CreateReturnFnMatrix_ExpAsset_Disc(TemptationFn, n_d1,n_d2,n_a1,level1iidiff(ii),n_a2,special_n_z, d_gridvals, a1_gridvals, a1_gridvals(level1ii(ii)+1:level1ii(ii+1)-1), a2_gridvals, z_val, TemptationFnParamsVec,1,0); % Level=1, Refine=0
                ...
                if maxgap(ii)>0
                    ...
                    TemptationMatrix_ii=CreateReturnFnMatrix_ExpAsset_Disc(TemptationFn, n_d1,n_d2,maxgap(ii)+1,level1iidiff(ii),n_a2,special_n_z, d_gridvals, a1_gridvals(a1primeindexes), a1_gridvals(level1ii(ii)+1:level1ii(ii+1)-1), a2_gridvals, z_val, TemptationFnParamsVec,2,0); % Level=2, Refine=0
                    entireRHS_ii=ReturnMatrix_ii+TemptationMatrix_ii;
                    [Vtempii,maxindex]=max(entireRHS_ii,[],1);
                    ...
                    TemptationMatrix_ii=CreateReturnFnMatrix_ExpAsset_Disc(TemptationFn, n_d1,n_d2,1,level1iidiff(ii),n_a2,special_n_z, d_gridvals, a1_gridvals(loweredge), a1_gridvals(level1ii(ii)+1:level1ii(ii+1)-1), a2_gridvals, z_val, TemptationFnParamsVec,2,0); % Level=2, Refine=0
                    entireRHS_ii=ReturnMatrix_ii+TemptationMatrix_ii;
                    [Vtempii,maxindex]=max(entireRHS_ii,[],1);
                    V(curraindex,z_c,N_j)=shiftdim(Vtempii-MostTempting_ii,1);
                    % maxindex does not need reworking, as with expasset there is no a2prime
    ...
    if vfoptions.lowmemory==0
        ...
        for ii=1:(vfoptions.level1n-1)
            ...
            if maxgap(ii)>0
                ...
                TemptationMatrix_ii=CreateReturnFnMatrix_ExpAsset_Disc(TemptationFn, n_d1,n_d2,maxgap(ii)+1,level1iidiff(ii),n_a2,n_z, d_gridvals, a1_gridvals(a1primeindexes), a1_gridvals(level1ii(ii)+1:level1ii(ii+1)-1), a2_gridvals, z_gridvals_J(:,:,N_j), TemptationFnParamsVec,3,0); % Level=2, Refine=0
                ...
                TemptationMatrix_ii=CreateReturnFnMatrix_ExpAsset_Disc(TemptationFn, n_d1,n_d2,1,level1iidiff(ii),n_a2,n_z, d_gridvals, a1_gridvals(loweredge), a1_gridvals(level1ii(ii)+1:level1ii(ii+1)-1), a2_gridvals, z_gridvals_J(:,:,N_j), TemptationFnParamsVec,3,0); % Level=2, Refine=0
        ...
        for z_c=1:N_z
            ...
            for ii=1:(vfoptions.level1n-1)
                ...
                if maxgap(ii)>0
                    ...
                    TemptationMatrix_ii_z=CreateReturnFnMatrix_ExpAsset_Disc(TemptationFn, n_d1,n_d2,maxgap(ii)+1,level1iidiff(ii),n_a2,special_n_z, d_gridvals, a1_gridvals(a1primeindexes), a1_gridvals(level1ii(ii)+1:level1ii(ii+1)-1), a2_gridvals, z_val, TemptationFnParamsVec,3,0); % Level=2, Refine=0
                    ...
                    TemptationMatrix_ii_z=CreateReturnFnMatrix_ExpAsset_Disc(TemptationFn, n_d1,n_d2,1,level1iidiff(ii),n_a2,special_n_z, d_gridvals, a1_gridvals(loweredge), a1_gridvals(level1ii(ii)+1:level1ii(ii+1)-1), a2_gridvals, z_val, TemptationFnParamsVec,3,0); % Level=2, Refine=0
                    d2aprime=d2ind+N_d2*(loweredge-1)+N_d2*N_a1*a2ind; % [N_d,1,1,N_a2]; linear index into DiscountedEV_z [N_d2,N_a1,1,N_a2]
                    entireRHS_ii_z=reshape(ReturnMatrix_ii_z+TemptationMatrix_ii_z+DiscountedEV_z(d2aprime),[N_d,level1iidiff(ii)*N_a2]);
                    [Vtempii,maxindex]=max(entireRHS_ii_z,[],1);
                    V(curraindex,z_c,N_j)=shiftdim(Vtempii-MostTempting_ii,1);
                    % maxindex does not need reworking, as with expasset there is no a2prime
                    %  the a1prime is relative to loweredge(allind), need to 'add' the loweredge
...
for reverse_j=1:N_j-1
    ...
    if vfoptions.lowmemory==0
        ...
        TemptationMatrix_ii=CreateReturnFnMatrix_ExpAsset_Disc(TemptationFn, n_d1,n_d2,n_a1,vfoptions.level1n,n_a2,n_z, d_gridvals, a1_gridvals, a1_gridvals(level1ii), a2_gridvals, z_gridvals_J(:,:,jj), TemptationFnParamsVec,1,0); % Level=1, Refine=0
        MostTempting1=max(reshape(TemptationMatrix_ii,[N_d1*N_d2*N_a1,vfoptions.level1n*N_a2,N_z]),[],1); % full (d,aprime) grid at the level-1 stations
        MostTempting1(MostTempting1==-Inf)=0; % a state where EVERY choice is infeasible leaves this -Inf: V there must be -Inf (as in the standard solver), not -Inf-(-Inf)=NaN
        ...
        for ii=1:(vfoptions.level1n-1)
            curraindex=repmat((level1ii(ii)+1:1:level1ii(ii+1)-1)',N_a2,1)+N_a1*repelem((0:1:N_a2-1)',level1iidiff(ii),1);
            % Most-tempting term over the FULL (d,aprime) grid for these a (never just the window)
            TemptationMatrix_full=CreateReturnFnMatrix_ExpAsset_Disc(TemptationFn, n_d1,n_d2,n_a1,level1iidiff(ii),n_a2,n_z, d_gridvals, a1_gridvals, a1_gridvals(level1ii(ii)+1:level1ii(ii+1)-1), a2_gridvals, z_gridvals_J(:,:,jj), TemptationFnParamsVec,1,0); % Level=1, Refine=0
            MostTempting_ii=max(reshape(TemptationMatrix_full,[N_d1*N_d2*N_a1,length(curraindex),N_z]),[],1);
            MostTempting_ii(MostTempting_ii==-Inf)=0; % a state where EVERY choice is infeasible leaves this -Inf: V there must be -Inf (as in the standard solver), not -Inf-(-Inf)=NaN
            if maxgap(ii)>0
                loweredge=min(maxindex1(:,1,ii,:,:),N_a1-maxgap(ii)); % maxindex1(ii,:), but avoid going off top of grid when we add maxgap(ii) points
                ...
                TemptationMatrix_ii=CreateReturnFnMatrix_ExpAsset_Disc(TemptationFn, n_d1,n_d2,maxgap(ii)+1,level1iidiff(ii),n_a2,n_z, d_gridvals, a1_gridvals(a1primeindexes), a1_gridvals(level1ii(ii)+1:level1ii(ii+1)-1), a2_gridvals, z_gridvals_J(:,:,jj), TemptationFnParamsVec,3,0); % Level=2, Refine=0
                d2aprimez=d2ind+N_d2*(a1primeindexes-1)+N_d2*N_a1*a2ind+N_d2*N_a*zind; % [N_d,maxgap+1,1,N_a2,N_z]; linear index into DiscountedEV [N_d2,N_a1,1,N_a2,N_z]
                entireRHS_ii=reshape(ReturnMatrix_ii+TemptationMatrix_ii+DiscountedEV(d2aprimez),[N_d*(maxgap(ii)+1),level1iidiff(ii)*N_a2,N_z]);
                [Vtempii,maxindex]=max(entireRHS_ii,[],1);
                ...
                TemptationMatrix_ii=CreateReturnFnMatrix_ExpAsset_Disc(TemptationFn, n_d1,n_d2,1,level1iidiff(ii),n_a2,n_z, d_gridvals, a1_gridvals(loweredge), a1_gridvals(level1ii(ii)+1:level1ii(ii+1)-1), a2_gridvals, z_gridvals_J(:,:,jj), TemptationFnParamsVec,3,0); % Level=2, Refine=0
                d2aprimez=d2ind+N_d2*(loweredge-1)+N_d2*N_a1*a2ind+N_d2*N_a*zind; % [N_d,1,1,N_a2,N_z]; linear index into DiscountedEV [N_d2,N_a1,1,N_a2,N_z]
                entireRHS_ii=reshape(ReturnMatrix_ii+TemptationMatrix_ii+DiscountedEV(d2aprimez),[N_d,level1iidiff(ii)*N_a2,N_z]);
                [Vtempii,maxindex]=max(entireRHS_ii,[],1);
                V(curraindex,:,jj)=shiftdim(Vtempii-MostTempting_ii,1);
