function [V, Policy]=ValueFnIter_Case1_FHorz_RiskyAsset_EpsteinZin(n_d,n_a1,n_a2,n_z,n_u,N_j,d_grid, a1_grid, a2_grid, z_grid, u_grid, pi_z, pi_u, ReturnFn, aprimeFn, Parameters, DiscountFactorParamNames, ReturnFnParamNames, aprimeFnParamNames, vfoptions)
...
if vfoptions.parallel==2
    if N_a1==0
        if isfield(vfoptions,'n_e')
            ...
            if N_z==0
                [VKron,PolicyKron]=ValueFnIter_Case1_FHorz_RiskyAsset_EpsteinZin_noz_e_raw(n_d, n_a2, vfoptions.n_e, n_u, N_j, d_grid, a2_grid, e_grid, u_grid, pi_e, pi_u, ReturnFn, aprimeFn, Parameters, DiscountFactorParamNames, ReturnFnParamNames, aprimeFnParamNames, vfoptions, sj, warmglow, ezc1,ezc2,ezc3,ezc4,ezc5,ezc6,ezc7,ezc8);
            else
                [VKron,PolicyKron]=ValueFnIter_Case1_FHorz_RiskyAsset_EpsteinZin_e_raw(n_d, n_a2, n_z, vfoptions.n_e, n_u, N_j, d_grid, a2_grid, z_grid, e_grid, u_grid, pi_z, pi_e, pi_u, ReturnFn, aprimeFn, Parameters, DiscountFactorParamNames, ReturnFnParamNames, aprimeFnParamNames, vfoptions, sj, warmglow, ezc1,ezc2,ezc3,ezc4,ezc5,ezc6,ezc7,ezc8);
            end
        else
            if N_z==0
                error('Cannot use Epstein-Zin preferences without any shocks (what is the point?); you have n_z=0 and no e variables')
            else
                [VKron, PolicyKron]=ValueFnIter_Case1_FHorz_RiskyAsset_EpsteinZin_raw(n_d,n_a2,n_z,n_u, N_j, d_grid, a2_grid, z_grid, u_grid, pi_z, pi_u, ReturnFn, aprimeFn, Parameters, DiscountFactorParamNames, ReturnFnParamNames, aprimeFnParamNames, vfoptions, sj, warmglow, ezc1,ezc2,ezc3,ezc4,ezc5,ezc6,ezc7,ezc8);
            end
        end
    else
        error('Riskyasset with Epstein-Zin only supports a single asset')
    end
else
