function varargout=ValueFnIter_FHorz_QuasiHyperbolicN_GI1_nod_noz_e_raw(n_a, n_e, N_j, a_grid, e_gridvals_J, pi_e_J, ReturnFn, Parameters, DiscountFactorParamNames, ReturnFnParamNames, vfoptions)
% Naive quasi-hyperbolic discounting variant of ValueFnIter_FHorz_GI_nod_noz_e_raw.
...
if ~isfield(vfoptions,'V_Jplus1')
    ...
    if vfoptions.lowmemory==0
        ...
        ReturnMatrix_ii_V=CreateReturnFnMatrix_Case1_Disc_DC1_nod_Par2(ReturnFn,n_e,aprime_grid(aprimeindexes_V),a_grid,e_gridvals_J(:,:,N_j),ReturnFnParamsVec,2);
        entireRHS_ii_V=ReturnMatrix_ii_V+beta*reshape(EVinterp(aprimeindexes_V(:)),[n2long,N_a,N_e]);
        ...
        ReturnMatrix_ii_Vt=CreateReturnFnMatrix_Case1_Disc_DC1_nod_Par2(ReturnFn,n_e,aprime_grid(aprimeindexes_Vt),a_grid,e_gridvals_J(:,:,N_j),ReturnFnParamsVec,2);
        entireRHS_ii_Vt=ReturnMatrix_ii_Vt+beta0beta*reshape(EVinterp(aprimeindexes_Vt(:)),[n2long,N_a,N_e]);
    ...
    elseif vfoptions.lowmemory==1
        for e_c=1:N_e
            ...
            midpoint_V=max(min(maxindex_V,n_a-1),2);
            aprimeindexes_V=(midpoint_V+(midpoint_V-1)*n2short)+(-n2short-1:1:1+n2short)';
            ReturnMatrix_ii_V_e=CreateReturnFnMatrix_Case1_Disc_DC1_nod_Par2(ReturnFn,special_n_e,aprime_grid(aprimeindexes_V),a_grid,e_val,ReturnFnParamsVec,2);
            entireRHS_ii_V_e=ReturnMatrix_ii_V_e+beta*reshape(EVinterp(aprimeindexes_V(:)),[n2long,N_a]);
            ...
            midpoint_Vt=max(min(maxindex_Vt,n_a-1),2);
            aprimeindexes_Vt=(midpoint_Vt+(midpoint_Vt-1)*n2short)+(-n2short-1:1:1+n2short)';
            ReturnMatrix_ii_Vt_e=CreateReturnFnMatrix_Case1_Disc_DC1_nod_Par2(ReturnFn,special_n_e,aprime_grid(aprimeindexes_Vt),a_grid,e_val,ReturnFnParamsVec,2);
            entireRHS_ii_Vt_e=ReturnMatrix_ii_Vt_e+beta0beta*reshape(EVinterp(aprimeindexes_Vt(:)),[n2long,N_a]);
...
for reverse_j=1:N_j-1
    ...
    if vfoptions.lowmemory==0
        ...
        midpoint_V=max(min(maxindex_V,n_a-1),2);
        aprimeindexes_V=(midpoint_V+(midpoint_V-1)*n2short)+(-n2short-1:1:1+n2short)';
        ReturnMatrix_ii_V=CreateReturnFnMatrix_Case1_Disc_DC1_nod_Par2(ReturnFn,n_e,aprime_grid(aprimeindexes_V),a_grid,e_gridvals_J(:,:,jj),ReturnFnParamsVec,2);
        entireRHS_ii_V=ReturnMatrix_ii_V+beta*reshape(EVinterp(aprimeindexes_V(:)),[n2long,N_a,N_e]);
        [Vtempii,~]=max(entireRHS_ii_V,[],1);
        ...
        midpoint_Vt=max(min(maxindex_Vt,n_a-1),2);
        aprimeindexes_Vt=(midpoint_Vt+(midpoint_Vt-1)*n2short)+(-n2short-1:1:1+n2short)';
        ReturnMatrix_ii_Vt=CreateReturnFnMatrix_Case1_Disc_DC1_nod_Par2(ReturnFn,n_e,aprime_grid(aprimeindexes_Vt),a_grid,e_gridvals_J(:,:,jj),ReturnFnParamsVec,2);
        entireRHS_ii_Vt=ReturnMatrix_ii_Vt+beta0beta*reshape(EVinterp(aprimeindexes_Vt(:)),[n2long,N_a,N_e]);
        [Vtempii,maxindexL2]=max(entireRHS_ii_Vt,[],1);
