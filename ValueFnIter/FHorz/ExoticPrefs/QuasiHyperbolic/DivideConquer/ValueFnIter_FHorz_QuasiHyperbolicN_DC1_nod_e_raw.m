...
if ~isfield(vfoptions,'V_Jplus1')
    ...
    if vfoptions.lowmemory==0
        ...
        V(level1ii,:,:,N_j)=shiftdim(Vtempii,1);
        maxgap_V=max(max(maxindex1_V(1,2:end,:,:)-maxindex1_V(1,1:end-1,:,:),[],4),[],3);
        for ii=1:(vfoptions.level1n-1)
            curraindex=level1ii(ii)+1:1:level1ii(ii+1)-1;
            if maxgap_V(ii)>0
                loweredge=min(maxindex1_V(1,ii,:,:),n_a-maxgap_V(ii));
                aprimeindexes=loweredge+(0:1:maxgap_V(ii))';
                ReturnMatrix_ii_V=CreateReturnFnMatrix_Case1_Disc_DC1_nod_Par2e(ReturnFn, n_z, n_e, a_grid(aprimeindexes), a_grid(curraindex), z_gridvals_J(:,:,N_j), e_gridvals_J(:,:,N_j), ReturnFnParamsVec,2);
                aprimez=aprimeindexes+N_a*zind;
                entireRHS_ii_V=ReturnMatrix_ii_V+beta*reshape(EV(aprimez),[maxgap_V(ii)+1,1,N_z,N_e]);
                [Vtempii,~]=max(entireRHS_ii_V,[],1);
                V(curraindex,:,:,N_j)=shiftdim(Vtempii,1);
            else
                loweredge=maxindex1_V(1,ii,:,:);
                ReturnMatrix_ii_V=CreateReturnFnMatrix_Case1_Disc_DC1_nod_Par2e(ReturnFn, n_z, n_e, reshape(a_grid(loweredge),loweredgesize), a_grid(curraindex), z_gridvals_J(:,:,N_j), e_gridvals_J(:,:,N_j), ReturnFnParamsVec,2);
                aprimez=loweredge+N_a*zind;
                entireRHS_ii_V=ReturnMatrix_ii_V+beta*reshape(EV(aprimez),[1,1,N_z,N_e]);
        ...
        maxgap_Vt=max(max(maxindex1_Vt(1,2:end,:,:)-maxindex1_Vt(1,1:end-1,:,:),[],4),[],3);
        for ii=1:(vfoptions.level1n-1)
            curraindex=level1ii(ii)+1:1:level1ii(ii+1)-1;
            if maxgap_Vt(ii)>0
                loweredge=min(maxindex1_Vt(1,ii,:,:),n_a-maxgap_Vt(ii));
                aprimeindexes=loweredge+(0:1:maxgap_Vt(ii))';
                ReturnMatrix_ii_Vt=CreateReturnFnMatrix_Case1_Disc_DC1_nod_Par2e(ReturnFn, n_z, n_e, a_grid(aprimeindexes), a_grid(curraindex), z_gridvals_J(:,:,N_j), e_gridvals_J(:,:,N_j), ReturnFnParamsVec,2);
                aprimez=aprimeindexes+N_a*zind;
                entireRHS_ii_Vt=ReturnMatrix_ii_Vt+beta0beta*reshape(EV(aprimez),[maxgap_Vt(ii)+1,1,N_z,N_e]);
                [Vtempii,maxindex]=max(entireRHS_ii_Vt,[],1);
                Vtilde(curraindex,:,:,N_j)=shiftdim(Vtempii,1);
                Policy(curraindex,:,:,N_j)=shiftdim(maxindex+loweredge-1,1);
            else
                loweredge=maxindex1_Vt(1,ii,:,:);
                ReturnMatrix_ii_Vt=CreateReturnFnMatrix_Case1_Disc_DC1_nod_Par2e(ReturnFn, n_z, n_e, reshape(a_grid(loweredge),loweredgesize), a_grid(curraindex), z_gridvals_J(:,:,N_j), e_gridvals_J(:,:,N_j), ReturnFnParamsVec,2);
                aprimez=loweredge+N_a*zind;
                entireRHS_ii_Vt=ReturnMatrix_ii_Vt+beta0beta*reshape(EV(aprimez),[1,1,N_z,N_e]);
        ...
        for e_c=1:N_e
            ...
            for ii=1:(vfoptions.level1n-1)
                ...
                if maxgap_V(ii)>0
                    loweredge=min(maxindex1_V(1,ii,:),n_a-maxgap_V(ii));
                    aprimeindexes=loweredge+(0:1:maxgap_V(ii))';
                    ReturnMatrix_ii_V=CreateReturnFnMatrix_Case1_Disc_DC1_nod_Par2e(ReturnFn, n_z, special_n_e, a_grid(aprimeindexes), a_grid(curraindex), z_gridvals_J(:,:,N_j), e_val, ReturnFnParamsVec,2);
                    aprimez=aprimeindexes+N_a*zind;
                    ...
                    loweredge=maxindex1_V(1,ii,:);
                    ReturnMatrix_ii_V=CreateReturnFnMatrix_Case1_Disc_DC1_nod_Par2e(ReturnFn, n_z, special_n_e, reshape(a_grid(loweredge),loweredgesize), a_grid(curraindex), z_gridvals_J(:,:,N_j), e_val, ReturnFnParamsVec,2);
                    aprimez=loweredge+N_a*zind;
                    entireRHS_ii_V=ReturnMatrix_ii_V+beta*reshape(EV(aprimez),[1,1,N_z]);
                    V(curraindex,:,e_c,N_j)=shiftdim(entireRHS_ii_V,1);
                end
            end
            % --- Vtilde search (beta0beta) ---
            entireRHS_ii_Vt=ReturnMatrix_ii+beta0beta*EV;
            ...
            Policy(level1ii,:,e_c,N_j)=shiftdim(maxindex1_Vt,1);
            ...
            for ii=1:(vfoptions.level1n-1)
                ...
                if maxgap_Vt(ii)>0
                    loweredge=min(maxindex1_Vt(1,ii,:),n_a-maxgap_Vt(ii));
                    aprimeindexes=loweredge+(0:1:maxgap_Vt(ii))';
                    ReturnMatrix_ii_Vt=CreateReturnFnMatrix_Case1_Disc_DC1_nod_Par2e(ReturnFn, n_z, special_n_e, a_grid(aprimeindexes), a_grid(curraindex), z_gridvals_J(:,:,N_j), e_val, ReturnFnParamsVec,2);
                    aprimez=aprimeindexes+N_a*zind;
                    ...
                    loweredge=maxindex1_Vt(1,ii,:);
                    ReturnMatrix_ii_Vt=CreateReturnFnMatrix_Case1_Disc_DC1_nod_Par2e(ReturnFn, n_z, special_n_e, reshape(a_grid(loweredge),loweredgesize), a_grid(curraindex), z_gridvals_J(:,:,N_j), e_val, ReturnFnParamsVec,2);
                    aprimez=loweredge+N_a*zind;
                    entireRHS_ii_Vt=ReturnMatrix_ii_Vt+beta0beta*reshape(EV(aprimez),[1,1,N_z]);
                    Vtilde(curraindex,:,e_c,N_j)=shiftdim(entireRHS_ii_Vt,1);
                    Policy(curraindex,:,e_c,N_j)=repelem(shiftdim(loweredge,1),level1iidiff(ii),1);
                end
            end
        end
    ...
    elseif vfoptions.lowmemory==2
        for z_c=1:N_z
            ...
            for e_c=1:N_e
                ...
                ReturnMatrix_ii=CreateReturnFnMatrix_Case1_Disc_DC1_nod_Par2e(ReturnFn, special_n_z, special_n_e, a_grid, a_grid(level1ii), z_val, e_val, ReturnFnParamsVec,1);
                % --- V search (beta) ---
                entireRHS_ii_V=ReturnMatrix_ii+beta*EV_z;
                [Vtempii,maxindex1_V]=max(entireRHS_ii_V,[],1);
                V(level1ii,z_c,e_c,N_j)=shiftdim(Vtempii,1);
                ...
                for ii=1:(vfoptions.level1n-1)
                    ...
                    if maxgap_V(ii)>0
                        loweredge=min(maxindex1_V(1,ii),n_a-maxgap_V(ii));
                        aprimeindexes=loweredge+(0:1:maxgap_V(ii))';
                        ReturnMatrix_ii_V=CreateReturnFnMatrix_Case1_Disc_DC1_nod_Par2e(ReturnFn, special_n_z, special_n_e, a_grid(aprimeindexes), a_grid(curraindex), z_val, e_val, ReturnFnParamsVec,2);
                        entireRHS_ii_V=ReturnMatrix_ii_V+beta*EV_z(aprimeindexes);
                        [Vtempii,~]=max(entireRHS_ii_V,[],1);
                        V(curraindex,z_c,e_c,N_j)=shiftdim(Vtempii,1);
                    else
                        loweredge=maxindex1_V(1,ii);
                        ReturnMatrix_ii_V=CreateReturnFnMatrix_Case1_Disc_DC1_nod_Par2e(ReturnFn, special_n_z, special_n_e, a_grid(loweredge), a_grid(curraindex), z_val, e_val, ReturnFnParamsVec,2);
                        entireRHS_ii_V=ReturnMatrix_ii_V+beta*EV_z(loweredge);
                        V(curraindex,z_c,e_c,N_j)=shiftdim(entireRHS_ii_V,1);
                    end
                end
                % --- Vtilde search (beta0beta) ---
                entireRHS_ii_Vt=ReturnMatrix_ii+beta0beta*EV_z;
                [Vtempii,maxindex1_Vt]=max(entireRHS_ii_Vt,[],1);
                Vtilde(level1ii,z_c,e_c,N_j)=shiftdim(Vtempii,1);
                Policy(level1ii,z_c,e_c,N_j)=shiftdim(maxindex1_Vt,1);
                ...
                for ii=1:(vfoptions.level1n-1)
                    ...
                    if maxgap_Vt(ii)>0
                        loweredge=min(maxindex1_Vt(1,ii),n_a-maxgap_Vt(ii));
                        aprimeindexes=loweredge+(0:1:maxgap_Vt(ii))';
                        ReturnMatrix_ii_Vt=CreateReturnFnMatrix_Case1_Disc_DC1_nod_Par2e(ReturnFn, special_n_z, special_n_e, a_grid(aprimeindexes), a_grid(curraindex), z_val, e_val, ReturnFnParamsVec,2);
                        ...
                        loweredge=maxindex1_Vt(1,ii);
                        ReturnMatrix_ii_Vt=CreateReturnFnMatrix_Case1_Disc_DC1_nod_Par2e(ReturnFn, special_n_z, special_n_e, a_grid(loweredge), a_grid(curraindex), z_val, e_val, ReturnFnParamsVec,2);
                        entireRHS_ii_Vt=ReturnMatrix_ii_Vt+beta0beta*EV_z(loweredge);
                        Vtilde(curraindex,z_c,e_c,N_j)=shiftdim(entireRHS_ii_Vt,1);
                        Policy(curraindex,z_c,e_c,N_j)=loweredge;
                    end
                end
            end
    ...
    if vfoptions.lowmemory==0
        ...
        for ii=1:(vfoptions.level1n-1)
            ...
            if maxgap_V(ii)>0
                loweredge=min(maxindex1_V(1,ii,:,:),n_a-maxgap_V(ii));
                aprimeindexes=loweredge+(0:1:maxgap_V(ii))';
                ReturnMatrix_ii_V=CreateReturnFnMatrix_Case1_Disc_DC1_nod_Par2e(ReturnFn, n_z, n_e, a_grid(aprimeindexes), a_grid(curraindex), z_gridvals_J(:,:,jj), e_gridvals_J(:,:,jj), ReturnFnParamsVec,2);
                aprimez=aprimeindexes+N_a*zind;
                entireRHS_ii_V=ReturnMatrix_ii_V+beta*reshape(EV(aprimez),[maxgap_V(ii)+1,1,N_z,N_e]);
                [Vtempii,~]=max(entireRHS_ii_V,[],1);
                ...
                loweredge=maxindex1_V(1,ii,:,:);
                ReturnMatrix_ii_V=CreateReturnFnMatrix_Case1_Disc_DC1_nod_Par2e(ReturnFn, n_z, n_e, reshape(a_grid(loweredge),loweredgesize), a_grid(curraindex), z_gridvals_J(:,:,jj), e_gridvals_J(:,:,jj), ReturnFnParamsVec,2);
                aprimez=loweredge+N_a*zind;
                entireRHS_ii_V=ReturnMatrix_ii_V+beta*reshape(EV(aprimez),[1,1,N_z,N_e]);
        ...
        for ii=1:(vfoptions.level1n-1)
            ...
            if maxgap_Vt(ii)>0
                loweredge=min(maxindex1_Vt(1,ii,:,:),n_a-maxgap_Vt(ii));
                aprimeindexes=loweredge+(0:1:maxgap_Vt(ii))';
                ReturnMatrix_ii_Vt=CreateReturnFnMatrix_Case1_Disc_DC1_nod_Par2e(ReturnFn, n_z, n_e, a_grid(aprimeindexes), a_grid(curraindex), z_gridvals_J(:,:,jj), e_gridvals_J(:,:,jj), ReturnFnParamsVec,2);
                aprimez=aprimeindexes+N_a*zind;
                entireRHS_ii_Vt=ReturnMatrix_ii_Vt+beta0beta*reshape(EV(aprimez),[maxgap_Vt(ii)+1,1,N_z,N_e]);
                [Vtempii,maxindex]=max(entireRHS_ii_Vt,[],1);
                ...
                loweredge=maxindex1_Vt(1,ii,:,:);
                ReturnMatrix_ii_Vt=CreateReturnFnMatrix_Case1_Disc_DC1_nod_Par2e(ReturnFn, n_z, n_e, reshape(a_grid(loweredge),loweredgesize), a_grid(curraindex), z_gridvals_J(:,:,jj), e_gridvals_J(:,:,jj), ReturnFnParamsVec,2);
                aprimez=loweredge+N_a*zind;
                entireRHS_ii_Vt=ReturnMatrix_ii_Vt+beta0beta*reshape(EV(aprimez),[1,1,N_z,N_e]);
        ...
        for e_c=1:N_e
            ...
            for ii=1:(vfoptions.level1n-1)
                ...
                if maxgap_V(ii)>0
                    loweredge=min(maxindex1_V(1,ii,:),n_a-maxgap_V(ii));
                    aprimeindexes=loweredge+(0:1:maxgap_V(ii))';
                    ReturnMatrix_ii_V=CreateReturnFnMatrix_Case1_Disc_DC1_nod_Par2e(ReturnFn, n_z, special_n_e, a_grid(aprimeindexes), a_grid(curraindex), z_gridvals_J(:,:,jj), e_val, ReturnFnParamsVec,2);
                    aprimez=aprimeindexes+N_a*zind;
                    entireRHS_ii_V=ReturnMatrix_ii_V+beta*reshape(EV(aprimez),[maxgap_V(ii)+1,1,N_z]);
                    [Vtempii,~]=max(entireRHS_ii_V,[],1);
                    ...
                    loweredge=maxindex1_V(1,ii,:);
                    ReturnMatrix_ii_V=CreateReturnFnMatrix_Case1_Disc_DC1_nod_Par2e(ReturnFn, n_z, special_n_e, reshape(a_grid(loweredge),loweredgesize), a_grid(curraindex), z_gridvals_J(:,:,jj), e_val, ReturnFnParamsVec,2);
                    aprimez=loweredge+N_a*zind;
                    entireRHS_ii_V=ReturnMatrix_ii_V+beta*reshape(EV(aprimez),[1,1,N_z]);
                    V(curraindex,:,e_c,jj)=shiftdim(entireRHS_ii_V,1);
                end
            end
            % --- Vtilde search (beta0beta) ---
            entireRHS_ii_Vt=ReturnMatrix_ii+beta0beta*EV;
            ...
            Policy(level1ii,:,e_c,jj)=shiftdim(maxindex1_Vt,1);
            maxgap_Vt=max(maxindex1_Vt(1,2:end,:)-maxindex1_Vt(1,1:end-1,:),[],3);
            for ii=1:(vfoptions.level1n-1)
                curraindex=level1ii(ii)+1:1:level1ii(ii+1)-1;
                if maxgap_Vt(ii)>0
                    loweredge=min(maxindex1_Vt(1,ii,:),n_a-maxgap_Vt(ii));
                    aprimeindexes=loweredge+(0:1:maxgap_Vt(ii))';
                    ReturnMatrix_ii_Vt=CreateReturnFnMatrix_Case1_Disc_DC1_nod_Par2e(ReturnFn, n_z, special_n_e, a_grid(aprimeindexes), a_grid(curraindex), z_gridvals_J(:,:,jj), e_val, ReturnFnParamsVec,2);
                    aprimez=aprimeindexes+N_a*zind;
                    entireRHS_ii_Vt=ReturnMatrix_ii_Vt+beta0beta*reshape(EV(aprimez),[maxgap_Vt(ii)+1,1,N_z]);
                    [Vtempii,maxindex]=max(entireRHS_ii_Vt,[],1);
                    ...
                    loweredge=maxindex1_Vt(1,ii,:);
                    ReturnMatrix_ii_Vt=CreateReturnFnMatrix_Case1_Disc_DC1_nod_Par2e(ReturnFn, n_z, special_n_e, reshape(a_grid(loweredge),loweredgesize), a_grid(curraindex), z_gridvals_J(:,:,jj), e_val, ReturnFnParamsVec,2);
                    aprimez=loweredge+N_a*zind;
                    entireRHS_ii_Vt=ReturnMatrix_ii_Vt+beta0beta*reshape(EV(aprimez),[1,1,N_z]);
                    Vtilde(curraindex,:,e_c,jj)=shiftdim(entireRHS_ii_Vt,1);
            ...
            for e_c=1:N_e
                ...
                for ii=1:(vfoptions.level1n-1)
                    ...
                    if maxgap_V(ii)>0
                        ...
                        ReturnMatrix_ii_V=CreateReturnFnMatrix_Case1_Disc_DC1_nod_Par2e(ReturnFn, special_n_z, special_n_e, a_grid(loweredge), a_grid(curraindex), z_val, e_val, ReturnFnParamsVec,2);
                        entireRHS_ii_V=ReturnMatrix_ii_V+beta*EV_z(loweredge);
                        V(curraindex,z_c,e_c,jj)=shiftdim(entireRHS_ii_V,1);
                    end
                end
                % --- Vtilde search (beta0beta) ---
                entireRHS_ii_Vt=ReturnMatrix_ii+beta0beta*EV_z;
                ...
                Policy(level1ii,z_c,e_c,jj)=shiftdim(maxindex1_Vt,1);
                maxgap_Vt=maxindex1_Vt(1,2:end)-maxindex1_Vt(1,1:end-1);
                for ii=1:(vfoptions.level1n-1)
                    curraindex=level1ii(ii)+1:1:level1ii(ii+1)-1;
                    if maxgap_Vt(ii)>0
                        ...
                        ReturnMatrix_ii_Vt=CreateReturnFnMatrix_Case1_Disc_DC1_nod_Par2e(ReturnFn, special_n_z, special_n_e, a_grid(loweredge), a_grid(curraindex), z_val, e_val, ReturnFnParamsVec,2);
                        entireRHS_ii_Vt=ReturnMatrix_ii_Vt+beta0beta*EV_z(loweredge);
                        Vtilde(curraindex,z_c,e_c,jj)=shiftdim(entireRHS_ii_Vt,1);
