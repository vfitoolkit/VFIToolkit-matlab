...
if ~isfield(vfoptions,'V_Jplus1')
    ...
    if vfoptions.lowmemory==0
        ...
        [Vtempii,~]=max(reshape(entireRHS_ii_V,[N_d*N_a,vfoptions.level1n,N_e]),[],1);
        V(level1ii,:,N_j)=shiftdim(Vtempii,1);
        ...
        for ii=1:(vfoptions.level1n-1)
            ...
            if maxgap_V(ii)>0
                ...
                ReturnMatrix_ii_V=CreateReturnFnMatrix_Case1_Disc_DC1_Par2(ReturnFn, n_d, n_e, d_gridvals, a_grid(aprimeindexes), a_grid(curraindex), e_gridvals_J(:,:,N_j), ReturnFnParamsVec,2);
                entireRHS_ii_V=ReturnMatrix_ii_V+beta*reshape(EV(aprimeindexes),[N_d*(maxgap_V(ii)+1),1,N_e]);
                [Vtempii,~]=max(entireRHS_ii_V,[],1);
                V(curraindex,:,N_j)=shiftdim(Vtempii,1);
            else
                loweredge=maxindex1_V(:,1,ii,:);
                ReturnMatrix_ii_V=CreateReturnFnMatrix_Case1_Disc_DC1_Par2(ReturnFn, n_d, n_e, d_gridvals, a_grid(loweredge), a_grid(curraindex), e_gridvals_J(:,:,N_j), ReturnFnParamsVec,2);
                entireRHS_ii_V=ReturnMatrix_ii_V+beta*reshape(EV(loweredge),[N_d*1,1,N_e]);
                [Vtempii,~]=max(entireRHS_ii_V,[],1);
                V(curraindex,:,N_j)=shiftdim(Vtempii,1);
            end
        end
        ...
        [Vtempii,maxindex2_Vt]=max(reshape(entireRHS_ii_Vt,[N_d*N_a,vfoptions.level1n,N_e]),[],1);
        ...
        for ii=1:(vfoptions.level1n-1)
            ...
            if maxgap_Vt(ii)>0
                ...
                ReturnMatrix_ii_Vt=CreateReturnFnMatrix_Case1_Disc_DC1_Par2(ReturnFn, n_d, n_e, d_gridvals, a_grid(aprimeindexes), a_grid(curraindex), e_gridvals_J(:,:,N_j), ReturnFnParamsVec,2);
                entireRHS_ii_Vt=ReturnMatrix_ii_Vt+beta0beta*reshape(EV(aprimeindexes),[N_d*(maxgap_Vt(ii)+1),1,N_e]);
                ...
                ReturnMatrix_ii_Vt=CreateReturnFnMatrix_Case1_Disc_DC1_Par2(ReturnFn, n_d, n_e, d_gridvals, a_grid(loweredge), a_grid(curraindex), e_gridvals_J(:,:,N_j), ReturnFnParamsVec,2);
                entireRHS_ii_Vt=ReturnMatrix_ii_Vt+beta0beta*reshape(EV(loweredge),[N_d*1,1,N_e]);
                [Vtempii,maxindex]=max(entireRHS_ii_Vt,[],1);
                Vtilde(curraindex,:,N_j)=shiftdim(Vtempii,1);
                dind=(rem(maxindex-1,N_d)+1);
                allind=dind+N_d*eind;
        ...
        for e_c=1:N_e
            ...
            for ii=1:(vfoptions.level1n-1)
                ...
                if maxgap_V(ii)>0
                    loweredge=min(maxindex1_V(:,1,ii),n_a-maxgap_V(ii));
                    aprimeindexes=loweredge+(0:1:maxgap_V(ii));
                    ReturnMatrix_ii_V=CreateReturnFnMatrix_Case1_Disc_DC1_Par2(ReturnFn, n_d, special_n_e, d_gridvals, a_grid(aprimeindexes), a_grid(curraindex), e_val, ReturnFnParamsVec,2);
                    entireRHS_ii_V=ReturnMatrix_ii_V+beta*reshape(EV(aprimeindexes),[N_d*(maxgap_V(ii)+1),1]);
                    [Vtempii,~]=max(entireRHS_ii_V,[],1);
                    V(curraindex,e_c,N_j)=shiftdim(Vtempii,1);
                else
                    loweredge=maxindex1_V(:,1,ii);
                    ReturnMatrix_ii_V=CreateReturnFnMatrix_Case1_Disc_DC1_Par2(ReturnFn, n_d, special_n_e, d_gridvals, a_grid(loweredge), a_grid(curraindex), e_val, ReturnFnParamsVec,2);
            ...
            for ii=1:(vfoptions.level1n-1)
                ...
                if maxgap_Vt(ii)>0
                    loweredge=min(maxindex1_Vt(:,1,ii),n_a-maxgap_Vt(ii));
                    aprimeindexes=loweredge+(0:1:maxgap_Vt(ii));
                    ReturnMatrix_ii_Vt=CreateReturnFnMatrix_Case1_Disc_DC1_Par2(ReturnFn, n_d, special_n_e, d_gridvals, a_grid(aprimeindexes), a_grid(curraindex), e_val, ReturnFnParamsVec,2);
                    ...
                    loweredge=maxindex1_Vt(:,1,ii);
                    ReturnMatrix_ii_Vt=CreateReturnFnMatrix_Case1_Disc_DC1_Par2(ReturnFn, n_d, special_n_e, d_gridvals, a_grid(loweredge), a_grid(curraindex), e_val, ReturnFnParamsVec,2);
    ...
    if vfoptions.lowmemory==0
        ...
        for ii=1:(vfoptions.level1n-1)
            ...
            if maxgap_V(ii)>0
                loweredge=min(maxindex1_V(:,1,ii,:),n_a-maxgap_V(ii));
                aprimeindexes=loweredge+(0:1:maxgap_V(ii));
                ReturnMatrix_ii_V=CreateReturnFnMatrix_Case1_Disc_DC1_Par2(ReturnFn, n_d, n_e, d_gridvals, a_grid(aprimeindexes), a_grid(curraindex), e_gridvals_J(:,:,jj), ReturnFnParamsVec,2);
                entireRHS_ii_V=ReturnMatrix_ii_V+beta*reshape(EV(aprimeindexes),[N_d*(maxgap_V(ii)+1),1,N_e]);
                [Vtempii,~]=max(entireRHS_ii_V,[],1);
                V(curraindex,:,jj)=shiftdim(Vtempii,1);
            else
                loweredge=maxindex1_V(:,1,ii,:);
                ReturnMatrix_ii_V=CreateReturnFnMatrix_Case1_Disc_DC1_Par2(ReturnFn, n_d, n_e, d_gridvals, a_grid(loweredge), a_grid(curraindex), e_gridvals_J(:,:,jj), ReturnFnParamsVec,2);
                entireRHS_ii_V=ReturnMatrix_ii_V+beta*reshape(EV(loweredge),[N_d*1,1,N_e]);
                [Vtempii,~]=max(entireRHS_ii_V,[],1);
                V(curraindex,:,jj)=shiftdim(Vtempii,1);
            end
        ...
        for ii=1:(vfoptions.level1n-1)
            ...
            if maxgap_Vt(ii)>0
                loweredge=min(maxindex1_Vt(:,1,ii,:),n_a-maxgap_Vt(ii));
                aprimeindexes=loweredge+(0:1:maxgap_Vt(ii));
                ReturnMatrix_ii_Vt=CreateReturnFnMatrix_Case1_Disc_DC1_Par2(ReturnFn, n_d, n_e, d_gridvals, a_grid(aprimeindexes), a_grid(curraindex), e_gridvals_J(:,:,jj), ReturnFnParamsVec,2);
                entireRHS_ii_Vt=ReturnMatrix_ii_Vt+beta0beta*reshape(EV(aprimeindexes),[N_d*(maxgap_Vt(ii)+1),1,N_e]);
                [Vtempii,maxindex]=max(entireRHS_ii_Vt,[],1);
                ...
                loweredge=maxindex1_Vt(:,1,ii,:);
                ReturnMatrix_ii_Vt=CreateReturnFnMatrix_Case1_Disc_DC1_Par2(ReturnFn, n_d, n_e, d_gridvals, a_grid(loweredge), a_grid(curraindex), e_gridvals_J(:,:,jj), ReturnFnParamsVec,2);
                entireRHS_ii_Vt=ReturnMatrix_ii_Vt+beta0beta*reshape(EV(loweredge),[N_d*1,1,N_e]);
                [Vtempii,maxindex]=max(entireRHS_ii_Vt,[],1);
                Vtilde(curraindex,:,jj)=shiftdim(Vtempii,1);
                dind=(rem(maxindex-1,N_d)+1);
