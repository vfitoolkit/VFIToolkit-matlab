...
Policy(4,:,:,:,:)=2;
Policyalt=zeros(4,N_a,N_semiz*N_z,N_e,N_j,'gpuArray'); % exponential discounter optimal [d2; midpoint; aprimeL2ind]
Policyalt(4,:,:,:,:)=2;
...
if ~isfield(vfoptions,'V_Jplus1')
    ...
    elseif vfoptions.lowmemory==2 % outer z / inner e, vectorize semiz
        for z_c=1:N_z
            ...
            for e_c=1:N_e
                ...
                inLowerStrict = (L2offset >= 2)         & (L2offset <= n2short+1);
                inUpperStrict = (L2offset >= n2short+3) & (L2offset <= n2long-1);
                Policy(4,:,semizblock,e_c,N_j) = 2 + (inLowerStrict & isInfLower) - (inUpperStrict & isInfUpper);
    ...
    Policyalt(4,:,:,:,N_j)=reshape(flagV_ford2_jj((1:1:N_a*N_semiz*N_z*N_e)'+(N_a*N_semiz*N_z*N_e)*(maxindexalt_lin-1)),[1,N_a,N_semiz*N_z,N_e]);
    ...
    Policy(4,:,:,:,N_j)=reshape(flag_ford2_jj((1:1:N_a*N_semiz*N_z*N_e)'+(N_a*N_semiz*N_z*N_e)*(maxindex-1)),[1,N_a,N_semiz*N_z,N_e]);
end
...
for reverse_j=1:N_j-1
    ...
    Policyalt(4,:,:,:,jj)=reshape(flagV_ford2_jj((1:1:N_a*N_semiz*N_z*N_e)'+(N_a*N_semiz*N_z*N_e)*(maxindexalt_lin-1)),[1,N_a,N_semiz*N_z,N_e]);
    [Vtilde_jj,maxindex]=max(Valt_ford2_jj,[],4);
    Vtilde(:,:,:,jj)=Vtilde_jj;
    ...
    Policy(4,:,:,:,jj)=reshape(flag_ford2_jj((1:1:N_a*N_semiz*N_z*N_e)'+(N_a*N_semiz*N_z*N_e)*(maxindex-1)),[1,N_a,N_semiz*N_z,N_e]);
end
