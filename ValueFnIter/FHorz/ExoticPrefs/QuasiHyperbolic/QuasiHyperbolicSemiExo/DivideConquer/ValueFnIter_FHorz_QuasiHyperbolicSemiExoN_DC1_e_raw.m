function [Vtilde,Policy3,V]=ValueFnIter_FHorz_QuasiHyperbolicSemiExoN_DC1_e_raw(n_d1,n_d2,n_a,n_z,n_semiz, n_e,N_j, d1_gridvals, d2_gridvals, a_grid, z_gridvals_J, semiz_gridvals_J, e_gridvals_J,pi_z_J, pi_semiz_J, pi_e_J, ReturnFn, Parameters, DiscountFactorParamNames, ReturnFnParamNames, vfoptions)
% Naive QH + SemiExo + DC, with d1, z, e. Dual-V; cross-d2 max on Vtilde.
...
if ~isfield(vfoptions,'V_Jplus1')
    ...
    V(:,:,:,N_j)=reshape(V_ford2_jj((1:1:NN)'+NN*(maxindex_lin-1)),[N_a,N_semiz*N_z,N_e]);
    ...
    if vfoptions.lowmemory==0
        for d2_c=1:N_d2
            ...
            [Vtempii_V,~]=max(reshape(entireRHS_V,[N_d1*N_a,vfoptions.level1n,N_bothz,N_e]),[],1);
    ...
    elseif vfoptions.lowmemory==1
        for d2_c=1:N_d2
            ...
            for e_c=1:N_e
                ...
                entireRHS_V=ReturnMatrix_d2ii+beta*shiftdim(EV_d2,-1);
                [~,maxindex1_V]=max(entireRHS_V,[],2);
                [Vtempii_V,~]=max(reshape(entireRHS_V,[N_d1*N_a,vfoptions.level1n,N_bothz]),[],1);
...
for reverse_j=1:N_j-1
    ...
    NN=N_a*N_semiz*N_z*N_e;
    maxindex_lin=reshape(maxindex,[NN,1]);
    V(:,:,:,jj)=reshape(V_ford2_jj((1:1:NN)'+NN*(maxindex_lin-1)),[N_a,N_semiz*N_z,N_e]);
    d1aprime_ind=reshape(Policy_ford2_jj((1:1:NN)'+NN*(maxindex_lin-1)),[1,N_a,N_semiz*N_z,N_e]);
