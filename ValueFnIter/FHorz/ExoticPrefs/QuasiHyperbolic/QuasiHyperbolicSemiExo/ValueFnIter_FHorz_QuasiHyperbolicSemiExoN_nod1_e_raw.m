function [Vtilde, Policy3, V, Policy3alt]=ValueFnIter_FHorz_QuasiHyperbolicSemiExoN_nod1_e_raw(n_d2, n_a, n_z, n_semiz, n_e, N_j, d2_gridvals, a_grid, z_gridvals_J, semiz_gridvals_J, e_gridvals_J, pi_z_J, pi_semiz_J, pi_e_J, ReturnFn, Parameters, DiscountFactorParamNames, ReturnFnParamNames, vfoptions)
...
Policy3alt=zeros(2,N_a,N_semiz*N_z,N_e,N_j,'gpuArray'); % exponential discounter optimal (d2, aprime)
...
if ~isfield(vfoptions,'V_Jplus1')
    ...
    Policy3alt(2,:,:,:,N_j)=reshape(Policy_V_ford2_jj((1:1:N_a*N_semiz*N_z*N_e)'+(N_a*N_semiz*N_z*N_e)*(maxindexalt_lin-1)),[1,N_a,N_semiz*N_z,N_e]);
end
...
for reverse_j=1:N_j-1
    ...
    maxindexalt_lin=reshape(maxindexalt_d2,[N_a*N_semiz*N_z*N_e,1]);
    Policy3alt(2,:,:,:,jj)=reshape(Policy_V_ford2_jj((1:1:N_a*N_semiz*N_z*N_e)'+(N_a*N_semiz*N_z*N_e)*(maxindexalt_lin-1)),[1,N_a,N_semiz*N_z,N_e]);
end
