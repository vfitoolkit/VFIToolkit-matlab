function [V1, Policy, Valt]=ValueFnIter_FHorz_QuasiHyperbolicSemiExoN_DC1_GI1_e_raw(n_d1,n_d2,n_a,n_z,n_semiz, n_e,N_j, d1_gridvals, d2_gridvals, a_grid, z_gridvals_J, semiz_gridvals_J, e_gridvals_J,pi_z_J, pi_semiz_J, pi_e_J, ReturnFn, Parameters, DiscountFactorParamNames, ReturnFnParamNames, vfoptions)
% Naive QH + SemiExo + DC + GI raw, with d1, z, e. Output: (V1=Vtilde, Policy3, Valt=V).
...
if ~isfield(vfoptions,'V_Jplus1')
    ...
    V(:,:,:,N_j)=max(Valt_ford2_jj,[],4); % max over d2 of V-at-V-optimal (Naive QH: V uses its own argmax, NOT the Vtilde argmax)
...
for reverse_j=1:N_j-1
    ...
    maxindex=reshape(maxindex,[N_a*N_semiz*N_z*N_e,1]);
    V(:,:,:,jj)=max(Valt_ford2_jj,[],4); % max over d2 of V-at-V-optimal (Naive QH: V uses its own argmax, NOT the Vtilde argmax)
    d1aprimeL2_ind=reshape(Policy_ford2_jj((1:1:N_a*N_semiz*N_z*N_e)'+(N_a*N_semiz*N_z*N_e)*(maxindex-1)),[1,N_a,N_semiz*N_z*N_e]);
