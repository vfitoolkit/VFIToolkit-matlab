...
if vfoptions.parallel==2
    if isfield(vfoptions,'n_e')
        ...
        if N_z==0
            [VKron,PolicyKron]=ValueFnIter_Case3_FHorz_EpsteinZin_noz_e_raw(n_d, n_a, vfoptions.n_e, n_u, N_j, d_grid, a_grid, e_grid, u_grid, pi_e, pi_u, ReturnFn, aprimeFn, Parameters, DiscountFactorParamNames, ReturnFnParamNames, aprimeFnParamNames, vfoptions);
        else
            [VKron,PolicyKron]=ValueFnIter_Case3_FHorz_EpsteinZin_e_raw(n_d, n_a, n_z, vfoptions.n_e, n_u, N_j, d_grid, a_grid, z_grid, e_grid, u_grid, pi_z, pi_e, pi_u, ReturnFn, aprimeFn, Parameters, DiscountFactorParamNames, ReturnFnParamNames, aprimeFnParamNames, vfoptions);
        end
    else
        if N_z==0
            error('Cannot use Epstein-Zin preferences without any shocks (what is the point?); you have n_z=0 and no e variables')
        else
            [VKron, PolicyKron]=ValueFnIter_Cas31_FHorz_EpsteinZin_raw(n_d,n_a,n_z,n_u, N_j, d_grid, a_grid, z_grid, u_grid, pi_z, pi_u, ReturnFn, aprimeFn, Parameters, DiscountFactorParamNames, ReturnFnParamNames, aprimeFnParamNames, vfoptions);
        end
    end
else
