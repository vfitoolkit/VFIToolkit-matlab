function [V,Policy]=ValueFnIter_FHorz_SemiExo_DC1_GI_nod1_e_raw(n_d2,n_a,n_z,n_semiz, n_e,N_j, d2_grid, a_grid, z_gridvals_J, semiz_gridvals_J, e_gridvals_J,pi_z_J, pi_semiz_J, pi_e_J, ReturnFn, Parameters, DiscountFactorParamNames, ReturnFnParamNames, vfoptions)
...
if ~isfield(vfoptions,'V_Jplus1')
    ...
    if vfoptions.lowmemory==0
        ...
        for ii=1:(vfoptions.level1n-1)
            ...
            if maxgap(ii)>0
                ...
                ReturnMatrix_ii=CreateReturnFnMatrix_Case1_Disc_DC1_Par2e(ReturnFn, n_d2, n_bothz, n_e, d2_gridvals, a_grid(aprimeindexes), a_grid(level1ii(ii)+1:level1ii(ii+1)-1), bothz_gridvals_J(:,:,N_j), e_gridvals_J(:,:,N_j), ReturnFnParamsVec,3);
        ...
        for e_c=1:N_e
            ...
            for ii=1:(vfoptions.level1n-1)
                ...
                if maxgap(ii)>0
                    ...
                    ReturnMatrix_ii_e=CreateReturnFnMatrix_Case1_Disc_DC1_Par2e(ReturnFn, n_d2, n_bothz, special_n_e, d2_gridvals, a_grid(aprimeindexes), a_grid(level1ii(ii)+1:level1ii(ii+1)-1), bothz_gridvals_J(:,:,N_j), e_val, ReturnFnParamsVec,3);
    ...
    if vfoptions.lowmemory==0
        for d2_c=1:N_d2
            ...
            allind=d1_ind+aind+N_a*bothzind+N_a*N_bothz*eind; % loweredge is n_d-by-1-by-n_a-by-n_bothz-by-n_e
    ...
    elseif vfoptions.lowmemory==1
        for d2_c=1:N_d2
            ...
            for e_c=1:N_e
                ...
                entireRHS_d2ii=ReturnMatrix_d2iie+DiscountFactorParamsVec*EV;
                % Treat standard problem as just being the first layer
                [~,maxindex1]=max(entireRHS_d2ii,[],1); % no d1
                ...
                ReturnMatrix_d2ii=CreateReturnFnMatrix_Case1_Disc_DC1_Par2e(ReturnFn, special_n_d2, n_bothz, n_e, d2_gridvals(d2_c,:), aprime_grid(aprimeindexes), a_grid, bothz_gridvals_J(:,:,N_j), e_val, ReturnFnParamsVec,2);
                aprimez=aprimeindexes+n2aprime*shiftdim((0:1:N_bothz-1),-2); % the current aprime
                entireRHS_ii=ReturnMatrix_d2ii+DiscountFactorParamsVec*reshape(EVinterp(aprimez(:)),[n2long,N_a,N_bothz,N_e]);
                [Vtemp,maxindex]=max(entireRHS_ii,[],1);
                ...
                d1_ind=rem(maxindex-1,N_d1)+1;
                allind=d1_ind+aind+N_a*bothzind; % loweredge is n_d-by-1-by-n_a-by-n_bothz
    ...
    elseif vfoptions.lowmemory==1 
        for d2_c=1:N_d2
            ...
            for e_c=1:N_e
                ...
                % aprime possibilities are
                % n2long-by-n_a-by-n_bothz
                ReturnMatrix_d2ii=CreateReturnFnMatrix_Case1_Disc_DC1_Par2e(ReturnFn, special_n_d2, n_bothz, n_e, d2_gridvals(d2_c,:), aprime_grid(aprimeindexes), a_grid, bothz_gridvals_J(:,:,jj), e_val, ReturnFnParamsVec,2);
                aprimez=aprimeindexes+n2aprime*shiftdim((0:1:N_bothz-1),-2); % the current aprime
                entireRHS_ii=ReturnMatrix_d2ii+DiscountFactorParamsVec*reshape(EVinterp(aprimez(:)),[n2long,N_a,N_bothz,N_e]);
                [Vtemp,maxindex]=max(entireRHS_ii,[],1);
