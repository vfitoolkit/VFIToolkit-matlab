function [VPath,PolicyPath]=ValueFnOnTransPath_Case1(PricePath, ParamPath, T, V_final, Policy_final, Parameters, n_d, n_a, n_z, pi_z, d_grid, a_grid,z_grid, DiscountFactorParamNames, ReturnFn, transpathoptions, vfoptions, simoptions)
...
if max(vfoptions.endotype)==1
    l_endotype=sum(vfoptions.endotype);
end
temp=getAnonymousFnInputNames(ReturnFn);
if length(temp)>(l_d+l_a+l_a+l_z)
    ReturnFnParamNames={temp{l_d+l_a+l_a+l_z-l_endotype+1:end}}; % the first inputs will always be (d,aprime,a,z)
    % (subtract l_endotype twice, once from each l_a, and then add once; net of subtract once)
else
    ReturnFnParamNames={};
end
