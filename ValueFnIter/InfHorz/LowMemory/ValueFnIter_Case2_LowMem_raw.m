...
if phiaprimematrix==2 % GPU
    Phi_aprime=CreatePhiaprimeMatrix_Case2_Disc_Par2(Phi_aprime, Case2_Type, n_d, n_a, n_z, d_grid, a_grid, z_grid,PhiaprimeParamsVec);
elseif phiaprimematrix==1
    % It is already a matrix
else
    fprintf('ERROR: must use vfoptions.phiaprimematrix==1 or 2 when doing ValueFnIter_Case2 with lowmemory==1 and parallel==2 \n')
    dbstack
    return
end
...
elseif Case2_Type==2 %a'(d,z,z')
    ...
    while currdist>Tolerance
        ...
        for z_c=1:N_z % Can probably eliminate this loop and replace with a matrix multiplication operation thereby making it faster
            ...
            ReturnMatrix_z=CreateReturnFnMatrix_Case2_Disc_Par2(ReturnFn, n_d, n_a, ones(l_z,1), d_grid, a_grid, zvals,ReturnFnParamsVec);
