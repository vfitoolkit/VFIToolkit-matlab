...
while PricePathDist>transpathoptions.tolerance && pathcounter<=transpathoptions.maxiter
    ...
    PolicyPath=zeros(1,N_a,N_j,N_z,T-1,'gpuArray'); %Periods 1 to T-1
    ...
    for ttr=1:T-1 %so tt=T-ttr
        ...
        PolicyPath(1,:,:,:,T-ttr)=Policy; % fastOLG: so a-by-j-by-z
    ...
    for tt=1:T-1
        ...
        Policy=PolicyPath(1,:,:,:,tt); % fastOLG: so a-by-j-by-z
        ...
        AggVars=EvalFnOnAgentDist_AggVars_FHorz_fastOLG(AgentDist.*AgeWeights_T(:,tt),[], Policy, FnsToEvaluate,FnsToEvaluateParamNames,AggVarNames,Parameters,N_j,0,l_a,l_a,l_z,N_a,N_z,[],aprime_gridvals,a_gridvals,z_gridvals_J,1);
        ...
        AgentDist=StationaryDist_FHorz_Case1_TPath_SingleStep_IterFast_raw(AgentDist,reshape(Policy(1,:,1:end-1,:),[1,N_a*(N_j-1)*N_z]),N_a,N_z,N_j,pi_z_J_sim,exceptlastj,exceptfirstj,justfirstj,jequalOneDist); % Policy for jj=1:N_j-1
    end
