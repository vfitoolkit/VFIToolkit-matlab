...
PolicyProbsPath=zeros([N_a*(N_j-1)*N_z*N_e,2,T-1],'gpuArray');
...
while PricePathDist>transpathoptions.tolerance && pathcounter<=transpathoptions.maxiter
    ...
    PolicyPath_aprime(:,1,:)=reshape(PolicyPath(1,:,1:end-1,:,:,:),[N_a*(N_j-1)*N_z*N_e,1,T-1]); % lower grid point index
    PolicyPath_aprime(:,2,:)=PolicyPath_aprime(:,1,:)+1; % upper grid point index
    PolicyProbsPath(:,2,:)=reshape((PolicyPath(2,:,1:end-1,:,:,:)-1)/n2short,[N_a*(N_j-1)*N_z*N_e,1,T-1]); % prob of upper grid point
    ...
    for tt=1:T-1
        ...
        AggVars=EvalFnOnAgentDist_AggVars_FHorz_fastOLG(AgentDist.*AgeWeights_T(:,:,tt),[], PolicyPath_aprimefine(:,:,:,:,:,tt), FnsToEvaluate,FnsToEvaluateParamNames,AggVarNames,Parameters,N_j,0,l_a,l_a,l_ze,N_a,N_ze,[],aprimefine_gridvals,a_gridvals,ze_gridvals_J,1);
        ...
        AgentDist=AgentDist_FHorz_TPath_SingleStep_IterFast_TwoProbs_e_raw(AgentDist,PolicyPath_aprime(:,:,tt),PolicyProbsPath(:,:,tt),N_a,N_z,N_e,N_j,pi_z_J_sim,pi_e_J_sim,exceptlastj,exceptfirstj,justfirstj,jequalOneDist);
    end    
