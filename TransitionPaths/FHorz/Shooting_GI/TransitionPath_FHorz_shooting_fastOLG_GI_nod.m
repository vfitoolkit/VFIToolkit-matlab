...
% preallocate
PolicyPath_aprime=zeros([N_a*(N_j-1)*N_z,2,T-1],'gpuArray');
PolicyProbsPath=zeros([N_a*(N_j-1)*N_z,2,T-1],'gpuArray');
...
while PricePathDist>transpathoptions.tolerance && pathcounter<=transpathoptions.maxiter
    ...
    PolicyPath=zeros(2,N_a,N_j,N_z,T-1,'gpuArray'); %Periods 1 to T-1
    ...
    PolicyPath_aprimefine=(n2short+1)*(PolicyPath(1,:,:,:,:)-1)+PolicyPath(2,:,:,:,:);
    PolicyPath_aprime(:,1,:)=reshape(PolicyPath(1,:,1:end-1,:,:),[N_a*(N_j-1)*N_z,1,T-1]); % lower grid point index
    PolicyPath_aprime(:,2,:)=PolicyPath_aprime(:,1,:)+1; % upper grid point index
    PolicyProbsPath(:,2,:)=reshape((PolicyPath(2,:,1:end-1,:,:)-1)/n2short,[N_a*(N_j-1)*N_z,1,T-1]); % prob of upper grid point
    ...
    for tt=1:T-1
        ...
        AggVars=EvalFnOnAgentDist_AggVars_FHorz_fastOLG(AgentDist.*AgeWeights_T(:,tt),[], PolicyPath_aprimefine(:,:,:,:,tt), FnsToEvaluate,FnsToEvaluateParamNames,AggVarNames,Parameters,N_j,0,l_a,l_a,l_z,N_a,N_z,[],aprimefine_gridvals,a_gridvals,z_gridvals_J,1);
        ...
        AgentDist=AgentDist_FHorz_TPath_SingleStep_IterFast_TwoProbs_raw(AgentDist,PolicyPath_aprime(:,:,tt),PolicyProbsPath(:,:,tt),N_a,N_z,N_j,pi_z_J_sim,exceptlastj,exceptfirstj,justfirstj,jequalOneDist); % Policy for jj=1:N_j-1
    end
