...
while PricePathDist>transpathoptions.tolerance && pathcounter<=transpathoptions.maxiter
    PolicyPath=zeros(1,N_a,N_z,N_j,T-1,'gpuArray'); %Periods 1 to T-1
    ...
    for tt=1:T-1
        ...
        Policy_fastOLG_aprime=reshape(permute(Policy(1,:,:,:),[1,2,4,3]),[1,N_a*N_j*N_z]);
        AggVars=EvalFnOnAgentDist_AggVars_FHorz_fastOLG(AgentDist.*AgeWeights,[], Policy_fastOLG_aprime, FnsToEvaluate,FnsToEvaluateParamNames,AggVarNames,Parameters,N_j,0,l_a,l_a,l_z,N_a,N_z,[],aprime_gridvals,a_gridvals,z_gridvals_J_fastOLG,1);
        ...
        if simoptions.fastOLG==0
            AgentDist=StationaryDist_FHorz_Case1_TPath_SingleStep_Iteration_raw(AgentDist,Policy,N_a,N_z,N_j,pi_z_J,jequalOneDist);
        else % simoptions.fastOLG==1
            AgentDist=StationaryDist_FHorz_Case1_TPath_SingleStep_IterFast_raw(AgentDist,Policy_fastOLG_aprime(exceptlastj),N_a,N_z,N_j,pi_z_J_sim,exceptlastj,exceptfirstj,justfirstj,jequalOneDist);
        end
    end
