function [V,Policy2]=ValueFnIter_Case1_FHorz_NQHyperbolic_SingleStep_fOLG_raw(V,n_d,n_a,n_z,N_j, d_grid, a_grid, z_grid, pi_z, ReturnFn, Parameters, DiscountFactorParamNames, ReturnFnParamNames, vfoptions)
...
ReturnMatrix=CreateReturnFnMatrix_Case1_Disc_Par2_fastOLG(ReturnFn, n_d, n_a, n_z, N_j, d_grid, a_grid, z_grid_J, ReturnFnParamsAgeMatrix);
...
% VKronNext is based on Vexp
VKronNext=zeros(N_a,N_j,N_z,'gpuArray');
VKronNext(:,1:N_j-1,:)=permute(V.Vexp(:,:,2:end),[1 3 2]); % Swap j and z
...
for z_c=1:N_z
    ...
    [Vtemp,~]=max(entirediscountedEV_z,[],1);
    V.Vexp(:,z_c,:)=reshape(Vtemp,[N_a,1,N_j]); % V is over (a,z,j)
    ...
    V.Vtilde(:,z_c,:)=reshape(Vtemp,[N_a,1,N_j]); % V is over (a,z,j)
