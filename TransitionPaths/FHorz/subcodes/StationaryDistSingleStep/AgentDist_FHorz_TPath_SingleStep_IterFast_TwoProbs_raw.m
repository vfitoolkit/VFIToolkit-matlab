function AgentDist=AgentDist_FHorz_TPath_SingleStep_IterFast_TwoProbs_raw(AgentDist,Policy_aprime,PolicyProbs,N_a,N_z,N_j,pi_z_J_sim,exceptlastj,exceptfirstj,justfirstj,jequalOneDist)
...
% Policy_aprime is [N_a*(N_j-1)*N_z,2]
% PolicyProbs is [N_a*(N_j-1)*N_z,2]
...
firststep=Policy_aprime+repelem(N_a*(0:1:(N_j-1)*N_z-1)',N_a,2);
II=repelem((1:1:N_a*(N_j-1)*N_z)',1,2);

Gammatranspose=sparse(firststep,II,PolicyProbs,N_a*(N_j-1)*N_z,N_a*(N_j-1)*N_z);
% Note: N_j-1, not N_j
