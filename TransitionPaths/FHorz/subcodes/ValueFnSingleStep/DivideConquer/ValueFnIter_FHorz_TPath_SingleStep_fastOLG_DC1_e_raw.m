function [V,Policy]=ValueFnIter_FHorz_TPath_SingleStep_fastOLG_DC1_e_raw(V,n_d,n_a,n_z,n_e,N_j, d_grid, a_grid, z_gridvals_J, e_gridvals_J, pi_z_J, pi_e_J, ReturnFn, Parameters, DiscountFactorParamNames, ReturnFnParamNames, vfoptions)
...
DiscountFactorParamsVec=reshape(DiscountFactorParamsVec,[1,1,1,N_j]);
...
if vfoptions.lowmemory==0
    ...
    discountedEV=repelem(DiscountFactorParamsVec.*reshape(EV,[1,N_a,1,N_j,N_z]),N_d,1); % [aprime]
    ...
    for ii=1:(vfoptions.level1n-1)
        if maxgap(ii)>0
            ...
            daprimez=(repmat(1:1:N_d,1,maxgap(ii)+1))'+N_d*repelem(reshape(aprimeindexes-1,[N_d*(maxgap(ii)+1),1,N_j,N_z,N_e]),1,level1iidiff(ii))+N_d*N_a*shiftdim((0:1:N_j-1),-1)+N_d*N_a*N_j*shiftdim((0:1:N_z-1),-2); % all the d, with the current aprimeii(ii):aprimeii(ii+1)
            entireRHS_ii=ReturnMatrix_ii+discountedentireEV(daprimez); % autofill e for the expectations
            [Vtempii,maxindex]=max(entireRHS_ii,[],1);
            V(level1ii(ii)+1:level1ii(ii+1)-1,:,:,:)=shiftdim(Vtempii,1);
            Policy(level1ii(ii)+1:level1ii(ii+1)-1,:,:,:)=shiftdim(maxindex+N_d*(loweredge(rem(maxindex-1,N_d)+1+N_d*shiftdim((0:1:N_j-1),-1)+N_d*N_j*shiftdim((0:1:N_z-1),-2)+N_d*N_j*N_z*shiftdim((0:1:N_e-1),-3))-1),1); % loweredge(given the d)
        else
            ...
            daprimez=(1:1:N_d)'+N_d*repelem(reshape(loweredge-1,[N_d,1,N_j,N_z,N_e]),1,level1iidiff(ii))+N_d*N_a*shiftdim((0:1:N_j-1),-1)+N_d*N_a*N_j*shiftdim((0:1:N_z-1),-2); % all the d, with the current aprimeii(ii):aprimeii(ii+1)
            entireRHS_ii=ReturnMatrix_ii+discountedentireEV(daprimez); % autofill e for the expectations
            [Vtempii,maxindex]=max(entireRHS_ii,[],1);
            V(level1ii(ii)+1:level1ii(ii+1)-1,:,:,:)=shiftdim(Vtempii,1);
            Policy(level1ii(ii)+1:level1ii(ii+1)-1,:,:,:)=shiftdim(maxindex+N_d*(loweredge(rem(maxindex-1,N_d)+1+N_d*shiftdim((0:1:N_j-1),-1)+N_d*N_j*shiftdim((0:1:N_z-1),-2)+N_d*N_j*N_z*shiftdim((0:1:N_e-1),-2))-1),1); % loweredge(given the d)
        end
    end
    ...
    for e_c=1:N_e
        ...
        for ii=1:(vfoptions.level1n-1)
            if maxgap(ii)>0
                ...
                entireRHS_ii=ReturnMatrix_ii_e+discountedentireEV(daprimez);
                [Vtempii,maxindex]=max(entireRHS_ii,[],1);
                V(level1ii(ii)+1:level1ii(ii+1)-1,:,:,e_c)=shiftdim(Vtempii,1);
                Policy(level1ii(ii)+1:level1ii(ii+1)-1,:,:,e_c)=shiftdim(maxindex+N_d*(loweredge(rem(maxindex-1,N_d)+1+N_d*shiftdim((0:1:N_j-1),-1)+N_d*N_j*shiftdim((0:1:N_z-1),-2))-1),1); % loweredge(given the d)
            else
                loweredge=maxindex1(:,1,ii,:,:);
                % Just use aprime(ii) for everything
        ...
        for e_c=1:N_e
            ...
            for ii=1:(vfoptions.level1n-1)
                if maxgap(ii)>0
                    ...
                    Policy(level1ii(ii)+1:level1ii(ii+1)-1,:,z_c,e_c)=shiftdim(maxindex+N_d*(loweredge(rem(maxindex-1,N_d)+1+N_d*shiftdim((0:1:N_j-1),-1))-1),1); % loweredge(given the d)
                else
                    loweredge=maxindex1(:,1,ii,:);
                    % Just use aprime(ii) for everything
