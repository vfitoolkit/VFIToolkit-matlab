...
if vfoptions.lowmemory==0
    ...
    linidx_lower=reshape(dind,[1,N_a1*N_a2,N_j,N_z,N_e])                   +N_d2*n2long*shiftdim(gpuArray(0:1:N_a-1),-1)+N_d2*n2long*N_a*shiftdim(gpuArray(0:1:N_j-1),-2)+N_d2*n2long*N_a*N_j*shiftdim(gpuArray(0:1:N_z-1),-3)+N_d2*n2long*N_a*N_j*N_z*shiftdim(gpuArray(0:1:N_e-1),-4);
    linidx_upper=reshape(dind,[1,N_a1*N_a2,N_j,N_z,N_e])+N_d2*(n2long-1)   +N_d2*n2long*shiftdim(gpuArray(0:1:N_a-1),-1)+N_d2*n2long*N_a*shiftdim(gpuArray(0:1:N_j-1),-2)+N_d2*n2long*N_a*N_j*shiftdim(gpuArray(0:1:N_z-1),-3)+N_d2*n2long*N_a*N_j*N_z*shiftdim(gpuArray(0:1:N_e-1),-4);
...
elseif vfoptions.lowmemory==1
    ...
    for e_c=1:N_e
        ...
        % L2 flag to later avoid -Inf ReturnFn (1=all to lower, 2=usual, 3=all to upper)
        L2offset=ceil(maxindexL2/N_d2);
        linidx_lower=reshape(dind,[1,N_a1*N_a2,N_j,N_z])                   +N_d2*n2long*shiftdim(gpuArray(0:1:N_a-1),-1)+N_d2*n2long*N_a*shiftdim(gpuArray(0:1:N_j-1),-2)+N_d2*n2long*N_a*N_j*shiftdim(gpuArray(0:1:N_z-1),-3);
        linidx_upper=reshape(dind,[1,N_a1*N_a2,N_j,N_z])+N_d2*(n2long-1)   +N_d2*n2long*shiftdim(gpuArray(0:1:N_a-1),-1)+N_d2*n2long*N_a*shiftdim(gpuArray(0:1:N_j-1),-2)+N_d2*n2long*N_a*N_j*shiftdim(gpuArray(0:1:N_z-1),-3);
        isInfLower=(ReturnMatrix_ii(linidx_lower)==-Inf);
        isInfUpper=(ReturnMatrix_ii(linidx_upper)==-Inf);
        inLowerStrict=(L2offset>=2)         & (L2offset<=n2short+1);
        inUpperStrict=(L2offset>=n2short+3) & (L2offset<=n2long-1);
