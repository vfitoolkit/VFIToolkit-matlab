...
if vfoptions.EVpre==0
    ...
    EVpre=[sum(V(N_a+1:end,:).*repelem(reshape(pi_e_J,[N_j,1,N_e]),N_a-1,1),3); zeros(N_a,N_z,'gpuArray')]; % I use zeros in j=N_j so that can just use pi_z_J to create expectations
...
elseif vfoptions.EVpre==1
    ...
    EVpre=sum(V.*repelem(reshape(pi_e_J,[N_j,1,N_e]),N_a,1,1),3);
