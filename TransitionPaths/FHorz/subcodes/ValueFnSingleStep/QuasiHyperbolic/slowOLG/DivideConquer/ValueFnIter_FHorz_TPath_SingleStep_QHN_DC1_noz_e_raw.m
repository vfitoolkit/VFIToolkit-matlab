function [V,Policy,Policyalt]=ValueFnIter_FHorz_TPath_SingleStep_QHN_DC1_noz_e_raw(V,n_d,n_a,n_e,N_j, d_gridvals, a_grid, e_gridvals_J, pi_e_J, ReturnFn, Parameters, DiscountFactorParamNames, ReturnFnParamNames, vfoptions)
...
for reverse_j=1:N_j-1
    ...
    if vfoptions.lowmemory==0
        ...
        [~,maxindex1]=max(entireRHS_ii,[],2);
        [~,maxindex2]=max(reshape(entireRHS_ii,[N_d*N_a,vfoptions.level1n,N_e]),[],1);
    ...
    elseif vfoptions.lowmemory==1
        for e_c=1:N_e
            ...
            entireRHS_ii=ReturnMatrix_ii+beta0beta*shiftdim(EV,-1);
            [~,maxindex1]=max(entireRHS_ii,[],2);
            [~,maxindex2]=max(reshape(entireRHS_ii,[N_d*N_a,vfoptions.level1n]),[],1);
