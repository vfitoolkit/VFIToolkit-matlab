...
if strcmp(vfoptions.exoticpreferences,'None')
    if vfoptions.lowmemory==0
        if N_d==0
            [VKron,PolicyKron]=ValueFnIter_Case1_FHorz_TPath_SingleStep_fastOLG_nod_noz_raw(VKron,n_a, N_j, a_grid, ReturnFn, Parameters, DiscountFactorParamNames, ReturnFnParamNames, vfoptions);
        else
            [VKron, PolicyKron]=ValueFnIter_Case1_FHorz_TPath_SingleStep_fastOLG_noz_raw(VKron,n_d,n_a, N_j, d_grid, a_grid, ReturnFn, Parameters, DiscountFactorParamNames, ReturnFnParamNames, vfoptions);
        end
    elseif vfoptions.lowmemory==1 
        error('vfoptions.lowmemory=1 is not an options for transition path with no z variables')
%         if N_d==0
...
elseif strcmp(vfoptions.exoticpreferences,'QuasiHyperbolic')
    error('Cannot yet use Quasi-hyperbolic discounting with no z, email me')
...
elseif strcmp(vfoptions.exoticpreferences,'EpsteinZin')
    error('Cannot yet use Epstein-Zin preferences with no z, email me')
%     if N_d==0
