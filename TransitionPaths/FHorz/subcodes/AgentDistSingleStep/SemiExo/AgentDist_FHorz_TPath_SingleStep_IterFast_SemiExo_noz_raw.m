...
Policy_aprime=gather(reshape(Policy_aprime(:,1:N_j-1),[N_AS*(N_j-1),1]));

% semizindex_short indexes pi_semiz_J_short and idxshort, which are [N_semiz,N_semizshort,N_dsemiz,N_j]
semizindexbase=repmat(repelem((1:1:N_semiz)',N_a,1),N_j-1,1)+N_semiz*(0:1:N_semizshort-1); % [N_AS*(N_j-1),N_semizshort]
jtermsemiz=(N_semiz*N_semizshort*N_dsemiz)*repelem((0:1:N_j-2)',N_AS,1); % age term for indexing pi_semiz [N_AS*(N_j-1),1]
destjblock=repelem(N_AS*(0:1:N_j-2)',N_AS,1); % dest age-block offset (j preserved; the j->j+1 shift is done by writing to exceptfirstj)
...
II2=repelem((1:1:N_AS*(N_j-1))',1,N_semizshort);
Gammatranspose=sparse(Policy_aprimesemiz,II2,semiztransitions,N_AS*(N_j-1),N_AS*(N_j-1)); % From (a,semiz,j) to (a',semiz',j)

% Index sets for the fastOLG age-shift
exceptlastj=(1:1:N_AS*(N_j-1))'; % (a,semiz,j=1..N_j-1) are the first N_AS*(N_j-1) entries
exceptfirstj=N_AS+(1:1:N_AS*(N_j-1))'; % (a,semiz,j=2..N_j)
justfirstj=(1:1:N_AS)'; % (a,semiz,j=1)
