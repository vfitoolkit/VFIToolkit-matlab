...
Policy_aprime=reshape(Policy_aprime,[N_AS,N_z,N_j]);
Policy_aprime=gather(reshape(permute(Policy_aprime(:,:,1:N_j-1),[1,3,2]),[N_AS*(N_j-1)*N_z,1]));

% semizindex_short indexes pi_semiz_J_short and idxshort, which are [N_semiz,N_semizshort,N_dsemiz,N_j]
semizindexbase=repmat(repelem((1:1:N_semiz)',N_a,1),(N_j-1)*N_z,1)+N_semiz*(0:1:N_semizshort-1); % [N_AS*(N_j-1)*N_z,N_semizshort]
jtermsemiz=(N_semiz*N_semizshort*N_dsemiz)*repmat(repelem((0:1:N_j-2)',N_AS,1),N_z,1); % age term for indexing pi_semiz
destoffset=repmat(repelem(N_AS*(0:1:N_j-2)',N_AS,1),N_z,1)+repelem(N_AS*(N_j-1)*(0:1:N_z-1)',N_AS*(N_j-1),1); % dest (j,z) block offset
...
II2=repelem((1:1:N_AS*(N_j-1)*N_z)',1,N_semizshort);
Gammatranspose=sparse(Policy_aprimesemizz,II2,semiztransitions,N_AS*(N_j-1)*N_z,N_AS*(N_j-1)*N_z); % From (a,semiz,j,z) to (a',semiz',j,z)
