...
Policy_aprime=permute(reshape(Policy_aprime,[N_asemiz,N_probs,N_j]),[1,3,2]); % [N_asemiz,N_j,N_probs]
Policy_aprime=gather(reshape(Policy_aprime(:,1:N_j-1,:),[N_asemiz*(N_j-1),N_probs]));
PolicyProbs=permute(reshape(PolicyProbs,[N_asemiz,N_probs,N_j]),[1,3,2]); % [N_asemiz,N_j,N_probs]
PolicyProbs=gather(reshape(PolicyProbs(:,1:N_j-1,:),[N_asemiz*(N_j-1),N_probs]));

semizindexbase=repmat(repelem((1:1:N_semiz)',N_a,1),N_j-1,1)+N_semiz*(0:1:N_semizshort-1);
jtermsemiz=(N_semiz*N_semizshort*N_dsemiz)*repelem((0:1:N_j-2)',N_asemiz,1);
destjblock=repelem(N_asemiz*(0:1:N_j-2)',N_asemiz,1);
