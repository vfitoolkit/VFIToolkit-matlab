function AgentDist=AgentDist_FHorz_TPath_SingleStep_Iteration_nProbs_SemiExo_raw(AgentDist,Policy_dsemiexo,Policy_aprime,PolicyProbs,N_probs,N_dsemiz,N_a,N_semiz,N_z,N_j,pi_semiz_J,pi_z_J,jequalOneDist)
% One time-step of the agent distribution iteration, with semi-exogenous state and markov z (no e), grid interpolation layer (N_probs points).
...
II2=repelem((1:1:N_a*N_bothz)',1,N_semizshort*N_probs);
...
for jjr=1:(N_j-1)
    ...
    Gammatranspose=sparse(Policy_aprimesemizz_jj,II2,PolicyProbs_jj,N_a*N_bothz,N_a*N_bothz); % sparse() accumulates at repeated indices
