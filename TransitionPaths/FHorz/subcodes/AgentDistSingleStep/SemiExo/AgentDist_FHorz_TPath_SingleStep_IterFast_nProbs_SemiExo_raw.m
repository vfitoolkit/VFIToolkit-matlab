...
Policy_aprime=permute(reshape(Policy_aprime,[N_asemiz,N_z,N_probs,N_j]),[1,4,2,3]); % [N_asemiz,N_j,N_z,N_probs]
Policy_aprime=gather(reshape(Policy_aprime(:,1:N_j-1,:,:),[N_asemiz*(N_j-1)*N_z,N_probs]));
PolicyProbs=permute(reshape(PolicyProbs,[N_asemiz,N_z,N_probs,N_j]),[1,4,2,3]); % [N_asemiz,N_j,N_z,N_probs]
PolicyProbs=gather(reshape(PolicyProbs(:,1:N_j-1,:,:),[N_asemiz*(N_j-1)*N_z,N_probs]));

semizindexbase=repmat(repelem((1:1:N_semiz)',N_a,1),(N_j-1)*N_z,1)+N_semiz*(0:1:N_semizshort-1);
jtermsemiz=(N_semiz*N_semizshort*N_dsemiz)*repmat(repelem((0:1:N_j-2)',N_asemiz,1),N_z,1);
destoffset=repmat(repelem(N_asemiz*(0:1:N_j-2)',N_asemiz,1),N_z,1)+repelem(N_asemiz*(N_j-1)*(0:1:N_z-1)',N_asemiz*(N_j-1),1);
...
II2=repelem((1:1:N_asemiz*(N_j-1)*N_z)',1,N_semizshort*N_probs);
Gammatranspose=sparse(Policy_aprimesemizz,II2,PolicyProbs_comb,N_asemiz*(N_j-1)*N_z,N_asemiz*(N_j-1)*N_z); % sparse() accumulates at repeated indices
