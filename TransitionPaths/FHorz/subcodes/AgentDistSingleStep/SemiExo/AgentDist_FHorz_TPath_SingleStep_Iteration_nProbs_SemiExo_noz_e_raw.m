function AgentDist=AgentDist_FHorz_TPath_SingleStep_Iteration_nProbs_SemiExo_noz_e_raw(AgentDist,Policy_dsemiexo,Policy_aprime,PolicyProbs,N_probs,N_dsemiz,N_a,N_semiz,N_e,N_j,pi_semiz_J,pi_e_J,jequalOneDist)
% One time-step of the agent distribution iteration, with semi-exogenous state and iid e (no z), grid interpolation layer (N_probs points).
...
II2=repelem((1:1:N_a*N_semiz*N_e)',1,N_semizshort*N_probs);
...
for jjr=1:(N_j-1)
    ...
    Gammatranspose=sparse(Policy_aprimesemiz_jj,II2,PolicyProbs_jj,N_a*N_semiz,N_a*N_semiz*N_e); % From (a,semiz,e) to (a',semiz'); sparse() accumulates at repeated indices
