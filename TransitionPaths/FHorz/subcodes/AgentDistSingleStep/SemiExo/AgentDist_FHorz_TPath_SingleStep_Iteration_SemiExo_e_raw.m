function AgentDist=AgentDist_FHorz_TPath_SingleStep_Iteration_SemiExo_e_raw(AgentDist,Policy_dsemiexo,Policy_aprime,N_dsemiz,N_a,N_semiz,N_z,N_e,N_j,pi_semiz_J,pi_z_J,pi_e_J,jequalOneDist)
% One time-step of the agent distribution iteration, with semi-exogenous state, markov z, and iid e.
...
% Policy_dsemiexo and Policy_aprime are [N_a*N_semiz*N_z*N_e,N_j]
% semiz->semiz' (depends on d2) is folded into Gammatranspose (which also sums out e); z->z' is the Tan second step; e' is added via kron(pi_e,.)
...
% semizindex_short_jj (built per age below) is [N_a*N_semiz*N_z*N_e,N_semizshort]
...
for jjr=1:(N_j-1)
    ...
    Gammatranspose=sparse(Policy_aprimesemizz_jj,II2,semiztransitions_jj,N_a*N_bothz,N_a*N_bothz*N_e); % From (a,semiz,z,e) to (a',semiz',z)
