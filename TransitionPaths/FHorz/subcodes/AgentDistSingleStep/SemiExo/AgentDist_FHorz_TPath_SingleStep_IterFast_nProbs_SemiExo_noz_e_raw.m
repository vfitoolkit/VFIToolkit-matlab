...
Policy_aprime=permute(reshape(Policy_aprime,[N_AS,N_e,N_probs,N_j]),[1,4,2,3]); % [N_AS,N_j,N_e,N_probs]
Policy_aprime=gather(reshape(Policy_aprime(:,1:N_j-1,:,:),[N_AS*(N_j-1)*N_e,N_probs]));
PolicyProbs=permute(reshape(PolicyProbs,[N_AS,N_e,N_probs,N_j]),[1,4,2,3]); % [N_AS,N_j,N_e,N_probs]
PolicyProbs=gather(reshape(PolicyProbs(:,1:N_j-1,:,:),[N_AS*(N_j-1)*N_e,N_probs]));
...
destjblock=repmat(repelem(N_AS*(0:1:N_j-2)',N_AS,1),N_e,1);
...
II2=repelem((1:1:N_AS*(N_j-1)*N_e)',1,N_semizshort*N_probs);
Gammatranspose=sparse(Policy_aprimesemiz,II2,PolicyProbs_comb,N_AS*(N_j-1),N_AS*(N_j-1)*N_e); % From (a,semiz,j,e) to (a',semiz',j); sparse() accumulates
