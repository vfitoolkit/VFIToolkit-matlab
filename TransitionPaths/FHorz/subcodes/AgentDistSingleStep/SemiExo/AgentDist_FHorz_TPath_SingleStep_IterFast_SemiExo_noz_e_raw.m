...
Policy_aprime=reshape(Policy_aprime,[N_asemiz,N_e,N_j]);
Policy_aprime=gather(reshape(permute(Policy_aprime(:,:,1:N_j-1),[1,3,2]),[N_asemiz*(N_j-1)*N_e,1]));
...
destjblock=repmat(repelem(N_asemiz*(0:1:N_j-2)',N_asemiz,1),N_e,1); % dest age-block offset (e summed out)
...
II2=repelem((1:1:N_asemiz*(N_j-1)*N_e)',1,N_semizshort);
