...
Policy_dsemiexo=reshape(Policy_dsemiexo,[N_asemiz,N_z,N_e,N_j]);
Policy_dsemiexo=gather(reshape(permute(Policy_dsemiexo(:,:,:,1:N_j-1),[1,4,2,3]),[N_asemiz*(N_j-1)*N_z*N_e,1]));
Policy_aprime=permute(reshape(Policy_aprime,[N_asemiz,N_z,N_e,N_probs,N_j]),[1,5,2,3,4]); % [N_asemiz,N_j,N_z,N_e,N_probs]
Policy_aprime=gather(reshape(Policy_aprime(:,1:N_j-1,:,:,:),[N_asemiz*(N_j-1)*N_z*N_e,N_probs]));
PolicyProbs=permute(reshape(PolicyProbs,[N_asemiz,N_z,N_e,N_probs,N_j]),[1,5,2,3,4]); % [N_asemiz,N_j,N_z,N_e,N_probs]
PolicyProbs=gather(reshape(PolicyProbs(:,1:N_j-1,:,:,:),[N_asemiz*(N_j-1)*N_z*N_e,N_probs]));
...
destoffset=repmat(repmat(repelem(N_asemiz*(0:1:N_j-2)',N_asemiz,1),N_z,1)+repelem(N_asemiz*(N_j-1)*(0:1:N_z-1)',N_asemiz*(N_j-1),1),N_e,1);
...
II2=repelem((1:1:N_asemiz*(N_j-1)*N_z*N_e)',1,N_semizshort*N_probs);
Gammatranspose=sparse(Policy_aprimesemizz,II2,PolicyProbs_comb,N_asemiz*(N_j-1)*N_z,N_asemiz*(N_j-1)*N_z*N_e); % From (a,semiz,j,z,e) to (a',semiz',j,z); sparse() accumulates

exceptlastj=repmat((1:1:N_asemiz)',(N_j-1)*N_z*N_e,1)+repmat(repelem(N_asemiz*(0:1:N_j-2)',N_asemiz,1),N_z*N_e,1)+repelem(N_asemiz*N_j*(0:1:N_z*N_e-1)',N_asemiz*(N_j-1),1);
exceptfirstj=repmat((1:1:N_asemiz)',(N_j-1)*N_z*N_e,1)+repmat(repelem(N_asemiz*(1:1:N_j-1)',N_asemiz,1),N_z*N_e,1)+repelem(N_asemiz*N_j*(0:1:N_z*N_e-1)',N_asemiz*(N_j-1),1);
justfirstj=repmat((1:1:N_asemiz)',N_z*N_e,1)+repelem(N_asemiz*N_j*(0:1:N_z*N_e-1)',N_asemiz,1);

% Tan improvement Step 1 (e summed out by Gammatranspose)
AgentDist_tt=sparse(gather(reshape(AgentDist(exceptlastj),[N_asemiz*(N_j-1)*N_z*N_e,1])));
