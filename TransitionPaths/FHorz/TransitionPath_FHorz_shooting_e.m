...
while PricePathDist>transpathoptions.tolerance && pathcounter<=transpathoptions.maxiter
    PolicyPath=zeros(2,N_a,N_z,N_e,N_j,T-1,'gpuArray'); % Periods 1 to T-1
    ...
    for tt=1:T-1
        ...
        Policy_fastOLG_d=reshape(permute(Policy(1,:,:,:,:),[1,2,5,3,4]),[1,N_a*N_j*N_z*N_e]);
        Policy_fastOLG_aprime=reshape(permute(Policy(2,:,:,:,:),[1,2,5,3,4]),[1,N_a*N_j*N_z*N_e]);
        AggVars=EvalFnOnAgentDist_AggVars_FHorz_fastOLG(AgentDist.*AgeWeights,Policy_fastOLG_d, Policy_fastOLG_aprime, FnsToEvaluate,FnsToEvaluateParamNames,AggVarNames,Parameters,N_j,l_d,l_a,l_a,l_ze,N_a,N_ze,d_gridvals,aprime_gridvals,a_gridvals,ze_gridvals_J,1);
        ...
        if simoptions.fastOLG==0
            AgentDist=StationaryDist_FHorz_Case1_TPath_SingleStep_Iteration_e_raw(AgentDist,Policy(2,:,:,:,1:end-1),N_a,N_z,N_e,N_j,pi_z_J,pi_e_J,jequalOneDist);
        else % simoptions.fastOLG==1
            AgentDist=StationaryDist_FHorz_Case1_TPath_SingleStep_IterFast_e_raw(AgentDist,Policy_fastOLG_aprime(exceptlastj),N_a,N_z,N_e,N_j,pi_z_J_sim,pi_e_J_sim,exceptlastj,exceptfirstj,justfirstj,jequalOneDist);
        end
    end
