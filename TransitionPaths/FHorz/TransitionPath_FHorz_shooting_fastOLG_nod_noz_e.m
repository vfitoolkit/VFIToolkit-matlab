...
if vfoptions.gridinterplayer==0
    PolicyIndexesPath=zeros(l_aprime,N_a,N_j,N_e,T-1,'gpuArray'); %Periods 1 to T-1
elseif vfoptions.gridinterplayer==1
    PolicyIndexesPath=zeros(l_aprime+1,N_a,N_j,N_e,T-1,'gpuArray'); %Periods 1 to T-1
...
while PricePathDist>transpathoptions.tolerance && pathcounter<=transpathoptions.maxiter
    ...
    for ttr=1:T-1 %so tt=T-ttr
        ...
        [V, Policy]=ValueFnIter_FHorz_TPath_SingleStep_fastOLG_noz_e(V,0,n_a,n_e,N_j,[], a_grid, e_gridvals_J, pi_e_J, ReturnFn, Parameters, DiscountFactorParamNames, ReturnFnParamNames, vfoptions);
    ...
    if isscalar(n_a)
        PolicyaprimePath=reshape(PolicyIndexesPath(1,:,1:N_j-1,:,:),[N_a*(N_j-1)*N_e,T-1]); % aprime index
    elseif length(n_a)==2
        PolicyaprimePath=reshape(PolicyIndexesPath(1,:,1:N_j-1,:,:)+n_a(1)*(PolicyIndexesPath(2,:,1:N_j-1,:,:)-1),[N_a*(N_j-1)*N_e,T-1]);
    elseif length(n_a)==3
        PolicyaprimePath=reshape(PolicyIndexesPath(1,:,1:N_j-1,:,:)+n_a(1)*(PolicyIndexesPath(2,:,1:N_j-1,:,:)-1)+n_a(1)*n_a(2)*(PolicyIndexesPath(3,:,1:N_j-1,:,:)-1),[N_a*(N_j-1)*N_e,T-1]);
    elseif length(n_a)==4
        PolicyaprimePath=reshape(PolicyIndexesPath(1,:,1:N_j-1,:,:)+n_a(1)*(PolicyIndexesPath(2,:,1:N_j-1,:,:)-1)+n_a(1)*n_a(2)*(PolicyIndexesPath(3,:,1:N_j-1,:,:)-1)+n_a(1)*n_a(2)*n_a(3)*(PolicyIndexesPath(4,:,1:N_j-1,:,:)-1),[N_a*(N_j-1)*N_e,T-1]);
    end
    PolicyaprimejPath=PolicyaprimePath+repmat(repelem(N_a*gpuArray(0:1:(N_j-1)-1)',N_a,1),N_e,1);
    if simoptions.gridinterplayer==1
        L2index=reshape(PolicyIndexesPath(l_aprime+1,:,1:N_j-1,:,:),[N_a*(N_j-1)*N_e,1,T-1]); % PolicyIndexesPath is of size [l_d+l_aprime+1,N_a,N_j,N_e,T-1]
    ...
    PolicyValuesPath=PolicyInd2Val_FHorz_TPath(PolicyIndexesPath,0,n_a,n_e,N_j,T-1,[],aprime_gridvals,vfoptions,1,1); % [size(PolicyValuesPath,1),N_a,N_j,N_e,T]
    PolicyValuesPath=permute(PolicyValuesPath,[2,3,4,1,5]); %[N_a,N_j,N_e,l_aprime,T-1] % fastOLG ordering is needed for AggVars
    ...
    for tt=1:T-1
        ...
        if simoptions.gridinterplayer==0
            AgentDistnext=AgentDist_FHorz_TPath_SingleStep_IterFast_noz_e_raw(AgentDist,PolicyaprimejPath(:,tt),N_a,N_e,N_j,pi_e_J_sim,II1orII,II2,exceptlastj,exceptfirstj,justfirstj,jequalOneDist); % II1orII is II1
        elseif simoptions.gridinterplayer==1
            AgentDistnext=AgentDist_FHorz_TPath_SingleStep_IterFast_nProbs_noz_e_raw(AgentDist,PolicyaprimejPath(:,:,tt),PolicyProbsPath(:,:,tt),N_a,N_e,N_j,pi_e_J_sim,II1orII,exceptlastj,exceptfirstj,justfirstj,jequalOneDist); % II1orII is II
        end
