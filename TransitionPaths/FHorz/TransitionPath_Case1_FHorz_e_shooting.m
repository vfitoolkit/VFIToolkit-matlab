...
for ff=1:length(AggVarNames)
    ...
    if length(temp)>(l_d+l_a+l_a+l_z+l_e)
        FnsToEvaluateParamNames(ff).Names={temp{l_d+l_a+l_a+l_z+l_e+1:end}}; % the first inputs will always be (d,aprime,a,z,e)
    else
...
while PricePathDist>transpathoptions.tolerance && pathcounter<transpathoptions.maxiterations
    ...
    for ttr=1:T-1 %so t=T-i
        ...
        if transpathoptions.epathtrivial==0
            vfoptions.pi_e_J=transpathoptions.pi_e_J_T(:,:,T-ttr);
