...
if vfoptions.gridinterplayer==0
    PolicyIndexesPath=zeros(l_aprime,N_a,N_e,N_j,T-1,'gpuArray'); %Periods 1 to T-1
elseif vfoptions.gridinterplayer==1
    PolicyIndexesPath=zeros(l_aprime+1,N_a,N_e,N_j,T-1,'gpuArray'); %Periods 1 to T-1
end
...
while PricePathDist>transpathoptions.tolerance && pathcounter<=transpathoptions.maxiter
    ...
    for ttr=1:T-1 %so tt=T-ttr
        ...
        [V, Policy]=ValueFnIter_FHorz_TPath_SingleStep_noz_e(V,0,n_a,n_e,N_j,[], a_grid, e_gridvals_J, pi_e_J, ReturnFn, Parameters, DiscountFactorParamNames, ReturnFnParamNames, vfoptions);
    ...
    if isscalar(n_a)
        PolicyaprimePath=reshape(PolicyIndexesPath(1,:,:,1:N_j-1,:),[N_a*N_e,N_j-1,T-1]); % aprime index
    elseif length(n_a)==2
        PolicyaprimePath=reshape(PolicyIndexesPath(1,:,:,1:N_j-1,:)+n_a(1)*(PolicyIndexesPath(2,:,:,1:N_j-1,:)-1),[N_a*N_e,N_j-1,T-1]);
    elseif length(n_a)==3
        PolicyaprimePath=reshape(PolicyIndexesPath(1,:,:,1:N_j-1,:)+n_a(1)*(PolicyIndexesPath(2,:,:,1:N_j-1,:)-1)+n_a(1)*n_a(2)*(PolicyIndexesPath(3,:,:,1:N_j-1,:)-1),[N_a*N_e,N_j-1,T-1]);
    elseif length(n_a)==4
        PolicyaprimePath=reshape(PolicyIndexesPath(1,:,:,1:N_j-1,:)+n_a(1)*(PolicyIndexesPath(2,:,:,1:N_j-1,:)-1)+n_a(1)*n_a(2)*(PolicyIndexesPath(3,:,:,1:N_j-1,:)-1)+n_a(1)*n_a(2)*n_a(3)*(PolicyIndexesPath(4,:,:,1:N_j-1,:)-1),[N_a*N_e,N_j-1,T-1]);
    end
    if simoptions.fastOLG==0
    ...
    elseif simoptions.fastOLG==1
        ...
        if simoptions.gridinterplayer==1
            L2index=reshape(PolicyIndexesPath(l_aprime+1,:,:,1:N_j-1,:),[1,N_a,N_e,N_j-1,T-1]); % PolicyIndexesPath is of size [l_d+l_aprime+1,N_a,N_e,N_j,T]
    ...
    PolicyValuesPath=PolicyInd2Val_FHorz_TPath(PolicyIndexesPath,0,n_a,n_e,N_j,T-1,[],a_grid,vfoptions,1,0);
    PolicyValuesPath=permute(PolicyValuesPath,[2,4,3,1,5]); %[N_a,N_j,N_e,l_aprime,T-1] % fastOLG ordering is needed for AggVars
    ...
    for tt=1:T-1
        ...
        if transpathoptions.epathtrivial==0
            e_gridvals_J_fastOLG=transpathoptions.ze_gridvals_J_fastOLG(:,:,:,tt);
            if simoptions.fastOLG==0
        ...
        AgeWeights=AgeWeights_T(:,:,tt); % By coincidence, this is same for fastOLG=0,1
        ...
        if simoptions.fastOLG==0
            AgentDistnext=AgentDist_FHorz_TPath_SingleStep_Iteration_noz_e_raw(AgentDist,PolicyaprimePath_slowOLG(:,:,tt),N_a,N_e,N_j,pi_e_J,II1,II2,jequalOneDist);
        else % simoptions.fastOLG==1
            if simoptions.gridinterplayer==0
                AgentDistnext=AgentDist_FHorz_TPath_SingleStep_IterFast_noz_e_raw(AgentDist,PolicyaprimejPath(:,tt),N_a,N_e,N_j,pi_e_J_sim,II1,II2,exceptlastj,exceptfirstj,justfirstj,jequalOneDist);
            elseif simoptions.gridinterplayer==1
                AgentDistnext=AgentDist_FHorz_TPath_SingleStep_IterFast_nProbs_noz_e_raw(AgentDist,PolicyaprimejPath(:,:,tt),PolicyProbsPath(:,:,tt),N_a,N_e,N_j,pi_e_J_sim,II,exceptlastj,exceptfirstj,justfirstj,jequalOneDist);   
            end
        end

        %% AggVars
        AggVars=EvalFnOnAgentDist_AggVars_FHorz_fastOLG(AgentDist.*AgeWeights,[], PolicyValuesPath(:,:,:,:,tt), FnsToEvaluateCell,FnsToEvaluateParamNames,AggVarNames,Parameters,N_j,0,l_a,l_a,l_e,N_a,N_e,a_gridvals,e_gridvals_J_fastOLG,1);
