...
while PricePathDist>transpathoptions.tolerance && pathcounter<=transpathoptions.maxiter
    ...
    PolicyPath=zeros(1,N_a,N_j,N_z,N_e,T-1,'gpuArray'); %Periods 1 to T-1
    ...
    for ttr=1:T-1 %so tt=T-ttr
        ...
        PolicyPath(1,:,:,:,:,T-ttr)=Policy; % fastOLG: so a-by-j-by-z-by-e
    ...
    for tt=1:T-1
        ...
        Policy=PolicyPath(1,:,:,:,:,tt); % fastOLG: so (a,j)-by-z-by-e
        ...
        AggVars=EvalFnOnAgentDist_AggVars_FHorz_fastOLG(AgentDist.*AgeWeights_T(:,:,tt),[], Policy, FnsToEvaluate,FnsToEvaluateParamNames,AggVarNames,Parameters,N_j,0,l_a,l_a,l_ze,N_a,N_ze,[],aprime_gridvals,a_gridvals,ze_gridvals_J,1);
        ...
        AgentDist=StationaryDist_FHorz_Case1_TPath_SingleStep_IterFast_e_raw(AgentDist,reshape(Policy(1,:,1:end-1,:),[1,N_a*(N_j-1)*N_z*N_e]),N_a,N_z,N_e,N_j,pi_z_J_sim,pi_e_J_sim,exceptlastj,exceptfirstj,justfirstj,jequalOneDist);
    end    
