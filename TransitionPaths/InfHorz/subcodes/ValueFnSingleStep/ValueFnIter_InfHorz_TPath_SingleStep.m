        ...
        elseif length(n_a)==2
            if vfoptions.level1n(2)==n_a(2) % Don't bother with divide-and-conquer on the second endogenous state
                ...
                if N_d==0
                    ...
                    [VKron, PolicyKron]=ValueFnIter_InfHorz_TPath_SingleStep_DC2B_raw(VKron,n_d,n_a,n_z, d_gridvals, a_grid, z_gridvals, pi_z, ReturnFn, Parameters, DiscountFactorParamNames, ReturnFnParamNames, vfoptions);
                end
                ...
                % else
                %     [VKron, PolicyKron]=ValueFnIter_InfHorz_TPath_SingleStep_DC2_raw(VKron,n_d,n_a,n_z, d_gridvals, a_grid, z_gridvals, pi_z, ReturnFn, Parameters, DiscountFactorParamNames, ReturnFnParamNames, vfoptions);
                % end
            end
        else
    ...
    elseif vfoptions.divideandconquer==1
        if isscalar(n_a)
            if N_d==0
                ...
                [VKron, PolicyKron]=ValueFnIter_InfHorz_TPath_SingleStep_DC1_GI_raw(VKron,n_d,n_a,n_z, d_gridvals, a_grid, z_gridvals, pi_z, ReturnFn, Parameters, DiscountFactorParamNames, ReturnFnParamNames, vfoptions);
            end
        elseif length(n_a)==2
            if vfoptions.level1n(2)==n_a(2) % Don't bother with divide-and-conquer on the second endogenous state
                vfoptions.level1n=vfoptions.level1n(1); % Only first one is relevant for DC2B
                if N_d==0
                    ...
                    [VKron, PolicyKron]=ValueFnIter_InfHorz_TPath_SingleStep_DC2B_GI_raw(VKron,n_d,n_a,n_z, d_gridvals, a_grid, z_gridvals, pi_z, ReturnFn, Parameters, DiscountFactorParamNames, ReturnFnParamNames, vfoptions);
                end
            else
                error('With two endogenous states, can only do divide-and-conquer in the first endogenous state (not in both)')
            end
