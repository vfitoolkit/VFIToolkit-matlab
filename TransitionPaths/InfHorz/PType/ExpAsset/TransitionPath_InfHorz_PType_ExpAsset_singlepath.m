function AggVarsPath=TransitionPath_InfHorz_PType_ExpAsset_singlepath(PricePathOld, ParamPath, PricePathNames, ParamPathNames, T, V_final, AgentDist_initial, ...
    l_d,n_d1,n_d2,n_a1,n_a2,N_z,n_z,d2_grid,a1_gridvals,a2_grid,d_gridvals,a_gridvals,z_grid,pi_z, ...
    ReturnFn, aprimeFn, FnsToEvaluate, Parameters, DiscountFactorParamNames, ReturnFnParamNames, aprimeFnParamNames, FnsToEvaluateParamNames, AggVarNames, ...
    PricePathSizeVec, ParamPathSizeVec, use_tminus1price, use_tminus1params, use_tplus1price, use_tminus1AggVars, tminus1priceNames, tminus1paramNames, tplus1priceNames, tminus1AggVarsNames, ...
...
for ttr=1:T-1 %so t=T-i
    ...
    [V, Policy]=ValueFnIter_InfHorz_TPath_SingleStep_ExpAsset(Vnext,n_d1,n_d2,n_a1,n_a2,n_z,d_gridvals, d2_grid, a1_gridvals, a2_grid, z_grid, pi_z, ReturnFn, aprimeFn, Parameters, DiscountFactorParamNames, ReturnFnParamNames, aprimeFnParamNames, vfoptions);
...
elseif length(n_a1)==3
    PolicyaprimePath=reshape(PolicyIndexesPath(l_d+1,:,:,:)+n_a1(1)*(PolicyIndexesPath(l_d+2,:,:,:)-1)+prod(n_a1(1:2))*(PolicyIndexesPath(l_d+3,:,:,:)-1),[N_a*N_z,T-1]);
elseif length(n_a1)==41
    PolicyaprimePath=reshape(PolicyIndexesPath(l_d+1,:,:,:)+n_a1(1)*(PolicyIndexesPath(l_d+2,:,:,:)-1)+prod(n_a1(1:2))*(PolicyIndexesPath(l_d+3,:,:,:)-1)+prod(n_a1(1:3))*(PolicyIndexesPath(l_d+4,:,:,:)-1),[N_a*N_z,T-1]);
end
...
for tt=1:T-1
    ...
    %% AggVars
    AggVars_Means=EvalFnOnAgentDist_InfHorz_TPath_SingleStep_AggVars(gpuArray(full(AgentDist)), PolicyValuesPath(:,:,:,tt), FnsToEvaluate, Parameters, FnsToEvaluateParamNames, AggVarNames, [n_a1,n_a2], n_z, a_gridvals, z_grid,1);
