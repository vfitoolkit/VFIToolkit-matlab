...
while itercounter<=transpathoptions.maxiter % convergence is tested further down, at the point where the distances are known, so that the loop stops on the path it just evaluated
    ...
    if any(~isfinite(GEcondnPath(1:T-1,:)),'all')
        error(['TransitionPath_MixHorz_PType_shooting: the general eqm conditions are NaN/Inf at iteration %i. ' ...
...
if converged==0
    warning(['TransitionPath_MixHorz_PType_shooting: reached maxiter (%i) without convergence; the general eqm ' ...
