function [VKron, PolicyKron]=ValueFnIter_Case1_EndoType_TPath_SingleStep(VKron,n_d,n_endostate,n_endotype,n_z,d_grid, endostate_grid, endotype_grid, z_grid, pi_z, ReturnFn, Parameters, DiscountFactorParamNames, ReturnFnParamNames, vfoptions)
...
if vfoptions.parallel==2 
   ...
   endostate_grid=gpuArray(endostate_grid);
   endotype_grid=gpuArray(endotype_grid);
   ...
   endostate_grid=gather(endostate_grid);
   endotype_grid=gather(endotype_grid);
...
if N_d==0
    error('vfoptions.endotype cannot yet be used without decision (d) variables in transition paths (email me if you need this)')
%     [VKron,PolicyKron]=ValueFnIter_Case1_TPath_SingleStep_no_d_raw(VKron,n_a, n_z, a_grid, z_grid, pi_z, ReturnFn, Parameters, DiscountFactorParamNames, ReturnFnParamNames, vfoptions);
    ...
    if strcmp(vfoptions.solnmethod,'purediscretization')
        error('vfoptions.endotype cannot yet be used with purediscretization for vfoptions.solnmethod in transition paths (email me if you need this)')
%         [VKron, PolicyKron]=ValueFnIter_Case1_TPath_SingleStep_raw(VKron,n_d,n_a,n_z, d_grid, a_grid, z_grid, pi_z, ReturnFn, Parameters, DiscountFactorParamNames, ReturnFnParamNames, vfoptions);
    elseif strcmp(vfoptions.solnmethod,'purediscretization_refinement')
        error('vfoptions.endotype cannot yet be used with purediscretization_refinement for vfoptions.solnmethod in transition paths (email me if you need this)')
%         [VKron, PolicyKron]=ValueFnIter_Case1_TPath_SingleStep_Refine_raw(VKron,n_d,n_a,n_z, d_grid, a_grid, z_grid, pi_z, ReturnFn, Parameters, DiscountFactorParamNames, ReturnFnParamNames, vfoptions);
    elseif strcmp(vfoptions.solnmethod,'purediscretization_refinement2')
        [VKron, PolicyKron]=ValueFnIter_Case1_EndoType_TPath_SingleStep_Refine2_raw(VKron,n_d,n_endostate,n_endotype,n_z,d_grid, endostate_grid, endotype_grid, z_grid, pi_z, ReturnFn, Parameters, DiscountFactorParamNames, ReturnFnParamNames, vfoptions);      
    end
end
