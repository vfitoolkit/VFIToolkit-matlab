...
while PricePathDist>transpathoptions.tolerance && pathcounter<transpathoptions.maxiterations
    ...
    for i=1:T-1 %so t=T-i
        ...
        for z_c=1:N_z
            ...
            temp2(isfinite(ReturnMatrix_z))=ReturnMatrix(isfinite(ReturnMatrix_z)).^(1-1/DiscountFactorParamsVec(3));
            temp2=(1-DiscountFactorParamsVec(1))*temp2;
            
            %Calc the condl expectation term (except beta), which depends on z but
            %not on control variables
            EV_z=Vtemp.*(ones(N_a,1,'gpuArray')*pi_z(z_c,:)); %kron(ones(N_a,1),pi_z(z_c,:));
            ...
            entireRHS=temp2+DiscountFactorParamsVec(1)*temp3*ones(1,N_a,1); %aprime by 1
