...
FnInputNames={}; % This is what we want to create
if isempty(Names_i)
    %% Get all the input names for FnsToEvaluate [these are where we might find _tplus1, _tminus1 being used]
...
elseif ~isempty(Names_i)
    % Redo but this time looking for when there is Names_i [have to allow for the possibility that FnsToEvalute depends on ptype]
    %% Get all the input names for FnsToEvaluate [these are where we might find _tplus1, _tminus1 being used]
    AggVarNames=fieldnames(FnsToEvaluate);
    for ff=1:length(AggVarNames)
        if ~isstruct(FnsToEvaluate.(AggVarNames{ff})) % usual
            temp=getAnonymousFnInputNames(FnsToEvaluate.(AggVarNames{ff}));
            FnInputNames={FnInputNames{:},temp{:}}; % Note, this will include the (d,aprime,a,z), but that is irrelevant to our current purposes
        else % FnsToEvaluate.(AggVarNames{ff}) is a structure, which denotes dependence on Names_i
            for ii=1:length(Names_i)
                if isfield(FnsToEvaluate.(AggVarNames{ff}), Names_i{ii}) % this Fn need not be defined for every ptype (matches PType_FnsToEvaluate convention)
...
%% Get all the input names for GeneralEqmEqns [these are where we might find _tplus1, _tminus1 being used]
...
elseif prod(tminus1UsedAsPriceOrAggVar)==0
    error('FnsToEvaluate or GeneralEqmEqns are trying to use a _tminus1 input that is NEITHER a price NOR an an aggregate (determined by FnsToEvaluate)')
end
...
if isfield(transpathoptions,'verbose') && transpathoptions.verbose==2
    fprintf('Info on what _tminus1 and _tplus1 have been detected')
    use_tplus1price
    use_tminus1price
    use_tminus1params
    use_tminus1AggVars
