function varargout=PricePathParamPath_StructToMatrix(PricePathStruct,ParamPathStruct,T,N_i)
...
if ~exist('N_i','var')
    ...
    % ParamPath is matrix of size T-by-'number of parameters that change over the transition path'.
    % Actually, some of those prices are 1-by-N_i, so is more subtle than this.
    PricePathNames=fieldnames(PricePathStruct);
    PricePathSizeVec=zeros(1,length(PricePathNames)); % Allows for a given price param to depend on age (or permanent type)
    for pp=1:length(PricePathNames)
        if isstruct(PricePathStruct.(PricePathNames{pp}))
            ...
            if ~any(PricePathSizeVec(pp)==[1,N_i])
                error(['PricePath for ', PricePathNames{pp}, ' appears to be the wrong size (should be 1-by-T or N_i-by-T)'])
            end
        else
            temp=PricePathStruct.(PricePathNames{pp});
            tempsize=size(temp);
    ...
    for pp=1:length(PricePathNames)
        if isstruct(PricePathStruct.(PricePathNames{pp})) % depends on ptype as structure
            ...
            for ii=1:length(tempptypenames)
                if size(PricePathStruct.(PricePathNames{pp}).(tempptypenames{ii}),1)==T % Note: size(PricePathStruct.(PricePathNames{pp}),2) will be 1 or N_i
    ...
    for pp=1:length(ParamPathNames)
        if isstruct(ParamPathStruct.(ParamPathNames{pp}))
            ...
            if ~any(ParamPathSizeVec(pp)==[1,N_i])
                error(['ParamPath for ', ParamPathNames{pp}, ' appears to be the wrong size (should be 1-by-T or N_i-by-T)'])
            end
    ...
    for pp=1:length(ParamPathNames)
        if isstruct(ParamPathStruct.(ParamPathNames{pp}))
            ...
            for ii=1:length(tempptypenames)
                if size(ParamPathStruct.(ParamPathNames{pp}).(tempptypenames{ii}),1)==T % Note: size(PricePathStruct.(PricePathNames{pp}),2) will be 1 or N_i
