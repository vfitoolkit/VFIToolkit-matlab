function [V, Policy]=ValueFnIter_Case1_TPath_SingleStep_DC2_nod_raw(Vnext,n_a,n_z, a_grid, z_gridvals,pi_z, ReturnFn, Parameters, DiscountFactorParamNames, ReturnFnParamNames, vfoptions)
...
entireRHS_ii=ReturnMatrix_iijj+DiscountFactorParamsVec*reshape(EV,[N_a1*N_a2,1,1,N_z]); % autoexpand (a,z)
...
maxgap1=max(maxindex11(1,2:end,2:end,:)-maxindex11(1,1:end-1,1:end-1,:),[],4);
maxgap2=max(maxindex12(1,2:end,2:end,:)-maxindex12(1,1:end-1,1:end-1,:),[],4);
for ii=1:(vfoptions.level1n(1)-1)
    for jj=1:(vfoptions.level1n(2)-1)
        ...
        if maxgap1(1,ii,jj)>0 && maxgap2(1,ii,jj)>0
            loweredge1=min(maxindex11(1,ii,jj,:),N_a1-maxgap1(1,ii,jj,:)); % maxindex1(ii,:), but avoid going off top of grid when we add maxgap(ii) points
            loweredge2=min(maxindex12(1,ii,jj,:),N_a2-maxgap2(1,ii,jj,:)); % maxindex1(ii,:), but avoid going off top of grid when we add maxgap(ii) points
            % loweredge is 1-by-1-by-1-by-n_z
            a1primeindexes=loweredge1+repmat((0:1:maxgap1(1,ii,jj))',maxgap2(1,ii,jj)+1,1);
            a2primeindexes=loweredge2+repelem((0:1:maxgap2(1,ii,jj))',maxgap1(1,ii,jj)+1,1);
            ...
            aprimez=repmat(a1primeindexes,1,level11iidiff(ii),1,1)+N_a1*repelem(a2primeindexes-1,1,1,level12jjdiff(jj),1)+N_a*shiftdim((0:1:N_z-1),-2); % the current aprimeii(ii):aprimeii(ii+1)
            entireRHS_ii=ReturnMatrix_iijj+DiscountFactorParamsVec*EV(reshape(aprimez,[(maxgap1(1,ii,jj)+1)*(maxgap2(1,ii,jj)+1),level11iidiff(ii),level12jjdiff(jj),N_z]));
            ...
            a1primeind=rem(maxindex-1,maxgap1(1,ii,jj)+1)+1;
            a2primeind=ceil(maxindex/(maxgap1(1,ii,jj)+1));
            ...
            zind=1:1:N_z; % already includes -1
            Policy(curra1index,curra2index,:)=shiftdim(maxindexfix+(loweredge1(zind)-1)+N_a1*(loweredge2(zind)-1),1);

        elseif maxgap1(1,ii,jj)>0 && maxgap2(1,ii,jj)==0
            loweredge1=min(maxindex11(1,ii,jj,:),N_a1-maxgap1(1,ii,jj,:)); % maxindex1(ii,:), but avoid going off top of grid when we add maxgap(ii) points
            loweredge2=maxindex12(1,ii,jj,:);
            % loweredge is 1-by-1-by-1-by-n_z
            a1primeindexes=loweredge1+(0:1:maxgap1(1,ii,jj))';
            ...
            aprimez=repmat(a1primeindexes,1,level11iidiff(ii),1,1)+N_a1*repelem(loweredge2-1,1,1,level12jjdiff(jj),1)+N_a*shiftdim((0:1:N_z-1),-2); % the current aprimeii(ii):aprimeii(ii+1)
            entireRHS_ii=ReturnMatrix_iijj+DiscountFactorParamsVec*EV(reshape(aprimez,[(maxgap1(1,ii,jj)+1)*1,level11iidiff(ii),level12jjdiff(jj),N_z]));
            [Vtempii,maxindex]=max(entireRHS_ii,[],1);
            V(curra1index,curra2index,:)=shiftdim(Vtempii,1);
            ...
            %  the a2prime is relative to loweredge2(zind), need to 'add'  the loweredge2
            zind=1:1:N_z; % already includes -1
            Policy(curra1index,curra2index,:)=shiftdim(maxindex+(loweredge1(zind)-1)+N_a1*(loweredge2(zind)-1),1);
        elseif maxgap1(1,ii,jj)==0 && maxgap2(1,ii,jj)>0
            loweredge1=maxindex11(1,ii,jj,:);
            loweredge2=min(maxindex12(1,ii,jj,:),N_a2-maxgap2(1,ii,jj,:)); % maxindex1(ii,:), but avoid going off top of grid when we add maxgap(ii) points
            % loweredge is 1-by-1-by-1-by-n_z (a1prime&a2prime,a1,a2,z)
            a2primeindexes=loweredge2+(0:1:maxgap2(1,ii,jj))';
        ...
        else % maxgap1(1,ii,jj)==0 && maxgap2(1,ii,jj)==0
            loweredge1=maxindex11(1,ii,jj,:);
            loweredge2=maxindex12(1,ii,jj,:);
            % loweredge is 1-by-1-by-1-by-n_z (a1prime&a2prime,a1,a2,z)
            ...
            aprimez=repmat(loweredge1,1,level11iidiff(ii),1,1)+N_a1*repelem(loweredge2-1,1,1,level12jjdiff(jj),1)+N_a*shiftdim((0:1:N_z-1),-2); % the current aprimeii(ii):aprimeii(ii+1)
            entireRHS_ii=ReturnMatrix_iijj+DiscountFactorParamsVec*EV(reshape(aprimez,[1*1,level11iidiff(ii),level12jjdiff(jj),N_z]));
            [Vtempii,maxindex]=max(entireRHS_ii,[],1);
            V(curra1index,curra2index,:)=shiftdim(Vtempii,1);
            % maxindex needs to be reworked:
            %  the a2prime is only an 'after maxgap(ii)+1, but needs to be after N_a1'
            a1primeind=ones(size(maxindex));
            a2primeind=ones(size(maxindex));
            maxindexfix=a1primeind+N_a1*(a2primeind-1); % put maxindex back together, using N_a1 to determine a2prime, rather than using (maxgap(ii)+1) which is what it originally was in maxindex
            %  the a1prime is relative to loweredge1(zind), need to 'add'  the loweredge1
            %  the a2prime is relative to loweredge2(zind), need to 'add'  the loweredge2
            zind=1:1:N_z; % already includes -1
