...
while PricePathDist>transpathoptions.tolerance
    PolicyIndexesPath=zeros(1,N_a,N_z,T-1); %Periods 1 to T-1
...
%             end
%             for a_c=1:N_a
%                 entireRHS=ReturnMatrix(:,a_c,z_c)+beta*RHSpart2; %aprime by 1
%                 %calculate in order, the maximizing aprime indexes
%                 [V(a_c,z_c),maxindex]=max(entireRHS,[],1);
%                 PolicyIndexes(:,a_c,z_c)=[rem(maxindex,N_d);floor(maxindex/N_d)+1];
%             end
%         end
...
%         SteadyStateDistnext=P*SteadyStateDist;
        ...
        optaprime=shiftdim(ceil(PolicyIndexes/N_d),-1); % This shipting of dimensions is probably not necessary
    ...
    PricePathOld(2:T-1)=transpathoptions.oldpathweight*PricePathOld(2:T-1)+(1-transpathoptions.oldpathweight)*PricePathNew(2:T-1);
