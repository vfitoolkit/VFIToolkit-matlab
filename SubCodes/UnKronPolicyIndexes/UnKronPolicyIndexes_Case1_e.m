function Policy=UnKronPolicyIndexes_Case1_e(PolicyKron, n_d,n_a, n_z,n_e, vfoptions)
% Can use vfoptions OR simoptions
% Input: PolicyKron is (2,N_a,N_z,N_e) first dim indexes the optimal choice for d and aprime
%                      (1,N_a,N_z,N_e) if there is no d
%    vfoptions.gridinterplayer=1 will mean the first dimension has one extra value (so 3 if d, 2 without)
% Output: Policy is (l_d+l_a,n_a,n_z,n_e);
...
if n_d(1)==0
    Policy=zeros(l_aprime+extra,N_a,N_z,N_e,'gpuArray');

    Policy(1,:,:,:)=rem(PolicyKron(1,:,:,:)-1,n_aprime(1))+1;
    if l_aprime>1
        if l_aprime>2
            for ii=2:l_aprime-1
                Policy(ii,:,:,:)=rem(ceil(PolicyKron(1,:,:,:)/prod(n_aprime(1:ii-1)))-1,n_a(ii))+1;
            end
        end
    ...
    Policy=reshape(Policy,[l_aprime+extra,n_a,n_z,n_e]);
else
    l_d=length(n_d);
    Policy=zeros(l_d+l_aprime+extra,N_a,N_z,N_e,'gpuArray');
    ...
    Policy=reshape(Policy,[l_d+l_aprime+extra,n_a,n_z,n_e]);
end
