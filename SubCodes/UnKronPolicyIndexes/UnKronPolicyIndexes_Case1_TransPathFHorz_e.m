function Policy=UnKronPolicyIndexes_Case1_TransPathFHorz_e(Policy, n_d, n_a, n_z,n_e,N_j,T)
...
if N_d==0
    ...
    PolicyTemp=zeros(l_a,N_a,N_z,N_e,N_j,T,'gpuArray');
    ...
    Policy=reshape(PolicyTemp,[l_a,n_a,n_z,n_e,N_j,T]);
        
else
    l_d=length(n_d);
    l_da=length(n_d)+length(n_a);
    n_da=[n_d,n_a];
    PolicyTemp=zeros(l_da,N_a,N_z,N_e,N_j,T,'gpuArray');
    ...
    Policy=reshape(PolicyTemp,[l_da,n_a,n_z,n_e,N_j,T]);
end
