...
error('THIS IS NOT SETUP CORRECTLY YET')
...
elseif vfoptions.gridinterplayer==1
    l_aprime=length(n_a1)+1;
    n_aprime=[n_a1,vfoptions.ngridinterp+2];
end
...
if l_d1==0
    ...
    % a1prime
    PolicyPath(l_d+1,:,:,:)=shiftdim(rem(PolicyPathKron(2,:,:,:)-1,n_aprime(1))+1,-1);
    if l_aprime>1
        for ii=2:l_aprime-1
            PolicyPath(l_d+ii,:,:,:)=shiftdim(rem(ceil(PolicyPathKron(2,:,:,:)/prod(n_aprime(1:ii-1)))-1,n_aprime(ii))+1,-1);
        end
        PolicyPath(l_d+l_aprime,:,:,:)=shiftdim(ceil(PolicyPathKron(2,:,:,:)/prod(n_aprime(1:l_aprime-1))),-1);
    end
    ...
    % a1prime
    PolicyPath(l_d+1,:,:,:)=shiftdim(rem(PolicyPathKron(3,:,:,:)-1,n_aprime(1))+1,-1);
    if l_aprime>1
        for ii=2:l_aprime-1
            PolicyPath(l_d+ii,:,:,:)=shiftdim(rem(ceil(PolicyPathKron(3,:,:,:)/prod(n_aprime(1:ii-1)))-1,n_aprime(ii))+1,-1);
        end
        PolicyPath(l_d+l_aprime,:,:,:)=shiftdim(ceil(PolicyPathKron(3,:,:,:)/prod(n_aprime(1:l_aprime-1))),-1);
    end
end
