function Policy=UnKronPolicyIndexes_Case1_FHorz_semiz_e(Policy3, n_d1, n_d2, n_a, n_z,n_e,N_j,vfoptions)
    ...
    if vfoptions.parallel~=2
        for a_c=1:N_a
            for z_c=1:N_z
                for e_c=1:N_e
                    for jj=1:N_j
                        optd2indexKron=Policy3(1,a_c,z_c,e_c,jj);
                        optaindexKron=Policy3(2,a_c,z_c,e_c,jj);
                        optD2=ind2sub_homemade(n_d2',optd2indexKron);
                        optA=ind2sub_homemade(n_a',optaindexKron);
                        Policy(:,a_c,z_c,e_c,jj)=[optD2';optA'];
                    end
                end
            end
        end
        Policy=reshape(Policy,[l_d2+l_a,n_a,n_z,n_e,N_j]);
    else
        Policy=zeros(l_d2+l_a,N_a,N_z,N_e,N_j,'gpuArray');
    ...
    if vfoptions.parallel~=2
        for a_c=1:N_a
            for z_c=1:N_z
                for e_c=1:N_e
                    for jj=1:N_j
                        optd1indexKron=Policy3(1,a_c,z_c,e_c,jj);
                        optd2indexKron=Policy3(2,a_c,z_c,e_c,jj);
                        optaindexKron=Policy3(3,a_c,z_c,e_c,jj);
                        optD1=ind2sub_homemade(n_d1',optd1indexKron);
                        optD2=ind2sub_homemade(n_d2',optd2indexKron);
                        optA=ind2sub_homemade(n_a',optaindexKron);
                        Policy(:,a_c,z_c,e_c,jj)=[optD1';optD2';optA'];
                    end
                end
            end
        end
        Policy=reshape(Policy,[l_d1+l_d2+l_a,n_a,n_z,n_e,N_j]);
    else
        Policy=zeros(l_d1+l_d2+l_a,N_a,N_z,N_e,N_j,'gpuArray');
