function Policy=UnKronPolicyIndexes_Case1_FHorz_semiz_e(PolicyKron, n_d1, n_d2, n_a, n_semiz,n_z,n_e,N_j,vfoptions)
% Can use vfoptions OR simoptions
% Input: PolicyKron is (3,N_a,N_semiz,N_z,N_e,N_j); % first dim indexes the optimal choice for (d1,d2,aprime)
...
% Output: Policy (l_d+l_a,n_a,n_semiz,n_z,n_e,N_j);
...
if N_d1==0
    Policy=zeros(l_d2+l_aprime+extra,N_a,N_semiz,N_z,N_e,N_j,'gpuArray');
    ...
    Policy=reshape(Policy,[l_d2+l_aprime+extra,n_a,n_semiz,n_z,n_e,N_j]);

else % N_d1>0
    l_d1=length(n_d1); % Note: this is anyway only used is N_d1~=0

    Policy=zeros(l_d1+l_d2+l_aprimeextra,N_a,N_semiz,N_z,N_e,N_j,'gpuArray');
    ...
    Policy=reshape(Policy,[l_d1+l_d2+l_aprime+extra,n_a,n_semiz,n_z,n_e,N_j]);
end
