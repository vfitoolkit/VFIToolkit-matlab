function PolicyPath=UnKronPolicyIndexes_Case1_TransPathFHorz(PolicyPathKron, n_d, n_a, n_z,N_j,T,vfoptions)
% Can use vfoptions OR simoptions
% Input: PolicyPathKron is (2,N_a,N_z,N_j,T) first dim indexes the optimal choice for d and aprime
%                      (1,N_a,N_z,N_j,T) if there is no d
%    vfoptions.gridinterplayer=1 will mean the first dimension has one extra value (so 3 if d, 2 without)
% Output: Policy is (l_d+l_a,n_a,n_z,N_j,T);
...
if n_d(1)==0
    PolicyPath=zeros(l_aprime+extra,N_a,N_z,N_j,T,'gpuArray');
    ...
    PolicyPath=reshape(PolicyPath,[l_d+l_aprime+extra,n_a,n_z,N_j,T]);
end
