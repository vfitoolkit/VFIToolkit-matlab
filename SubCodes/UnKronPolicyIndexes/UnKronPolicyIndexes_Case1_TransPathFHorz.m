...
if N_d==0
    ...
    PolicyTemp=zeros(l_aprime,N_a,N_z,N_j,T,'gpuArray');
    ...
    Policy=reshape(PolicyTemp,[l_aprime,n_a,n_z,N_j,T]);
        
else
    l_d=length(n_d);
    l_daprime=length(n_d)+l_aprime;
    PolicyTemp=zeros(l_daprime,N_a,N_z,N_j,T,'gpuArray');
    ...
    Policy=reshape(PolicyTemp,[l_daprime,n_a,n_z,N_j,T]);
end
