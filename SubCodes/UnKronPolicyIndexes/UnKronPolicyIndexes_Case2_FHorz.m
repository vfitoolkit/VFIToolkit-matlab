function Policy=UnKronPolicyIndexes_Case2_FHorz(Policy, n_d, n_a, n_z,N_j, vfoptions)
...
if vfoptions.parallel~=2
    PolicyTemp=zeros(l_d,N_a,N_z,N_j);
    for i=1:N_a
        for j=1:N_z
            for k=1:N_j
                optdindexKron=Policy(i,j,k);
                optD=ind2sub_homemade(n_d',optdindexKron);
                PolicyTemp(:,i,j,k)=[optD'];
            end
        end
    end
    Policy=reshape(PolicyTemp,[l_d,n_a,n_z, N_j]);
else
    PolicyTemp=zeros(l_d,N_a,N_z,N_j,'gpuArray');
