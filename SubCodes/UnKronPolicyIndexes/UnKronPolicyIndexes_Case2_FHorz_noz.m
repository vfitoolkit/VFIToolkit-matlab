function Policy=UnKronPolicyIndexes_Case2_FHorz_noz(PolicyKron, n_d, n_a,N_j, vfoptions)
% Can input vfoptions OR simoptions
% Input: PolicyKron=zeros(N_a,N_j); % indexes the optimal choice for d
% Output: Policy is (l_d,n_a,N_j);
%
% Note: if you input (Policy,n_d,N_a,N_j,vfoptions) then the output only unpacks the first dimension and is (l_d,N_a,N_j)
...
Policy=zeros(l_d,N_a,N_j,'gpuArray');
...
Policy=reshape(Policy,[l_d,n_a,N_j]);

% --- TEMPORARY (pilot): append PolicyL2flag as trailing channel for comparison
if isfield(vfoptions,'gridinterplayer') && vfoptions.gridinterplayer==1
    Policy = cat(1, Policy, reshape(PolicyL2flag, [1, n_a, N_j]));
end
