...
if N_d==0
    ...
    elseif vfoptions.gridinterplayer==1
        ...
        PolicyPath(1,:,:,:)=shiftdim(rem(PolicyPathKron(1,:,:,:)-1,n_a(1))+1,-1);
        if l_aprime>1
            if l_aprime>2
                for ii=2:l_aprime-1
                    PolicyPath(ii,:,:,:)=shiftdim(rem(ceil(PolicyPathKron(1,:,:,:)/prod(n_a(1:ii-1)))-1,n_a(ii))+1,-1);
                end
            end
            PolicyPath(l_aprime,:,:,:)=shiftdim(ceil(PolicyPathKron(1,:,:,:)/prod(n_a(1:l_aprime-1))),-1);
        end
...
elseif N_d>0 
    ...
    if l_d>1
        ...
        PolicyPath(l_aprime,:,:,:)=shiftdim(ceil(PolicyPathKron(1,:,:,:)/prod(n_d(1:l_d-1))),-1);
    end
