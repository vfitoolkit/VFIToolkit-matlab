function Policy=UnKronPolicyIndexes_Case1_noz(PolicyKron, n_d,n_a, vfoptions)
% Can use vfoptions OR simoptions
% Input: PolicyKron is (2,N_a) first dim indexes the optimal choice for d and aprime
%                      (1,N_a) if there is no d
%    vfoptions.gridinterplayer=1 will mean the first dimension has one extra value (so 3 if d, 2 without)
% Output: Policy is (l_d+l_a,n_a);
...
if n_d(1)==0
    Policy=zeros(l_aprime+extra,N_a,'gpuArray');
    ...
    Policy=reshape(Policy,[l_aprime+extra,n_a]);
else
    l_d=length(n_d);
    Policy=zeros(l_d+l_aprime+extra,N_a,'gpuArray');
    ...
    if vfoptions.gridinterplayer==1
        Policy(l_d+l_aprime+1,:)=PolicyKron(2,:);
    end

    Policy=reshape(Policy,[l_d+l_aprime+extra,n_a]);
end
