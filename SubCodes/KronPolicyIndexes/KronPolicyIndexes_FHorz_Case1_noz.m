...
elseif simoptions.gridinterplayer==1
    % Under GI the inner KronPolicyIndexes_Case1_noz returns rows:
    ...
    if n_d(1)==0
        PolicyKron=zeros(n_rows_a,N_a,N_j,'gpuArray');
        for jj=1:N_j
            PolicyKron(:,:,jj)=KronPolicyIndexes_Case1_noz(Policy(:,:,jj), n_d, n_a, simoptions);
        end
    else
        PolicyKron=zeros(n_rows_a+1,N_a,N_j,'gpuArray');
        for jj=1:N_j
            PolicyKron(:,:,jj)=KronPolicyIndexes_Case1_noz(Policy(:,:,jj), n_d, n_a, simoptions);
        end
    end
