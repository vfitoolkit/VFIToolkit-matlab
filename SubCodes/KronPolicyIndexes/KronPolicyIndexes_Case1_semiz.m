function PolicyKron=KronPolicyIndexes_Case1_semiz(Policy, n_d1, n_d2, n_a, n_z, n_semiz) %,options)
...
if n_d1(1)==0
    ...
    if isa(Policy,'gpuArray')
        ...
        if l_d2==1        
            ...
            PolicyTemp=(reshape(Policy(1:l_d2,:,:),[l_d2,N_a*N_z*N_semiz])-temp*ones(1,N_a*N_z*N_semiz,'gpuArray')).*([1;temp2(1:end-1)]*ones(1,N_a*N_z*N_semiz,'gpuArray'));
        ...
        if l_a==1        
            ...
            PolicyTemp=(reshape(Policy(l_d2+1:l_d+l_a,:,:),[l_a,N_a*N_z*N_semiz])-temp*ones(1,N_a*N_z*N_semiz,'gpuArray')).*([1;temp2(1:end-1)]*ones(1,N_a*N_z*N_semiz,'gpuArray'));
            PolicyKron(2,:,:,:)=reshape(sum(PolicyTemp,1),[1,N_a,N_z,N_semiz]);
        end
    else
        PolicyKron=zeros(2,N_a,N_z*N_semiz);
        for i=1:N_a
            for j=1:N_z*N_semiz
    ...
    if isa(Policy,'gpuArray')
        ...
        if l_d1==1        
            ...
            PolicyTemp=(reshape(Policy(1:l_d1,:,:),[l_d1,N_a*N_z*N_semiz])-temp*ones(1,N_a*N_z*N_semiz,'gpuArray')).*([1;temp2(1:end-1)]*ones(1,N_a*N_z*N_semiz,'gpuArray'));
            PolicyKron(1,:,:,:)=reshape(sum(PolicyTemp,1),[N_a,N_z,N_semiz]);
        end
        % Then, d2
        if l_d2==1        
            ...
            temp=ones(l_d2,1,'gpuArray')-eye(l_d2,1,'gpuArray');
            temp2=gpuArray(cumprod(n_d2')); % column vector
            PolicyTemp=(reshape(Policy(l_d1+1:l_d1+l_d2,:,:),[l_d2,N_a*N_z*N_semiz])-temp*ones(1,N_a*N_z*N_semiz,'gpuArray')).*([1;temp2(1:end-1)]*ones(1,N_a*N_z*N_semiz,'gpuArray'));
            PolicyKron(2,:,:,:)=reshape(sum(PolicyTemp,1),[1,N_a,N_z,N_semiz]);
        end
        % Then, a
        if l_a==1        
            ...
            temp=ones(l_a,1,'gpuArray')-eye(l_a,1,'gpuArray');
            temp2=gpuArray(cumprod(n_a')); % column vector
            PolicyTemp=(reshape(Policy(l_d1+l_d2+1:l_d1+l_d2+l_a,:,:),[l_a,N_a*N_z*N_semiz])-temp*ones(1,N_a*N_z*N_semiz,'gpuArray')).*([1;temp2(1:end-1)]*ones(1,N_a*N_z*N_semiz,'gpuArray'));
            PolicyKron(3,:,:,:)=reshape(sum(PolicyTemp,1),[1,N_a,N_z,N_semiz]);
        end

    else
        PolicyKron=zeros(3,N_a,N_z*N_semiz);
        for i=1:N_a
            for j=1:N_z*N_semiz
        ...
        PolicyKron=reshape(PolicyKron,[3,N_a,N_z,N_semiz]); %Overwrite
    end
end
