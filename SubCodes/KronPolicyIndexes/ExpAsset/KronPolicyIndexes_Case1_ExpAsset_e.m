function PolicyKron=KronPolicyIndexes_Case1_ExpAsset_e(Policy, n_d, n_a1, n_a2, n_z, n_e, simoptions)
...
if simoptions.gridinterplayer==0
    ...
    if n_d(1)==0
        if l_aprime==1
            ...
            temp2=gpuArray(cumprod(n_a1')); % column vector
            PolicyTemp=(reshape(Policy,[l_aprime,N_a*N_z*N_e])-temp*ones(1,N_a*N_z*N_e,'gpuArray')).*([1;temp2(1:end-1)]*ones(1,N_a*N_z*N_e,'gpuArray'));
        ...
        if l_d==1
            ...
            PolicyKron(1,:,:,:)=reshape(sum(PolicyTemp,1),[1,N_a,N_z,N_e]);
        end
        % Then, a
        if l_aprime==1
...
elseif simoptions.gridinterplayer==1
    ...
    if n_d(1)==0
        if l_aprime==1 || l_aprime==2
            PolicyKron=Policy; % a1, possibly a2, L2
        else %l_a>2
            PolicyKron=zeros(3,N_a,N_z,N_e,'gpuArray');
            PolicyKron(1,:,:,:)=Policy(1,:,:); % a1
            temp=[0; ones(l_aprime-2,1,'gpuArray')];
            temp2=gpuArray(cumprod(n_a1(2:end)')); % column vector
            PolicyTemp=(reshape(Policy(2:l_aprime,:,:,:),[l_aprime-1,N_a*N_z*N_e])-temp).*[1;temp2(1:end-1)];
            PolicyKron(2,:,:,:)=reshape(sum(PolicyTemp,1),[1,N_a,N_z,N_e]);
            PolicyKron(3,:,:,:)=Policy(l_aprime+1,:,:,:); % L2 index
        end
    else
        l_d=length(n_d);
        if l_aprime==1
        ...
        if l_aprime==1
            PolicyKron(2,:,:,:)=Policy(l_d+1,:,:,:);
            PolicyKron(3,:,:,:)=Policy(l_aprime+l_d+1,:,:,:); % L2 index
        elseif l_aprime==2
            PolicyKron(2,:,:,:)=Policy(l_d+1,:,:,:);
            PolicyKron(3,:,:,:)=Policy(l_d+2,:,:,:);            
            PolicyKron(4,:,:,:)=Policy(l_aprime+l_d+1,:,:,:); % L2 index
        else
            PolicyKron(2,:,:,:)=Policy(l_d+1,:,:,:);
            temp=[0; ones(l_aprime-2,1,'gpuArray')];
            temp2=gpuArray(cumprod(n_a1(2:end)')); % column vector
            PolicyTemp=(reshape(Policy(l_d+2:l_d+l_aprime,:,:,:),[l_aprime-1,N_a*N_z*N_e])-temp).*[1;temp2(1:end-1)];
