...
else % Now, check for other options, that might be set (e.g., because of setting internal defaults)
    for ff=1:nFields
...
%% Some other things that are always set when using PType
if ~isfield(options_temp,'verbose')
    options_temp.verbose=0;
end
if ~isfield(options_temp,'verboseparams')
    options_temp.verboseparams=0;
end
