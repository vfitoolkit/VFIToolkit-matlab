function Fmatrix=CreateReturnFnMatrix_Case1_Disc_Par2_fastOLGe(ReturnFn, n_d, n_a, n_z, n_e, N_j, d_grid, a_grid, z_gridvals_J, e_gridvals_J, ReturnFnParamsAgeMatrix)
...
l_e=length(n_e);
if l_d>4
    disp('ERROR: Using GPU for the return fn does not allow for more than four of d variable (you have length(n_d)>4): (in CreateReturnFnMatrix_Case1_Disc_Par2_fastOLGe)')
end
if l_a>4
    disp('ERROR: Using GPU for the return fn does not allow for more than four of a variable (you have length(n_a)>4): (in CreateReturnFnMatrix_Case1_Disc_Par2_fastOLGe)')
end
if l_z>4
    disp('ERROR: Using GPU for the return fn does not allow for more than four of z variable (you have length(n_z)>4): (in CreateReturnFnMatrix_Case1_Disc_Par2_fastOLGe)')
end
if l_e>4
    disp('ERROR: Using GPU for the return fn does not allow for more than four of e variable (you have length(n_e)>4): (in CreateReturnFnMatrix_Case1_Disc_Par2_fastOLGe)')
end

nReturnFnParams=size(ReturnFnParamsAgeMatrix,2);

if nargin(ReturnFn)~=l_d+l_a+l_a+l_z+l_e+nReturnFnParams
    dbstack
...
if l_z>=1 % Note that z_grid is an abnormal shape due to fastOLG so the z dimension is after the j dimension (for the return function params, and possibly to allow z_grid to also depend on j)
    ...
    if l_z>=2
        ...
        if l_z>=3
            ...
            if l_z>=4
                z4vals=shiftdim(z_gridvals_J(:,:,4),-l_d-l_a-l_a);
            end
        end
    end
end
if l_e>=1 % Note that e_grid is an abnormal shape due to fastOLG so the e dimension is after the j dimension (for the return function params, and possibly to allow e_grid to also depend on j)
    e1vals=shiftdim(e_gridvals_J(:,:,:,1),-l_d-l_a-l_a);
    if l_e>=2
        e2vals=shiftdim(e_gridvals_J(:,:,:,2),-l_d-l_a-l_a);
        if l_e>=3
            e3vals=shiftdim(e_gridvals_J(:,:,:,3),-l_d-l_a-l_a);
            if l_e>=4
                e4vals=shiftdim(e_gridvals_J(:,:,:,4),-l_d-l_a-l_a);
            end
        end
    end
end
...
if l_d==0
    Fmatrix=reshape(Fmatrix,[N_a,N_a*N_j,N_z,N_e]);
else
    Fmatrix=reshape(Fmatrix,[N_d*N_a,N_a*N_j,N_z,N_e]);
end
