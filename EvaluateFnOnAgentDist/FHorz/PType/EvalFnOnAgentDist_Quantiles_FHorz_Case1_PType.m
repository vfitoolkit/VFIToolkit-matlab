function Quantiles=EvalFnOnAgentDist_Quantiles_FHorz_Case1_PType(StationaryDist, Policy, FnsToEvaluate, Parameters, FnsToEvaluateParamNames,n_d,n_a,n_z,N_j,Names_i,d_grid, a_grid, z_grid, simoptions)
...
if simoptions.groupptypesforstats==0
    for ii=1:N_i
        ...
        Quantiles.(Names_i{ii})=EvalFnOnAgentDist_Quantiles_FHorz_Case1(StationaryDist_temp,PolicyIndexes_temp, FnsToEvaluate_temp,Parameters_temp,FnsToEvaluateParamNames_temp,n_d_temp,n_a_temp,n_z_temp,N_j_temp,d_grid_temp,a_grid_temp,z_grid_temp,Parallel_temp,simoptions_temp);
    ...
    for kk=1:numFnsToEvaluate % Each of the functions to be evaluated on the grid
        ...
        quantiles=(1:1:simoptions.nquantiles-1)/simoptions.nquantiles;
...
if isstruct(FnsToEvaluate) && simoptions.groupptypesforstats==0
    ...
    Quantiles2=Quantiles;
    Quantiles=struct();
    %     AggVarNames=fieldnames(FnsToEvaluate);
    for ff=1:length(FnNames)
        for ii=1:N_i
            Quantiles.(FnNames{ff}).CutOffs.(Names_i{ii})=Quantiles2.(Names_i{ii}).(FnNames{ff}).CutOffs;
            Quantiles.(FnNames{ff}).Means.(Names_i{ii})=Quantiles2.(Names_i{ii}).(FnNames{ff}).Means;
        end
    end
end
%Note: nothing needs to be done if simoptions.groupptypesforstats==1
