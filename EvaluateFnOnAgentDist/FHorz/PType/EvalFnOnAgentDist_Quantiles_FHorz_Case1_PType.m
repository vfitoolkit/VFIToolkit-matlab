...
if isstruct(FnsToEvaluate) && simoptions.groupptypesforstats==0
    ...
    for ff=1:length(FnNames)
        for ii=1:N_i
            Quantiles.(FnNames{ff}).(Names_i{ii}).CutOffs=Quantiles2.(Names_i{ii}).(FnNames{ff}).CutOffs;
            Quantiles.(FnNames{ff}).(Names_i{ii}).Means=Quantiles2.(Names_i{ii}).(FnNames{ff}).Means;
        end
    end
end
