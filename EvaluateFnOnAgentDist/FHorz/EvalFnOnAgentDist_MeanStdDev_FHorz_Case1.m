function MeanStdDev=EvalFnOnAgentDist_MeanStdDev_FHorz_Case1(StationaryDist,PolicyIndexes, FnsToEvaluate,Parameters,FnsToEvaluateParamNames,n_d,n_a,n_z,N_j,d_grid,a_grid,z_grid,Parallel,simoptions)
% Returns both the mean and standard deviation of each of the FnsToEvaluate
% across the StationaryDist
...
if isstruct(FnsToEvaluate)
    ...
    for ff=1:length(AggVarNames)
        ...
        if length(temp)>(l_d+l_a+l_a+l_z) % Note: l_e is rolled into l_z above
...
%%
if Parallel==2
    MeanStdDev=zeros(length(FnsToEvaluate),2,'gpuArray');
    ...
    for ii=1:length(FnsToEvaluate)
        ...
        % Mean
        MeanStdDev(ii,1)=sum(Values.*StationaryDistVec);
        % Standard Deviation
        MeanStdDev(ii,2)=sqrt(sum(StationaryDistVec.*((Values-MeanStdDev(ii,1).*ones(N_a*N_z*N_j,1)).^2)));
    end
    
else
    MeanStdDev=zeros(length(FnsToEvaluate),2);
    if l_d>0
        d_val=zeros(1,l_d);
    end
